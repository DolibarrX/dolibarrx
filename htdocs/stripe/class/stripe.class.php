...
class Stripe extends CommonObject
{
	 ...
	 *
	 * @param	Societe|Member	$object				Object thirdparty to check, or create on stripe (create on stripe also update the stripe_account table for current entity).  Used for MemberType and Societe.
					...
					try {
						if (empty($stripeacc)) {				// If the Stripe connect account not set, we use common API usage
							if (!getDolGlobalString('STRIPE_USE_INTENT_WITH_AUTOMATIC_CONFIRMATION')) {
								...
								$urtoswitchonstripe = '<a href="'.$url.'" target="_stripe">'.img_picture($langs->trans('ShowInStripe'), 'globe').'</a>';
