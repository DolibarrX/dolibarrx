...
if ($action == 'edit') {
	...
	foreach ($arrayofparameters as $key => $val) {
		...
		print $form->textWithPicture($langs->trans($key), $tooltiphelp);
								...
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										if (getDolGlobalString($constforvar) == $name) {
											...
											print '<a href="'.$_SERVER["PHP_SELF"].'?action=unsetdoc&token='.newToken().'&object='.urlencode(strtolower($myTmpObjectKey)).'&value='.urlencode($name).'&scan_dir='.urlencode($module->scandir).'&label='.urlencode($module->name).'&type='.urlencode($type).'" alt="'.$langs->trans("Disable").'">'.img_picture($langs->trans("Enabled"), 'on').'</a>';
										} else {
											print '<a href="'.$_SERVER["PHP_SELF"].'?action=setdoc&token='.newToken().'&object='.urlencode(strtolower($myTmpObjectKey)).'&value='.urlencode($name).'&scan_dir='.urlencode($module->scandir).'&label='.urlencode($module->name).'" alt="'.$langs->trans("Default").'">'.img_picture($langs->trans("Disabled"), 'off').'</a>';
										}
										print '</td>';
