...
if ($nbModulesNotAutoEnabled <= getDolGlobalInt('MAIN_MIN_NB_ENABLED_MODULE_FOR_WARNING', 1)) {	// If only autoenabled modules (property ->enabledByDefault in modules) are activated
	header("Location: " . DOL_URL_ROOT . "/admin/index.php?mainmenu=home&leftmenu=setup&mesg=setupnotcomplete");
	exit;
}
...
function getWeatherStatus($totallate)
{
	...
	$usedConf = (getDolGlobalString('MAIN_USE_METEO_WITH_PERCENTAGE') ? 'MAIN_METEO_PERCENTAGE_LEVEL' : 'MAIN_METEO_LEVEL');

	$weather->level = 0;

	$level0 = $offset;
	$level0 = getDolGlobalString($usedConf . '0', $level0);
	$level1 = $offset + $factor;
	$level1 = getDolGlobalString($usedConf . '1', $level1);
	$level2 = $offset + 2 * $factor;
	$level2 = getDolGlobalString($usedConf . '2', $level2);
