...
function skillPrepareHead($object)
{
	...
	$upload_dir = $config->hrm->dir_output."/skill/".dol_sanitizeFileName($object->ref);
	...
	//	'entity:-tabname:Title:@hrm:/hrm/mypage.php?id=__ID__'
	//); // to remove a tab
	complete_head_from_modules($config, $langs, $object, $head, $h, 'skill@hrm');

	complete_head_from_modules($config, $langs, $object, $head, $h, 'skill@hrm', 'remove');
