...
$upload_dir = $config->hrm->multidir_output[isset($object->entity) ? $object->entity : 1] . '/job';
...
if ($action == 'create') {
	...
	print '&nbsp; ';
	print '<input type="' . ($backtopage ? "submit" : "button") . '" class="button button-cancel" name="cancel" value="' . dol_escape_htmltag($langs->trans("Cancel")) . '"' . ($backtopage ? '' : ' onclick="history.go(-1)"') . '>'; // Cancel for create does not post form if we do not know the backtopage
	print '</div>';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action != 'presend') {
		...
		$tmparray = $form->showLinkToObjectBlock($object, [], array('job'), 1);
