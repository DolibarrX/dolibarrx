...
class Position extends CommonObject
{
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($config->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->hrm->position->write))
		 || (!empty($config->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->hrm->position->position_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql .= " WHERE filename LIKE '" . $this->db->escape($this->ref) . "%' AND filepath = 'position/" . $this->db->escape($this->ref) . "' and entity = " . $config->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$sql .= " WHERE filepath = 'position/".$this->db->escape($this->ref)."' and entity = ".$config->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $config->hrm->dir_output . '/position/' . $oldref;
				$dirdest = $config->hrm->dir_output . '/position/' . $newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($config->hrm->dir_output . '/position/' . $newref, 'files', 1, '^' . preg_quote($oldref, '/'));
	...
	public function getNomUrl($withPicture = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picture('', $this->picture) . ' <u>' . $langs->trans("Position") . '</u>';
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('hrm_POSITION_ADDON')) {
			$config->global->hrm_POSITION_ADDON = 'mod_position_standard';
		}
