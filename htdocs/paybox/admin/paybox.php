...
if ($action == 'setvalue' && $user->admin) {
	$db->begin();
	//$result=dolibarr_set_const($db, "PAYBOX_IBS_DEVISE", GETPOST("PAYBOX_IBS_DEVISE"),'chaine',0,'',$config->entity);
	$result = dolibarr_set_const($db, "PAYBOX_CGI_URL_V1", GETPOST('PAYBOX_CGI_URL_V1', 'alpha'), 'chaine', 0, '', $config->entity);
	if (!($result > 0)) {
		$error++;
	}
	$result = dolibarr_set_const($db, "PAYBOX_CGI_URL_V2", GETPOST('PAYBOX_CGI_URL_V2', 'alpha'), 'chaine', 0, '', $config->entity);
	if (!($result > 0)) {
		$error++;
	}
	$result = dolibarr_set_const($db, "PAYBOX_IBS_SITE", GETPOST('PAYBOX_IBS_SITE', 'alpha'), 'chaine', 0, '', $config->entity);
	if (!($result > 0)) {
		$error++;
	}
	$result = dolibarr_set_const($db, "PAYBOX_IBS_RANG", GETPOST('PAYBOX_IBS_RANG', 'alpha'), 'chaine', 0, '', $config->entity);
	if (!($result > 0)) {
		$error++;
	}
	$result = dolibarr_set_const($db, "PAYBOX_PBX_IDENTIFIANT", GETPOST('PAYBOX_PBX_IDENTIFIANT', 'alpha'), 'chaine', 0, '', $config->entity);
	if (!($result > 0)) {
		$error++;
	}
	...
	$result = dolibarr_set_const($db, "PAYBOX_BANK_ACCOUNT_FOR_PAYMENTS", GETPOSTINT('PAYBOX_BANK_ACCOUNT_FOR_PAYMENTS'), 'chaine', 0, '', $config->entity);
	if (!($result > 0)) {
		$error++;
	}
	...
	$result = dolibarr_set_const($db, "PAYBOX_HMAC_KEY", dol_encode(GETPOST('PAYBOX_HMAC_KEY', 'alpha')), 'chaine', 0, '', $config->entity);
	if (!($result > 0)) {
		$error++;
	}
...
if (!getDolGlobalString('PAYBOX_IBS_SITE')) {
	$config->global->PAYBOX_IBS_SITE = $IBS_SITE;
}
$IBS_RANG = "99"; // Rang test
if (!getDolGlobalString('PAYBOX_IBS_RANG')) {
	$config->global->PAYBOX_IBS_RANG = $IBS_RANG;
}
$IBS_DEVISE = "978"; // Euro
if (!getDolGlobalString('PAYBOX_IBS_DEVISE')) {
	$config->global->PAYBOX_IBS_DEVISE = $IBS_DEVISE;
}
...
print '<input size="100" type="text" name="PAYBOX_HMAC_KEY" value="'.dol_decode($config->global->PAYBOX_HMAC_KEY).'">';
...
print '<input size="32" type="text" name="PAYBOX_IBS_DEVISE" value="'.$config->global->PAYBOX_IBS_DEVISE.'">';
...
print '<input size="64" type="text" name="PAYBOX_CGI_URL_V1" value="'.$config->global->PAYBOX_CGI_URL_V1.'">';
...
if (isModEnabled("bank")) {
	...
	$form->select_comptes($config->global->PAYBOX_BANK_ACCOUNT_FOR_PAYMENTS, 'PAYBOX_BANK_ACCOUNT_FOR_PAYMENTS', 0, '', 1);
	print '</td></tr>';
}
...
$doleditor = new DolEditor('ONLINE_PAYMENT_MESSAGE_FORM', $config->global->ONLINE_PAYMENT_MESSAGE_FORM, '', 100, 'dolibarr_details', 'In', false, true, true, ROWS_2, '90%');
$doleditor->Create();
print '</td></tr>';

print '<tr class="oddeven"><td>';
print $langs->trans("MessageOK").'</td><td>';
$doleditor = new DolEditor('ONLINE_PAYMENT_MESSAGE_OK', $config->global->ONLINE_PAYMENT_MESSAGE_OK, '', 100, 'dolibarr_details', 'In', false, true, true, ROWS_2, '90%');
$doleditor->Create();
print '</td></tr>';
...
$doleditor = new DolEditor('ONLINE_PAYMENT_MESSAGE_KO', $config->global->ONLINE_PAYMENT_MESSAGE_KO, '', 100, 'dolibarr_details', 'In', false, true, true, ROWS_2, '90%');
$doleditor->Create();
print '</td></tr>';
...
print $form->selectyesno("PAYMENT_SECURITY_TOKEN_UNIQUE", (!getDolGlobalString('PAYMENT_SECURITY_TOKEN') ? 0 : $config->global->PAYMENT_SECURITY_TOKEN_UNIQUE), 1);
print '</td></tr>';
