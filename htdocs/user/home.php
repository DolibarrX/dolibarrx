...
if ($resql) {
	...
	while ($i < $num && $i < $max) {
		...
		if (isModEnabled('multicompany') && $obj->admin && !$obj->entity) {
			$lastcreatedbox .= img_picture($langs->trans("SuperAdministratorDesc"), 'redstar');
		} elseif ($obj->admin) {
			$lastcreatedbox .= img_picture($langs->trans("AdministratorDesc"), 'star');
		}
		$lastcreatedbox .= "</td>";
...
if ($permissiontoreadgroup) {
	...
	if ($resql) {
		...
		while ($i < $num && (!$max || $i < $max)) {
			...
			if (!$obj->entity) {
				$lastgroupbox .= img_picture($langs->trans("GlobalGroup"), 'redstar');
			}
			$lastgroupbox .= "</td>";
			if (isModEnabled('multicompany') && is_object($mc)) {
