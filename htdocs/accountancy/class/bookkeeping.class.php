...
class BookKeeping extends CommonObject
{
	...
	public function create(User $user, $notrigger = 0)
	{
		...
		if (isset($this->number_compte)) {
			$this->number_compte = trim($this->number_compte);
		}
		...
		// Check parameters
		if (($this->number_compte == "") || $this->number_compte == '-1' || $this->number_compte == 'NotDefined') {
			...
			if (in_array($this->doc_type, array('bank', 'expense_report'))) {
				...
				$mesg = $this->doc_ref . ', ' . $langs->trans("AccountAccounting") . ': ' . ($this->number_compte != -1 ? $this->number_compte : $langs->trans("Unknown"));
				if ($this->subledger_account && $this->subledger_account != $this->number_compte) {
		...
		// Note that we must include 'doc_type - fk_doc - number_compte - label - subledger_account (if not empty)' to be sure to have unicity of line (because we may have several lines
		// with same doc_type, fk_doc, number_compte for 1 invoice line when using localtaxes with same account)
		...
		$sql .= " AND number_compte = '" . $this->db->escape($this->number_compte) . "'";
		$sql .= " AND label_operation = '" . $this->db->escape($this->label_operation) . "'";
		if (!empty($this->subledger_account)) {
			$sql .= " AND subledger_account = '" . $this->db->escape($this->subledger_account) . "'";
		}
		$sql .= " AND entity = " . $config->entity; // Do not use getEntity for accounting features
		...
		if ($resql) {
			...
			if ($row->nb == 0) {	// Not already into bookkeeping
				...
				$sqlnum .= " AND entity = " . $config->entity; // Do not use getEntity for accounting features
				...
				if (empty($this->piece_num)) {
					...
					$sqlnum .= " FROM " . $this->db->prefix() . $this->table_element;
					$sqlnum .= " WHERE entity = " . ((int) $config->entity); // Do not use getEntity for accounting features
				...
				$sql .= ", '" . $this->db->escape($this->number_compte) . "'";
	...
	public function createStd(User $user, $notrigger = 0, $mode = '')
	{
		...
		$sql .= ' ' . (!isset($this->number_compte) ? 'NULL' : "'" . $this->db->escape($this->number_compte) . "'") . ',';
	...
	public function fetch($id, $ref = null, $mode = '')
	{
		...
		$sql .= ' WHERE 1 = 1';
		$sql .= " AND entity = " . ((int) $config->entity); // Do not use getEntity for accounting features
		if (null !== $ref) {
		...
		if ($resql) {
			...
			if ($numrows) {
				...
				$this->number_compte = $obj->number_compte;
	...
	public function fetchAllByAccount($sortorder = '', $sortfield = '', $limit = 0, $offset = 0, array $filter = array(), $filtermode = 'AND', $option = 0, $countonly = 0)
	{
		...
		if (count($filter) > 0) {
			foreach ($filter as $key => $value) {
				if ($key == 't.doc_date>=') {
					...
					$sqlwhere[] = "t.number_compte >= '" . $this->db->escape($value) . "'";
				} elseif ($key == 't.number_compte<=') {
					$sqlwhere[] = "t.number_compte <= '" . $this->db->escape($value) . "'";
					...
					$sqlwhere[] = 't.number_compte IN (' . $this->db->sanitize($value, 1) . ')';
				} else {
		...
		if (!empty($option)) {
			...
			$sortfield = 't.number_compte' . ($sortfield ? ',' . $sortfield : '');
	...
	public function fetchAll($sortorder = '', $sortfield = '', $limit = 0, $offset = 0, $filter = '', $filtermode = 'AND', $showAlreadyExportMovements = 1)
	{
		...
		$sql .= ' WHERE t.entity = ' . ((int) $config->entity); // Do not use getEntity for accounting features
	...
	public function fetchAllBalance($sortorder = '', $sortfield = '', $limit = 0, $offset = 0, $filter = '', $filtermode = 'AND', $option = 0)
	{
		...
		if (!empty($option)) {
			$sql .= " AND t.subledger_account IS NOT NULL";
			$sql .= " AND t.subledger_account <> ''";
			$sql .= " GROUP BY t.number_compte, t.subledger_account, t.subledger_label";
			$sortfield = 't.subledger_account' . ($sortfield ? ',' . $sortfield : '');
			$sortorder = 'ASC' . ($sortfield ? ',' . $sortfield : '');
		} else {
	...
	public function update(User $user, $notrigger = 0, $mode = '')
	{
		...
		$sql .= ' number_compte = ' . (isset($this->number_compte) ? "'" . $this->db->escape($this->number_compte) . "'" : "null") . ',';
	...
	public function fetchAllPerMvt($piecenum, $mode = '')
	{
		...
		$sql .= " number_compte, label_compte, label_operation, debit, credit,";
	...
	public function transformTransaction($direction = 0, $piece_num = '')
	{
		...
		if ($direction == 0) {
			...
			if (!$error) {
				...
				$sql = 'DELETE FROM ' . $this->db->prefix() . $this->table_element . '_tmp WHERE piece_num = ' . ((int) $piece_num) . ' AND entity = ' . ((int) $config->entity) . " AND number_compte IS NULL AND debit = 0 AND credit = 0";
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->errors[] = 'Error ' . $this->db->lasterror();
			...
			if (!$error) {
				...
				$sql .= ' FROM ' . $this->db->prefix() . $this->table_element . '_tmp WHERE piece_num = ' . ((int) $piece_num) . ' AND number_compte IS NOT NULL AND entity = ' . ((int) $config->entity);
				$sql .= $sql_filter;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
			...
			if (!$error) {
				$sql = 'DELETE FROM ' . $this->db->prefix() . $this->table_element . '_tmp WHERE piece_num = ' . ((int) $piece_num) . ' AND entity = ' . ((int) $config->entity);
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->errors[] = 'Error ' . $this->db->lasterror();
			...
			if (!$error) {
				...
				$sql .= ' number_compte, label_compte, label_operation, debit, credit,';
				$sql .= ' montant, sens, fk_user_author, import_key, code_journal, journal_label, piece_num';
				$sql .= ' FROM ' . $this->db->prefix() . $this->table_element . ' WHERE piece_num = ' . ((int) $piece_num) . ' AND entity = ' . ((int) $config->entity);
				$sql .= $sql_filter;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
		...
		$sql .= " LEFT JOIN " . $this->db->prefix() . "accounting_account as aa ON aa.account_number = ab.number_compte";
		$sql .= " AND aa.active = 1";
	...
	public function select_account($selectid, $htmlname = 'account', $showempty = 0, $event = array(), $select_in = 0, $select_out = 0, $aabase = '')
	{
		...
		$sql = "SELECT DISTINCT ab.number_compte as account_number, aa.label as label, aa.rowid as rowid, aa.fk_pcg_version";
		$sql .= " FROM " . $this->db->prefix() . $this->table_element . " as ab";
		$sql .= " LEFT JOIN " . $this->db->prefix() . "accounting_account as aa ON aa.account_number = ab.number_compte";
		$sql .= " AND aa.active = 1";
		$sql .= " INNER JOIN " . $this->db->prefix() . "accounting_system as asy ON aa.fk_pcg_version = asy.pcg_version";
		$sql .= " AND asy.rowid = " . ((int) $pcgver);
		$sql .= " AND ab.entity = " . ((int) $config->entity); // Do not use getEntity for accounting features
		$sql .= " ORDER BY account_number ASC";
	...
	public function getRootAccount($account = null)
	{
		...
		$sql .= " AND aa.entity = " . ((int) $config->entity); // Do not use getEntity for accounting features
	...
	public function get_compte_desc($account = null)
	{
		...
		$sql .= " WHERE aa.entity = " . ((int) $config->entity); // Do not use getEntity for accounting features
	...
	public function canModifyBookkeeping($id, $mode = '')
	{
		...
		if (getDolGlobalString('ACCOUNTANCY_FISCAL_PERIOD_MODE') == 'blockedonclosed') {
			...
			if (!empty($config->cache['closed_fiscal_period_cached']) && is_array($config->cache['closed_fiscal_period_cached'])) {
				foreach ($config->cache['closed_fiscal_period_cached'] as $fiscal_period) {
	...
	public function loadFiscalPeriods($force = false, $mode = 'active')
	{
		...
		if ($mode == 'active') {
			if (!isset($config->cache['active_fiscal_period_cached']) || $force) {
				...
				$config->cache['active_fiscal_period_cached'] = $list;
			}
		}
		if ($mode == 'closed') {
			if (!isset($config->cache['closed_fiscal_period_cached']) || $force) {
				$sql = "SELECT date_start, date_end";
				$sql .= " FROM " . $this->db->prefix() . "accounting_fiscalyear";
				$sql .= " WHERE entity = " . ((int) $config->entity);
				$sql .= " AND statut = 1";
				...
				$config->cache['closed_fiscal_period_cached'] = $list;
			}
		}
	...
	public function getCountByMonthForFiscalPeriod($date_start, $date_end)
	{
		...
		if (getDolGlobalString("ACCOUNTANCY_DISABLE_CLOSURE_LINE_BY_LINE")) {
			...
			$sql .= " AND NOT EXISTS (SELECT rowid FROM " . MAIN_DB_PREFIX . 'accounting_fiscalyear as af WHERE b.doc_date >= af.date_start AND b.doc_date <= af.date_end AND af.entity = ' . ((int) $config->entity) . " AND af.statut = 1)";
		} else {
	...
	public function accountingResult($date_start, $date_end)
	{
		...
		if (getDolGlobalString('ACCOUNTING_CLOSURE_ACCOUNTING_GROUPS_USED_FOR_INCOME_STATEMENT')) {
			...
			$sql .= ' FROM ' . $this->db->prefix() . $this->table_element . ' as t';
			$sql .= ' LEFT JOIN  ' . $this->db->prefix() . 'accounting_account as aa ON aa.account_number = t.number_compte';
			$sql .= ' WHERE t.entity = ' . ((int) $config->entity); // Do not use getEntity for accounting features
			...
			$sql .= ' GROUP BY t.number_compte, aa.pcg_type';
	...
	public function closeFiscalPeriod($fiscal_period_id, $new_fiscal_period_id, $separate_auxiliary_account = false, $generate_bookkeeping_records = true)
	{
		...
		if (!$error && !empty($generate_bookkeeping_records)) {
			...
			if (!$error && is_object($journal)) {
				...
				$sql .= $this->db->order("t.number_compte", "ASC");
				...
				if (!$resql) {
					...
					while ($obj = $this->db->fetch_object($resql)) {
						if (in_array($obj->pcg_type, $accounting_groups_used_for_income_statement)) {
							...
							$bookkeeping->number_compte = $obj->number_compte;
							$accountingaccount = new AccountingAccount($this->db);
							$accountingaccount->fetch(0, $obj->number_compte);
					...
					if (!$error && $income_statement_amount != 0) {
						...
						$bookkeeping->number_compte = $accountingaccount->account_number;
