...
if ($result) {
	...
	while ($i < $num) {
		...
		//$tabfac[$obj->rowid]["fk_invoicedet"] = $obj->fdid;
...
foreach ($tabfac as $key => $val) {		// Loop on each invoice
	...
	$sql .= " AND fd.total_ttc <> 0 AND fk_invoice = ".((int) $key);
	$resql = $db->query($sql);
	if ($resql) {
...
FROM
    ".MAIN_DB_PREFIX."invoicedet as fd
WHERE
    fd.product_type <= 2
    AND fd.fk_code_ventilation <= 0
    AND fd.total_ttc <> 0
    AND fk_invoice IN (".$db->sanitize(implode(",", array_keys($tabfac))).")
GROUP BY fk_invoice
";
$resql = $db->query($sql);
if ($resql) {
...
if ($action == 'writebookkeeping' && !$error && $user->hasRight('accounting', 'bind', 'write')) {
	...
	foreach ($tabfac as $key => $val) {		// Loop on each invoice
		...
		$replacedinvoice = 0;
		if ($invoicestatic->close_code == Invoice::CLOSECODE_REPLACED) {
			$replacedinvoice = 1;
