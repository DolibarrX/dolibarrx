...
if (($action == 'clean' || $action == 'validatehistory') && $user->hasRight('accounting', 'bind', 'write')) {
	...
	$db->begin();
	$sql1 = "UPDATE ".$db->prefix()."invoice_fourn_det as fd";
	$sql1 .= " SET fk_code_ventilation = 0";
	$sql1 .= ' WHERE fd.fk_code_ventilation NOT IN';
	$sql1 .= '	(SELECT accnt.rowid ';
	$sql1 .= "	FROM ".$db->prefix()."accounting_account as accnt";
	$sql1 .= "	INNER JOIN ".$db->prefix()."accounting_system as syst";
	$sql1 .= "	ON accnt.fk_pcg_version = syst.pcg_version AND syst.rowid = ".getDolGlobalInt('CHARTOFACCOUNTS')." AND accnt.entity = ".((int) $config->entity).")";
	$sql1 .= " AND fd.fk_invoice_fourn IN (SELECT rowid FROM ".$db->prefix()."invoice_fourn WHERE entity = ".((int) $config->entity).")";
...
if ($action == 'validatehistory' && $user->hasRight('accounting', 'bind', 'write')) {
	...
	$sql .= " FROM ".$db->prefix()."invoice_fourn as f";
	...
	$sql .= " INNER JOIN ".$db->prefix()."invoice_fourn_det as l ON f.rowid = l.fk_invoice_fourn";
	...
	$sql .= " LEFT JOIN ".$db->prefix()."accounting_account as aa  ON " . $alias_product_perentity . ".accountancy_code_buy = aa.account_number         AND aa.active = 1  AND aa.fk_pcg_version = '".$db->escape($chartaccountcode)."' AND aa.entity = ".$config->entity;
	$sql .= " LEFT JOIN ".$db->prefix()."accounting_account as aa2 ON " . $alias_product_perentity . ".accountancy_code_buy_intra = aa2.account_number  AND aa2.active = 1 AND aa2.fk_pcg_version = '".$db->escape($chartaccountcode)."' AND aa2.entity = ".$config->entity;
	$sql .= " LEFT JOIN ".$db->prefix()."accounting_account as aa3 ON " . $alias_product_perentity . ".accountancy_code_buy_export = aa3.account_number AND aa3.active = 1 AND aa3.fk_pcg_version = '".$db->escape($chartaccountcode)."' AND aa3.entity = ".$config->entity;
	...
	if (!$result) {
		...
		$invoice_static = new InvoiceSupplier($db);
		$invoice_static_det = new SupplierInvoiceLine($db);
		...
		while ($i < min($num_lines, 10000)) {	// No more than 10000 at once
			...
			$invoice_static->ref_supplier = $objp->ref_supplier;
			$invoice_static->label = $objp->invoice_label;
			...
			$return = $accountingAccount->getAccountingCodeToBind($mysoc, $thirdpartystatic, $product_static, $invoice_static, $invoice_static_det, $accountingAccountArray, 'supplier');
			...
			if ($suggestedid > 0) {
				$sqlupdate = "UPDATE ".$db->prefix()."invoice_fourn_det";
...
$buttonbind = '<a class="button small" href="'.$_SERVER['PHP_SELF'].'?action=validatehistory&token='.newToken().'&year='.$year_current.'">'.img_picture($langs->trans("ValidateHistory"), 'link', 'class="picturefixedwidth fa-color-unset"').$langs->trans("ValidateHistory").'</a>';


print_barre_liste(img_picture('', 'unlink', 'class="paddingright fa-color-unset"').$langs->trans("OverviewOfAmountOfLinesNotBound"), 0, '', '', '', '', '', -1, '', '', 0, '', '', 0, 1, 1, 0, $buttonbind);
...
$sql .= " FROM ".$db->prefix()."invoice_fourn_det as ffd";
$sql .= "  LEFT JOIN ".$db->prefix()."invoice_fourn as ff ON ff.rowid = ffd.fk_invoice_fourn";
...
$sql .= " AND ff.entity IN (".getEntity('invoice_fourn', 0).")"; // We do not share object for accountancy
$sql .= " AND aa.account_number IS NULL";
if (getDolGlobalString('FACTURE_SUPPLIER_DEPOSITS_ARE_JUST_PAYMENTS')) {
	$sql .= " AND ff.type IN (".InvoiceSupplier::TYPE_STANDARD.",".InvoiceSupplier::TYPE_REPLACEMENT.",".InvoiceSupplier::TYPE_CREDIT_NOTE.")";
} else {
	$sql .= " AND ff.type IN (".InvoiceSupplier::TYPE_STANDARD.",".InvoiceSupplier::TYPE_REPLACEMENT.",".InvoiceSupplier::TYPE_CREDIT_NOTE.",".InvoiceSupplier::TYPE_DEPOSIT.")";
}
	...
	while ($row = $db->fetch_row($resql)) {
		...
		for ($i = 2; $i <= 13; $i++) {
			...
			if (!empty(price2num($row[2*$i - 2])) || !empty($row[2*$i - 1])) {
				...
				print img_picture($langs->trans("ValidateHistory").' ('.$langs->trans('Month'.str_pad((string) $cursormonth, 2, '0', STR_PAD_LEFT)).' '.$cursoryear.')', 'link', 'class="marginleft2"');
				print '</a>';
			}
			print '</td>';
		}
...
print_barre_liste(img_picture('', 'link', 'class="paddingright fa-color-unset"').$langs->trans("OverviewOfAmountOfLinesBound"), 0, '', '', '', '', '', -1, '', '', 0, '', '', 0, 1, 1);
