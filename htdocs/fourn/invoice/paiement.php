...
/**
 *	\file       htdocs/fourn/invoice/paiement.php
...
if ($cancel) {
	...
	header("Location: ".DOL_URL_ROOT.'/fourn/invoice/list.php');
	exit;
}
...
if (empty($resHook)) {
	if (($action == 'add_paiement' || ($action == 'confirm_paiement' && $confirm == 'yes')) && $permissionToAdd) {
		...
		$tmpinvoice = new InvoiceSupplier($db);
		$i = 0;
	...
	if ($action == 'confirm_paiement' && $confirm == 'yes') {
		...
		foreach ($amounts as $key => $value) {	// How payment is dispatched
			...
			if ($tmpinvoice->type == InvoiceSupplier::TYPE_CREDIT_NOTE) {
		...
		if (!$error) {
			...
			if (!$error) {
				...
				if ($invoiceid > 0) {
					$loc = DOL_URL_ROOT.'/fourn/invoice/card.php?facid='.$invoiceid;
				} else {
...
llxHeader('', $langs->trans('ListPayment'), '', '', 0, 0, '', '', '', 'mod-fourn-invoice page-paiement');
...
if ($action == 'create' || $action == 'confirm_paiement' || $action == 'add_paiement') {
	...
	$dateinvoice = ($dateinvoice == '' ? (!getDolGlobalString('MAIN_AUTOFILL_DATE') ? -1 : '') : $dateinvoice);
	...
	$sql .= ' FROM '.MAIN_DB_PREFIX.'societe as s, '.MAIN_DB_PREFIX.'invoice_fourn as f';
	if (!$user->hasRight("societe", "client", "voir") && !$socid) {
				...
				if ($resql) {
					...
					if ($num > 0) {
						...
						while ($i < $num) {
							...
							$sign = 1;
							if ($objp->type == InvoiceSupplier::TYPE_CREDIT_NOTE) {
								$sign = -1;
							}
							...
							if (isModEnabled("multicurrency")) {
								...
								if ($objp->multicurrency_code && $objp->multicurrency_code != $config->currency) {
									if ($action != 'add_paiement') {
										if (!empty($config->use_javascript_ajax)) {
											print img_picture("Auto fill", 'rightarrow', "class='AutoFillAmount' data-rowname='".$namef."' data-value='".($sign * (float) $multicurrency_remaintopay)."'");
										}
							...
							if (isModEnabled('paymentbybanktransfer')) {
								...
								$sql .= " WHERE fk_invoice_fourn = ".((int) $objp->facid);
							...
							if ($action != 'add_paiement') {
								if (!empty($config->use_javascript_ajax)) {
									print img_picture("Auto fill", 'rightarrow', "class='AutoFillAmount' data-rowname='".$namef."' data-value='".($sign * (float) $remaintopay)."'");
								}
