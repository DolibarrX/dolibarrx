...
/**
 *	\file       htdocs/fourn/invoice/rapport.php
 *	\ingroup    fourn
...
$result = restrictedArea($user, 'fournisseur', 0, 'invoice_fourn', 'invoice');
...
$dir = $config->fournisseur->invoice->dir_output.'/payments';
...
llxHeader('', $titre, '', '', 0, 0, '', '', '', 'mod-fourn-invoice page-rapport');
		...
		if (is_resource($handle)) {
			while (($file = readdir($handle)) !== false) {
				if (preg_match('/^supplier_payment/i', $file)) {
					...
					print '<tr class="oddeven"><td><a data-ajax="false" href="'.DOL_URL_ROOT.'/document.php?modulepart=invoice_fournisseur&amp;file=payments/'.urlencode($relativepath).'">'.img_pdf().' '.$file.'</a>'.$formfile->showPreview($file, 'invoice_fournisseur', 'payments/'.$relativepath, 0).'</td>';
