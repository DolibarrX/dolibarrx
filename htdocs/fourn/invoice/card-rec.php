...
/**
 *    \file       htdocs/fourn/invoice/card-rec.php
...
$permissionnote = $user->hasRight("fournisseur", "invoice", "creer") || $user->hasRight("supplier_invoice", "creer"); // Used by the include of actions_setnotes.inc.php
$permissiondellink = $user->hasRight("fournisseur", "invoice", "creer") || $user->hasRight("supplier_invoice", "creer"); // Used by the include of actions_dellink.inc.php
$permissiontoedit = $user->hasRight("fournisseur", "invoice", "creer") || $user->hasRight("supplier_invoice", "creer"); // Used by the include of actions_lineupdonw.inc.php
...
if (empty($resHook)) {
	...
	if ($action == 'add' && $permissionToAdd) {
		...
		if (! $error) {
			...
			$oldinvoice = new InvoiceSupplier($db);
	...
	if ($action == 'confirm_deleteinvoice' && $confirm == 'yes' && $permissiontodelete) {
		...
		header('Location: ' . DOL_URL_ROOT . '/fourn/invoice/list-rec.php');
		exit;
	}
	...
	if ($action == 'confirm_deleteline' && $confirm == 'yes' && $permissionToAdd) {
		...
		$line = new InvoiceSupplierLigneRec($db);
	...
	if ($action == 'addline' && $permissionToAdd) {
		...
		$objectline = new InvoiceSupplierLigneRec($db);
...
$help_url = '';
llxHeader('', $langs->trans("RepeatableSupplierInvoice"), $help_url, '', 0, 0, '', '', '', 'mod-fourn-invoice page-card-rec');
...
$invoicerectmp = new InvoiceSupplierRec($db);
...
if ($action == 'create') {
	...
	$object = new InvoiceSupplier($db); // Source invoice
	...
	if ($object->id > 0) {
		...
		$linkback = '<a href="' . DOL_URL_ROOT . '/fourn/invoice/list-rec.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans('BackToList') . '</a>';
		...
		if (getDolGlobalInt('INVOICE_SUBTYPE_ENABLED')) {
			print "<tr><td>".$langs->trans("InvoiceSubtype")."</td><td>";
			print $object->getSubtypeLabel('invoice_fourn_rec');
			print "</td></tr>";
		}
		...
		print '<td>';
		if ($action != 'editbankaccount' && $usercancreate && $object->statut == InvoiceSupplierRec::STATUS_NOTSUSPENDED) {
		...
		print '<td>';
		if ($action != 'editmodelpdf' && $usercancreate && $object->statut == InvoiceSupplierRec::STATUS_NOTSUSPENDED) {
			...
			if ($usercancreate) {
				if (!empty($object->frequency) && $object->nb_gen_max > 0 && ($object->nb_gen_done >= $object->nb_gen_max)) {
					...
					if (empty($object->frequency) || $object->date_when <= $nowlasthour) {
						print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/fourn/invoice/card.php?action=create&socid=' . $object->thirdparty->id . '&fac_rec=' . $object->id . '">' . $langs->trans("CreateBill") . '</a></div>';
					} else {
		...
		// Delete
		print dolGetButtonAction($langs->trans("Delete"), '', 'delete', $_SERVER["PHP_SELF"].'?id='.$object->id.'&action=ask_deleteinvoice&token='.newToken(), 'delete', ($user->hasRight("fournisseur", "invoice", "supprimer") || $user->hasRight("supplier_invoice", "supprimer")));
