...
/**
 *      \file       htdocs/fourn/invoice/contact.php
 *      \ingroup    invoice, fournisseur
 *      \brief      Onglet de gestion des contacts des invoices
 */
...
if (empty($resHook)) {
	if ($action == 'addcontact' && ($user->hasRight("fournisseur", "invoice", "creer") || $user->hasRight("supplier_invoice", "creer"))) {
	...
	} elseif ($action == 'swapstatut' && ($user->hasRight("fournisseur", "invoice", "creer") || $user->hasRight("supplier_invoice", "creer"))) {
	...
	} elseif ($action == 'deletecontact' && ($user->hasRight("fournisseur", "invoice", "creer") || $user->hasRight("supplier_invoice", "creer"))) {
		// Efface un contact
		$object->fetch($id);
...
if ($id > 0 || !empty($ref)) {
	if ($object->fetch($id, $ref) > 0) {
		...
		llxHeader('', $title, $helpurl, '', 0, 0, '', '', '', 'mod-fourn-invoice page-card_contact');
