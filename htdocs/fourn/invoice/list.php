...
/**
 *       \file       htdocs/fourn/invoice/list.php
...
$diroutputmassaction = $config->fournisseur->invoice->dir_output.'/temp/massgeneration/'.$user->id;
...
$permissiontoread = ($user->hasRight("fournisseur", "invoice", "lire") || $user->hasRight("supplier_invoice", "lire"));
$permissionToAdd = ($user->hasRight("fournisseur", "invoice", "creer") || $user->hasRight("supplier_invoice", "creer"));
$permissiontodelete = ($user->hasRight("fournisseur", "invoice", "supprimer") || $user->hasRight("supplier_invoice", "supprimer"));
...
if (empty($resHook)) {
	...
	// Mass actions
	$objectclass = 'InvoiceSupplier';
	$objectlabel = 'SupplierInvoices';

	$uploaddir = $config->fournisseur->invoice->dir_output;
		...
		if (!$user->hasRight("paymentbybanktransfer", "create")) {
			...
			foreach ($arrayofselected as $toselectid) {
				...
				if ($result > 0) {
					...
					if ($objecttmp->status == InvoiceSupplier::STATUS_DRAFT) {
						$error++;
					...
					$rsql .= " WHERE fk_invoice_fourn = ".((int) $objecttmp->id);
...
$help_url = 'EN:Suppliers_Invoices|FR:InvoiceSupplier|ES:Facturas_de_proveedores';
...
if (!$search_all) {
	$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'paiementfourn_invoicefourn as pf ON pf.fk_invoicefourn = f.rowid';
}
if ($search_all) {
	$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'invoice_fourn_det as pd ON f.rowid=pd.fk_invoice_fourn';
}
...
if ($option == 'late') {
	$sql .= " AND f.date_lim_reglement < '".$db->idate(dol_now() - $config->invoice->fournisseur->warning_delay)."'";
}
if ($search_label) {
...
if (!empty($searchCategoryProductList)) {
	...
	foreach ($searchCategoryProductList as $searchCategoryProduct) {
		if (intval($searchCategoryProduct) == -2) {
			$searchCategoryProductSqlList[] = "NOT EXISTS (SELECT ck.fk_product FROM ".MAIN_DB_PREFIX."category_product as ck, ".MAIN_DB_PREFIX."invoice_fourn_det as fd WHERE fd.fk_invoice_fourn = f.rowid AND fd.fk_product = ck.fk_product)";
		} elseif (intval($searchCategoryProduct) > 0) {
			if ($searchCategoryProductOperator == 0) {
				$searchCategoryProductSqlList[] = " EXISTS (SELECT ck.fk_product FROM ".MAIN_DB_PREFIX."category_product as ck, ".MAIN_DB_PREFIX."invoice_fourn_det as fd WHERE fd.fk_invoice_fourn = f.rowid AND fd.fk_product = ck.fk_product AND ck.fk_category = ".((int) $searchCategoryProduct).")";
			} else {
	...
	if ($listofcategoryid) {
		$searchCategoryProductSqlList[] = " EXISTS (SELECT ck.fk_product FROM ".MAIN_DB_PREFIX."category_product as ck, ".MAIN_DB_PREFIX."invoice_fourn_det as fd WHERE fd.fk_invoice_fourn = f.rowid AND fd.fk_product = ck.fk_product AND ck.fk_category IN (".$db->sanitize($listofcategoryid)."))";
	}
...
if (!getDolGlobalInt('MAIN_DISABLE_FULL_SCANLIST')) {
	...
	$sqlforcount = preg_replace('/LEFT JOIN '.MAIN_DB_PREFIX.'paiementfourn_invoicefourn as pf ON pf.fk_invoicefourn = f.rowid/', '', $sqlforcount);
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->facid;
	header("Location: ".DOL_URL_ROOT.'/fourn/invoice/card.php?facid='.$id);
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'bodyforlist mod-fourn-invoice page-list');

if ($search_fk_fac_rec_source) {
	require_once DOL_DOCUMENT_ROOT . '/fourn/class/fournisseur.invoice-rec.class.php';
	require_once DOL_DOCUMENT_ROOT . '/core/lib/invoice.lib.php';
	$object = new InvoiceSupplierRec($db);
...
if (isModEnabled('paymentbybanktransfer') && $user->hasRight("paymentbybanktransfer", "create")) {
	$langs->load('withdrawals');
	$arrayofmassactions['banktransfertrequest'] = img_picture('', 'payment', 'class="picturefixedwidth"').$langs->trans("MakeBankTransferOrder");
}
...
$url = DOL_URL_ROOT.'/fourn/invoice/card.php?action=create';
...
$newcardbutton .= dolGetButtonTitle($langs->trans('NewBill'), '', 'fa fa-plus-circle', $url, '', (int) ($user->hasRight("fournisseur", "invoice", "creer") || $user->hasRight("supplier_invoice", "creer")));
...
if (!empty($arrayfields['f.type']['checked'])) {
	...
	$typearray = array(
		InvoiceSupplier::TYPE_STANDARD => $langs->trans("InvoiceStandard"),
		InvoiceSupplier::TYPE_REPLACEMENT => $langs->trans("InvoiceReplacement"),
		InvoiceSupplier::TYPE_CREDIT_NOTE => $langs->trans("InvoiceAvoir"),
		InvoiceSupplier::TYPE_DEPOSIT => $langs->trans("InvoiceDeposit"),
	);
...
while ($i < $imaxinloop) {
	...
	$totalpay = $paiement + $totalcreditnotes + $totaldeposits;
	$remaintopay = price2num($invoicestatic->total_ttc - $totalpay);

	$multicurrency_totalpay = $multicurrency_paiement + $multicurrency_totalcreditnotes + $multicurrency_totaldeposits;
	$multicurrency_remaintopay = price2num($invoicestatic->multicurrency_total_ttc - $multicurrency_totalpay);

	if ($invoicestatic->status == InvoiceSupplier::STATUS_CLOSED && $invoicestatic->close_code == 'discount_vat') {		// If invoice closed with discount for anticipated payment
		$remaintopay = 0;
		$multicurrency_remaintopay = 0;
	}
	if ($invoicestatic->type == InvoiceSupplier::TYPE_CREDIT_NOTE && $obj->paye == 1) {		// If credit note closed, we take into account the amount not yet consumed
		$remaincreditnote = $discount->getAvailableDiscounts($thirdparty, '', 'rc.fk_invoice_source='.$invoicestatic->id);
		$remaintopay = -$remaincreditnote;
		$totalpay = price2num($invoicestatic->total_ttc - $remaintopay);
		$multicurrency_remaincreditnote = $discount->getAvailableDiscounts($thirdparty, '', 'rc.fk_invoice_source='.$invoicestatic->id, 0, 0, 1);
	...
	$invoicestatic->alreadypaid = ($paiement ? $paiement : 0);
	...
	if ($invoicestatic->type == InvoiceSupplier::TYPE_CREDIT_NOTE) {
		if ($invoicestatic->isCreditNoteUsed()) {
			$remaintopay = -$invoicestatic->getSumFromThisCreditNotesNotUsed();
		}
	}
	...
	if ($mode == 'kanban') {
		...
		if (!empty($arrayfields['f.ref']['checked'])) {
			...
			print $invoicestatic->getNomUrl(1, '', 0, 0, '', 0, -1, 1);

			$filename = dol_sanitizeFileName($obj->ref);
			$filedir = $config->fournisseur->invoice->dir_output.'/'.get_exdir($obj->facid, 2, 0, 0, $invoicestatic, 'invoice_supplier').dol_sanitizeFileName($obj->ref);
			$subdir = get_exdir($obj->facid, 2, 0, 0, $invoicestatic, 'invoice_supplier').dol_sanitizeFileName($obj->ref);
			print $formfile->getDocumentsLink('invoice_fournisseur', $subdir, $filedir);
			print '</td></tr></table>';
		...
		if (!empty($arrayfields['f.type']['checked'])) {
			...
			print $invoicestatic->getLibType();
			print "</td>";
			if (!$i) {
				$totalarray['nbfield']++;
			}
		...
		if (!empty($arrayfields['f.subtype']['checked'])) {
			...
			if ($invoicestatic->subtype > 0) {
				$labeltoshow = $invoicestatic->getSubtypeLabel('invoice_fourn');
			}
		...
		if (!empty($arrayfields['f.nb_docs']['checked'])) {
			...
			$upload_dir = $config->fournisseur->invoice->dir_output.'/'.get_exdir($invoicestatic->id, 2, 0, 0, $invoicestatic, 'invoice_supplier').$invoicestatic->ref;
			$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
			$nbLinks = Link::count($db, $invoicestatic->element, $invoicestatic->id);
		...
		if (!empty($arrayfields['f.fk_statut']['checked'])) {
			print '<td class="center nowrap">';
			print $invoicestatic->getLibStatut(5, $paiement);
			print "</td>";
			if (!$i) {
				$totalarray['nbfield']++;
			}
