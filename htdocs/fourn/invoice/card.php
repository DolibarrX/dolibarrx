...
/**
 *	\file       htdocs/fourn/invoice/card.php
...
$isdraft = (($object->status == InvoiceSupplier::STATUS_DRAFT) ? 1 : 0);
$result = restrictedArea($user, 'fournisseur', $id, 'invoice_fourn', 'invoice', 'fk_soc', 'rowid', $isdraft);

// Common permissions
$usercanread = ($user->hasRight("fournisseur", "invoice", "lire") || $user->hasRight("supplier_invoice", "lire"));
$usercancreate = ($user->hasRight("fournisseur", "invoice", "creer") || $user->hasRight("supplier_invoice", "creer"));
$usercandelete = ($user->hasRight("fournisseur", "invoice", "supprimer") || $user->hasRight("supplier_invoice", "supprimer"));
...
if (empty($resHook)) {
	$backurlforlist = DOL_URL_ROOT.'/fourn/invoice/list.php';
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = DOL_URL_ROOT.'/fourn/invoice/card.php?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $permissionToAdd) {
		...
		'@phan-var-force InvoiceSupplier $objectutil';  // Same object type for cloned object
	...
	if ($action == 'settransportmode' && $usercancreate) {
		...
		$canconvert = 0;
		if ($object->type == InvoiceSupplier::TYPE_DEPOSIT && empty($discountcheck->id)) {
		...
		if (($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE || $object->type == InvoiceSupplier::TYPE_STANDARD) && $object->paid == 0 && empty($discountcheck->id)) {
		...
		if ($canconvert) {
			...
			if (getDolGlobalString('SUPPLIER_INVOICE_ALLOW_REUSE_OF_CREDIT_WHEN_PARTIALLY_REFUNDED') && $object->type == InvoiceSupplier::TYPE_CREDIT_NOTE) {
			...
			$discount = new DiscountAbsolute($db);
			if ($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE) {
				$discount->description = '(CREDIT_NOTE)';
			} elseif ($object->type == InvoiceSupplier::TYPE_DEPOSIT) {
				$discount->description = '(DEPOSIT)';
			} elseif ($object->type == InvoiceSupplier::TYPE_STANDARD || $object->type == InvoiceSupplier::TYPE_REPLACEMENT || $object->type == InvoiceSupplier::TYPE_SITUATION) {
			...
			if ($object->type == InvoiceSupplier::TYPE_STANDARD || $object->type == InvoiceSupplier::TYPE_REPLACEMENT || $object->type == InvoiceSupplier::TYPE_SITUATION) {
				...
				$sql .= ' WHERE pf.fk_invoicefourn = '.((int) $object->id);
			...
			if ($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE || $object->type == InvoiceSupplier::TYPE_DEPOSIT) {
			...
			if (empty($error)) {
				if ($object->type != InvoiceSupplier::TYPE_DEPOSIT) {
					// Set invoice as paid
					$result = $object->setPaid($user);
					if ($result >= 0) {
						$db->commit();
		...
		// Delete payment
		$object->fetch($id);
		if ($object->status == InvoiceSupplier::STATUS_VALIDATED && $object->paid == 0) {
		...
		if (GETPOST('type') == InvoiceSupplier::TYPE_REPLACEMENT) {
			...
			if (!$error) {
				...
				// Proprietes particulieres a invoice de replacement
				$object->fk_invoice_source = GETPOSTINT('fac_replacement');
				$object->type = InvoiceSupplier::TYPE_REPLACEMENT;
		...
		// Credit note invoice
		if (GETPOST('type') == InvoiceSupplier::TYPE_CREDIT_NOTE) {
			...
			if (!$error) {
				...
				// Proprietes particulieres a invoice avoir
				$object->fk_invoice_source = $sourceinvoice > 0 ? $sourceinvoice : '';
				$object->type = InvoiceSupplier::TYPE_CREDIT_NOTE;
				...
				if (GETPOSTINT('invoiceAvoirWithLines') == 1 && $id > 0) {
					$invoice_source = new InvoiceSupplier($db); // fetch origin object
					if ($invoice_source->fetch($object->fk_invoice_source) > 0) {
						...
						foreach ($invoice_source->lines as $line) {
							...
							$line->fk_invoice_fourn = $object->id;
				...
				if (GETPOSTINT('invoiceAvoirWithPaymentRestAmount') == 1 && $id > 0) {
					$invoice_source = new InvoiceSupplier($db); // fetch origin object if not previously defined
					if ($invoice_source->fetch($object->fk_invoice_source) > 0) {
		...
		} elseif ($fac_recid > 0 && (GETPOST('type') == InvoiceSupplier::TYPE_STANDARD || GETPOST('type') == InvoiceSupplier::TYPE_DEPOSIT)) {
			...
			if (!$error) {
				...
				$object->mode_reglement_id	= GETPOSTINT('mode_reglement_id');
				$object->fk_account         = GETPOSTINT('fk_account');
				$object->amount             = (float) price2num(GETPOST('amount'));  // FIXME: InvoiceSupplier::$amount is deprecated and not used?
				...
				$fac_rec = new InvoiceSupplierRec($db);
			...
			if (!$error) {
				...
				if (!$error && GETPOST('origin', 'alpha') && GETPOST('originid')) {
					...
					if ($id > 0) {
						...
						if (GETPOST('type') == InvoiceSupplier::TYPE_DEPOSIT && in_array($typeamount, array('amount', 'variable'))) {
		...
		if (!$ventilExportCompta) {
			...
			if ($resteapayer == price2num($object->total_ttc, 'MT', 1) && $object->status == InvoiceSupplier::STATUS_VALIDATED) {
				$idwarehouse = GETPOST('idwarehouse');
		...
		if ($object->status == InvoiceSupplier::STATUS_CLOSED
		|| ($object->status == InvoiceSupplier::STATUS_ABANDONED && $object->close_code != 'replaced')) {
...
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-fourn-invoice page-card');
...
if ($action == 'create') {
	...
	if (!empty($origin) && !empty($originid)) {
		...
		$objectsrc = new $classname($db);
		'@phan-var-force Project|Order|Propal|Invoice|Contract|OrderFournisseur|CommonObject $objectsrc';
		$objectsrc->fetch($originid);
		$objectsrc->fetch_thirdparty();
	...
	if (empty($resHook)) {
		...
		$exampletemplateinvoice = new InvoiceSupplierRec($db);
		$invoice_predefined = new InvoiceSupplierRec($db);
		...
		if (empty($origin) && empty($originid) && $fac_recid > 0) {
			...
			$sql .= ' FROM '.MAIN_DB_PREFIX.'invoice_fourn_rec as r';
		...
		$desc = $form->textWithPicture($tmp.'<label for="radio_standard">'.$langs->trans("InvoiceStandardAsk").'</label>', $langs->transnoentities("InvoiceStandardDesc"), 1, 'help', '', 0, 3);
		print $desc;
		print '</div></div>';
		...
		if (empty($origin) || ($origin == 'order_supplier' && !empty($originid))) {
			...
			if (!getDolGlobalString('INVOICE_DISABLE_DEPOSIT')) {
				...
				$desc = $form->textWithPicture($tmp, $langs->transnoentities("InvoiceDepositDesc"), 1, 'help', '', 0, 3);
		...
		{
			// Replacement
			if (empty($config->global->INVOICE_DISABLE_REPLACEMENT))
			{
				// Type invoice
				$facids = $invoicestatic->list_replacable_supplier_invoices($societe->id);
				if ($facids < 0) {
				...
				foreach ($facids as $facparam)
				{
					...
					$options .= ' (' . $invoicestatic->LibStatut(0, $facparam ['status']) . ')';
					$options .= '</option>';
				}
		...
		*/
		...
		if (empty($origin)) {
			if (!empty($societe->id) && $societe->id > 0) {
				...
				if (!getDolGlobalString('INVOICE_DISABLE_CREDIT_NOTE')) {
					...
					$facids = $invoicestatic->list_qualified_avoir_supplier_invoices($societe->id);
					if ($facids < 0) {
						dol_print_error($db, $invoicestatic->error, $invoicestatic->errors);
						exit;
					}
					$optionsav = "";
					$newinvoice_static = new InvoiceSupplier($db);
		...
		if (isModEnabled('project')) {
			...
			print img_picture('', 'project', 'class="picturefixedwidth"').$formproject->select_projects((!getDolGlobalString('PROJECT_CAN_ALWAYS_LINK_TO_ALL_SUPPLIERS') ? $societe->id : -1), $projectid, 'projectid', 0, 0, 1, 1, 0, 0, 0, '', 1, 0, 'maxwidth500 widthcentpercentminusxx');
		...
		if (!empty($objectsrc) && is_object($objectsrc)) {
			...
			'@phan-var-force null|InvoiceSupplier[] $invoice_supplier';
	...
	if ($id > 0 || !empty($ref)) {
		...
		// $resteapayer=bcadd($object->total_ttc,$totalpaid,$config->global->MAIN_MAX_DECIMALS_TOT);
		...
		$head = invoicefourn_prepare_head($object);
		...
		if ($action == 'converttoreduc') {
			$type_fac = '';
			if ($object->type == InvoiceSupplier::TYPE_STANDARD) {
				$type_fac = 'ExcessPaid';
			} elseif ($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE) {
				$type_fac = 'CreditNote';
			} elseif ($object->type == InvoiceSupplier::TYPE_DEPOSIT) {
				$type_fac = 'Deposit';
			}
			...
			if ($numref < 0) {
				...
				if (isModEnabled('stock') && getDolGlobalString('STOCK_CALCULATE_ON_SUPPLIER_BILL') && $qualified_for_stock_change) {
					...
					if (count($warehouse_array) == 1) {
						$label = $object->type == InvoiceSupplier::TYPE_CREDIT_NOTE ? $langs->trans("WarehouseForStockDecrease", current($warehouse_array)) : $langs->trans("WarehouseForStockIncrease", current($warehouse_array));
						$value = '<input type="hidden" id="idwarehouse" name="idwarehouse" value="'.key($warehouse_array).'">';
					} else {
						$label = $object->type == InvoiceSupplier::TYPE_CREDIT_NOTE ? $langs->trans("SelectWarehouseForStockDecrease") : $langs->trans("SelectWarehouseForStockIncrease");
		...
		if ($action == 'edit') {
			...
			if (isModEnabled('stock') && getDolGlobalString('STOCK_CALCULATE_ON_SUPPLIER_BILL') && $qualified_for_stock_change) {
				...
				$warehouse_array = $warehouse->list_[];
				if (count($warehouse_array) == 1) {
					$label = $object->type == InvoiceSupplier::TYPE_CREDIT_NOTE ? $langs->trans("WarehouseForStockIncrease", current($warehouse_array)) : $langs->trans("WarehouseForStockDecrease", current($warehouse_array));
					$value = '<input type="hidden" id="idwarehouse" name="idwarehouse" value="'.key($warehouse_array).'">';
				} else {
					$label = $object->type == InvoiceSupplier::TYPE_CREDIT_NOTE ? $langs->trans("SelectWarehouseForStockIncrease") : $langs->trans("SelectWarehouseForStockDecrease");
					$value = $formproduct->selectWarehouses(GETPOST('idwarehouse') ? GETPOST('idwarehouse') : 'ifone', 'idwarehouse', '', 1);
				}
				$formquestion = array(
					array('type' => 'other', 'name' => 'idwarehouse', 'label' => $label, 'value' => $value)
		...
		if ($action == 'canceled') {
			...
			// Text
			$close[1]['reason'] = $form->textWithPicture($langs->transnoentities("ConfirmClassifyPaidPartiallyReasonBadSupplier", $object->ref), $close[1]['label'], 1);
		...
		// Confirmation de la suppression de la invoice fournisseur
		if ($action == 'delete') {
			$formquestion = [];
		...
		$linkback = '<a href="'.DOL_URL_ROOT.'/fourn/invoice/list.php?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
		...
		if (!getDolGlobalString('MAIN_DISABLE_OTHER_LINK') && $object->thirdparty->id > 0) {
			$morehtmlref .= ' <div class="inline-block valignmiddle">(<a class="valignmiddle" href="'.DOL_URL_ROOT.'/fourn/invoice/list.php?socid='.((int) $object->thirdparty->id).'&search_company='.urlencode($object->thirdparty->name).'">'.$langs->trans("OtherBills").'</a>)</div>';
		}
		// Project
		if (isModEnabled('project')) {
			$langs->load("projects");
		...
		if (empty($resHook)) {
			...
			print '</span>';
			if ($object->subtype > 0) {
				print ' '.$object->getSubtypeLabel('invoice_fourn');
			}
			if ($object->type == InvoiceSupplier::TYPE_REPLACEMENT) {
				$facreplaced = new InvoiceSupplier($db);
			...
			if ($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE) {
				if ($object->fk_invoice_source > 0) {
					$facusing = new InvoiceSupplier($db);
			...
			if (!empty($object->creditnote_ids)) {
				...
				foreach ($object->creditnote_ids as $invoiceid) {
					$creditnote = new InvoiceSupplier($db);
			...
			if (isset($objectidnext) && $objectidnext > 0) {
				$facthatreplace = new InvoiceSupplier($db);
			...
			if ($object->fk_fac_rec_source > 0) {
				$tmptemplate = new InvoiceSupplierRec($db);
				$result = $tmptemplate->fetch($object->fk_fac_rec_source);
				if ($result > 0) {
					print ' <span class="opacitymediumbycolor paddingleft">';
					$link = '<a href="'.DOL_URL_ROOT.'/fourn/invoice/card-rec.php?facid='.$tmptemplate->id.'">'.dol_escape_htmltag($tmptemplate->title).'</a>';
			...
			//$form_permission = ($object->status < InvoiceSupplier::STATUS_CLOSED) && $usercancreate && ($object->getSommePaiement() <= 0);
			$form_permission = ($object->status < InvoiceSupplier::STATUS_CLOSED) && $usercancreate;
			...
			if (isModEnabled('incoterm')) {
				...
				if ($usercancreate) {
					print '<a class="editfielda" href="'.DOL_URL_ROOT.'/fourn/invoice/card.php?facid='.$object->id.'&action=editincoterm&token='.newToken().'">'.img_edit().'</a>';
				} else {
					print '&nbsp;';
				}
				print '</td></tr></table>';
			...
			if (isModEnabled('intracommreport')) {
				...
				print '</td>';
				if ($action != 'edittransportmode' && ($user->hasRight("fournisseur", "invoice", "creer") || $user->hasRight("supplier_invoice", "creer"))) {
			...
			if ($object->getVentilExportCompta() == 0) {
				...
				print $form->textwithtooltip($s, $langs->trans("CalculationRuleDesc", $calculationrulenum) . '<br>' . $langs->trans("CalculationRuleDescSupplier"), 2, 1, img_picture('', 'help'), '', 3, '', 0, 'recalculate');
				print '&nbsp; &nbsp; &nbsp; &nbsp;';
				print '</div>';
			}
			print price($object->total_tva, 1, $langs, 0, -1, -1, $config->currency);
			print '</td>';
			if (isModEnabled("multicurrency") && ($object->multicurrency_code && $object->multicurrency_code != $config->currency)) {
			...
			if ($societe->localtax1_assuj == "1") { //Localtax1
				...
				print '<td class="nowrap amountcard right">' . price($object->total_localtax1, 1, $langs, 0, -1, -1, $config->currency) . '</td>';
				print '</tr>';
			}
			if ($societe->localtax2_assuj == "1") { //Localtax2
				print '<tr>';
				print '<td>' . $langs->transcountry("AmountLT2", $societe->country_code) . '</td>';
				print '<td class="nowrap amountcard right">' . price($object->total_localtax2, 1, $langs, 0, -1, -1, $config->currency) . '</td>';
				print '</tr>';
			}
			...
			$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'paiementfourn_invoicefourn as pf ON pf.fk_paiementfourn = p.rowid';
			$sql .= ' WHERE pf.fk_invoicefourn = '.((int) $object->id);
			...
			if ($result) {
				...
				print '<td class="liste_titre">'.($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE ? $langs->trans("PaymentsBack") : $langs->trans('Payments')).'</td>';
				...
				if ($num > 0) {
					while ($i < $num) {
						...
						if ($object->status == InvoiceSupplier::STATUS_VALIDATED && $object->paid == 0 && $user->socid == 0) {
			...
			if ($object->type != InvoiceSupplier::TYPE_CREDIT_NOTE) {
				...
				if ($resql) {
					...
					while ($i < $num) {
						...
						print '<tr><td colspan="'.$nbcols.'" class="right">';
						if ($invoice->type == InvoiceSupplier::TYPE_CREDIT_NOTE) {
				...
				if (($object->status == InvoiceSupplier::STATUS_CLOSED || $object->status == InvoiceSupplier::STATUS_ABANDONED) && $object->close_code == 'discount_vat') {
				...
				if (($object->status == InvoiceSupplier::STATUS_CLOSED || $object->status == InvoiceSupplier::STATUS_ABANDONED) && $object->close_code == 'badsupplier') {
					print '<tr><td colspan="'.$nbcols.'" class="right nowrap">';
					print '<span class="opacitymedium">';
				...
				if (($object->status == InvoiceSupplier::STATUS_CLOSED || $object->status == InvoiceSupplier::STATUS_ABANDONED) && $object->close_code == 'product_returned') {
					print '<tr><td colspan="'.$nbcols.'" class="right nowrap">';
					print '<span class="opacitymedium">';
				...
				if (($object->status == InvoiceSupplier::STATUS_CLOSED || $object->status == InvoiceSupplier::STATUS_ABANDONED) && $object->close_code == 'abandon') {
					print '<tr><td colspan="'.$nbcols.'" class="right nowrap">';
				...
				if (isModEnabled('multicurrency') && $object->multicurrency_code != $config->currency || $object->multicurrency_tx != 1) {
					print '<tr><td colspan="'.$nbcols.'" class="right">';
					print '<span class="opacitymedium">';
				...
				if (isModEnabled('multicurrency') && $object->multicurrency_code != $config->currency || $object->multicurrency_tx != 1) {
					...
					print '</span>';
					print '</td>';
					print '<td class="right'.($resteapayeraffiche ? ' amountremaintopay' : (' '.$cssforamountpaymentcomplete)).'">'.(!empty($object->multicurrency_code) ? $object->multicurrency_code : $config->currency).' '.price(price2num($sign * $object->multicurrency_tx * $resteapayeraffiche, 'MT')).'</td><td>&nbsp;</td></tr>';
				}
			...
			if (!empty($config->use_javascript_ajax) && $object->status == InvoiceSupplier::STATUS_DRAFT) {
			...
			if ($object->status == InvoiceSupplier::STATUS_DRAFT && $usercancreate) {
				if ($action != 'editline') {
		...
		if ($action != 'presend') {
			...
			if (empty($resHook)) {
				...
				if ($object->status == InvoiceSupplier::STATUS_VALIDATED && $action != 'confirm_edit' && $object->getSommePaiement() == 0 && $usercancreate) {
					...
					if ($ventilExportCompta == 0) {
						print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&action=edit&token='.newToken().'">'.$langs->trans('Modify').'</a>';
					} else {
				...
				if (($object->type == InvoiceSupplier::TYPE_STANDARD || $object->type == InvoiceSupplier::TYPE_REPLACEMENT
					|| ($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE && empty($discount->id))
					|| ($object->type == InvoiceSupplier::TYPE_DEPOSIT && empty($discount->id)))
					&& ($object->status == InvoiceSupplier::STATUS_CLOSED || $object->status == InvoiceSupplier::STATUS_ABANDONED)) {				// A paid invoice (partially or completely)
					if (!$objectidnext && $object->close_code != 'replaced' && $usercancreate) {	// Not replaced by another invoice
						print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&action=reopen&token='.newToken().'">'.$langs->trans('ReOpen').'</a>';
					} else {
						if ($usercancreate) {
				...
				// Validate
				if ($action != 'confirm_edit' && $object->status == InvoiceSupplier::STATUS_DRAFT && count($object->lines) > 0
					&& ((($object->type == InvoiceSupplier::TYPE_STANDARD || $object->type == InvoiceSupplier::TYPE_REPLACEMENT || $object->type == InvoiceSupplier::TYPE_DEPOSIT || $object->type == InvoiceSupplier::TYPE_PROFORMA || $object->type == InvoiceSupplier::TYPE_SITUATION) && (getDolGlobalString('SUPPLIER_INVOICE_ENABLE_NEGATIVE') || $object->total_ttc >= 0))  // @phan-suppress-current-line PhanDeprecatedClassConstant
						|| ($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE && $object->total_ttc <= 0))) {
				...
				// Send by mail
				if (empty($user->socid)) {
					if (($object->status == InvoiceSupplier::STATUS_VALIDATED || $object->status == InvoiceSupplier::STATUS_CLOSED)) {
						if ($usercansend) {
				...
				// Create payment
				if ($object->type != InvoiceSupplier::TYPE_CREDIT_NOTE && $object->status == InvoiceSupplier::STATUS_VALIDATED && $object->paid == 0) {
					print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.DOL_URL_ROOT.'/fourn/invoice/paiement.php?facid='.$object->id.'&action=create'.($object->fk_account > 0 ? '&amp;accountid='.$object->fk_account : '').'">'.$langs->trans('DoPayment').'</a>'; // must use facid because id is for payment id not invoice
				}

				// Reverse back money or convert to reduction
				if ($object->status != InvoiceSupplier::STATUS_DRAFT && ($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE || $object->type == InvoiceSupplier::TYPE_DEPOSIT || $object->type == InvoiceSupplier::TYPE_STANDARD)) {
					// For credit note only
					if ($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE && $object->status == 1 && $object->paid == 0) {
						if ($resteapayer == 0) {
							...
							print '<a class="butAction" href="'.DOL_URL_ROOT.'/fourn/invoice/paiement.php?facid='.$object->id.'&action=create&accountid='.$object->fk_account.'">'.$langs->trans('DoPaymentBack').'</a>';
						}
					}

					// For standard invoice with excess paid
					if ($object->type == InvoiceSupplier::TYPE_STANDARD && empty($object->paid) && ($object->total_ttc - $totalpaid - $totalcreditnotes - $totaldeposits) < 0 && $usercancreate && empty($discount->id)) {
						print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER["PHP_SELF"].'?facid='.$object->id.'&action=converttoreduc&token='.newToken().'">'.$langs->trans('ConvertExcessPaidToReduc').'</a>';
					}
					// For credit note
					if ($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE && $object->status == 1 && $object->paid == 0 && $usercancreate
						&& (getDolGlobalString('SUPPLIER_INVOICE_ALLOW_REUSE_OF_CREDIT_WHEN_PARTIALLY_REFUNDED') || $object->getSommePaiement() == 0)
					) {
						print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER["PHP_SELF"].'?facid='.$object->id.'&action=converttoreduc&token='.newToken().'" title="'.dol_escape_htmltag($langs->trans("ConfirmConvertToReducSupplier2")).'">'.$langs->trans('ConvertToReduc').'</a>';
					}
					// For deposit invoice
					if ($object->type == InvoiceSupplier::TYPE_DEPOSIT && $usercancreate && $object->status > 0 && empty($discount->id)) {
				...
				// Classify paid
				if ($object->status == InvoiceSupplier::STATUS_VALIDATED && $object->paid == 0 && (
					($object->type != InvoiceSupplier::TYPE_CREDIT_NOTE && $object->type != InvoiceSupplier::TYPE_DEPOSIT && ($resteapayer <= 0 || (getDolGlobalString('SUPPLIER_INVOICE_CAN_SET_PAID_EVEN_IF_PARTIALLY_PAID') && $object->total_ttc == $resteapayer))) ||
						($object->type == InvoiceSupplier::TYPE_CREDIT_NOTE && $resteapayer >= 0) ||
						($object->type == InvoiceSupplier::TYPE_DEPOSIT && $object->total_ttc > 0 && ($resteapayer == 0 || (getDolGlobalString('SUPPLIER_INVOICE_CAN_SET_PAID_EVEN_IF_PARTIALLY_PAID') && $object->total_ttc == $resteapayer)))
				)
				) {
					print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=paid&token='.newToken().'">'.$langs->trans('ClassifyPaid').'</a>';
				}

				// Classify 'closed not completely paid' (possible if validated and not yet filed paid)
				if ($object->status == InvoiceSupplier::STATUS_VALIDATED && $object->paid == 0 && $resteapayer > 0 && (!getDolGlobalString('SUPPLIER_INVOICE_CAN_SET_PAID_EVEN_IF_PARTIALLY_PAID') || $object->total_ttc != $resteapayer)) {
					if ($totalpaid > 0 || $totalcreditnotes > 0) {
						// If one payment or one credit note was linked to this invoice
						print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&action=paid&token='.newToken().'">'.$langs->trans('ClassifyPaidPartially').'</a>';
					} else {
						if (!getDolGlobalString('INVOICE_CAN_NEVER_BE_CANCELED')) {
							print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&action=canceled">'.$langs->trans('ClassifyCanceled').'</a>';
						}
					}
				}
				...
				if (($object->type == InvoiceSupplier::TYPE_STANDARD || $object->type == InvoiceSupplier::TYPE_DEPOSIT) && $object->status > 0 && $usercancreate) {
					if (!$objectidnext) {
				...
				if (($object->type ==  InvoiceSupplier::TYPE_STANDARD || $object->type == InvoiceSupplier::TYPE_DEPOSIT) && $object->status == 0 && $usercancreate) {
					if (!$objectidnext && count($object->lines) > 0) {
						print '<a class="butAction" href="'.DOL_URL_ROOT.'/fourn/invoice/card-rec.php?facid='.$object->id.'&action=create">'.$langs->trans("ChangeIntoRepeatableInvoice").'</a>';
					}
				}
				...
				if ($action != 'confirm_edit') {
					...
					$filedir = $config->fournisseur->invoice->dir_output.'/'.$subdir;
					...
					print $formfile->showdocuments('invoice_fournisseur', $subdir, $filedir, $urlsource, $genallowed, $delallowed, $modelpdf, $genifempty, 0, 0, 40, 0, '', '', '', $societe->default_lang);
		...
		$diroutput = $config->fournisseur->invoice->dir_output;
		$autocopy = 'MAIN_MAIL_AUTOCOPY_SUPPLIER_INVOICE_TO';
		$trackid = 'sinv'.$object->id;
