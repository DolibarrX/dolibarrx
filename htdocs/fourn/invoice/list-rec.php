...
/**
 *	\file       htdocs/fourn/invoice/list-rec.php
 *	\ingroup    invoice
...
$supplierinvoicerectmp = new InvoiceSupplierRec($db);
$tmpuser = new User($db);
...
$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'invoice_fourn_rec_extrafields as ef ON ef.fk_object = f.rowid';
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'bodyforlist mod-fourn-invoice page-list-rec');
...
while ($i < $imaxinloop) {
	...
	if (getDolGlobalString('MAIN_CHECKBOX_LEFT_COLUMN')) {
		...
		if ($user->hasRight('invoice', 'creer') && empty($supplierinvoicerectmp->suspended)) {
			if ($supplierinvoicerectmp->isMaxNbGenReached()) {
				...
				print '<a href="'.DOL_URL_ROOT.'/fourn/invoice/card.php?action=create&amp;socid='.$objp->socid.'&amp;fac_rec='.$objp->facid.'">';
