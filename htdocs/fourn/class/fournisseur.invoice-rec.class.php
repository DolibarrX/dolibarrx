...
/**
 *	\file       htdocs/fourn/invoice/class/fournisseur.invoice-rec.class.php
 *	\ingroup    invoice
...
require_once DOL_DOCUMENT_ROOT.'/fourn/class/fournisseur.invoice-rec.ligne.class.php';
 ...
 */
class InvoiceSupplierRec extends CommonInvoice
{
	 ...
	 */
	public $table_element = 'invoice_fourn_rec';

	/**
	 * @var string    Name of subtable line
	 */
	public $table_element_line = 'invoice_fourn_det_rec';
	...
	*/
	...
	public function create($user, $facFournId, $notrigger = 0)
	{
		...
		// On charge la invoice fournisseur depuis laquelle on crée la invoice fournisseur modèle
		$facfourn_src = new InvoiceSupplier($this->db);
		$result = $facfourn_src->fetch($facFournId);
		if ($result > 0) {
			$sql = 'INSERT INTO '.MAIN_DB_PREFIX.'invoice_fourn_rec (';
			$sql .= 'titre';
			$sql .= ", subtype";
			$sql .= ', ref_supplier';
			$sql .= ', entity';
			...
			if ($this->db->query($sql)) {
				$this->id = $this->db->last_insert_id(MAIN_DB_PREFIX. 'invoice_fourn_rec');
	...
	public function update(User $user, $notrigger = 0)
	{
		...
		$sql = "UPDATE ".MAIN_DB_PREFIX."invoice_fourn_rec SET";
	...
	public function fetch($rowid, $ref = '', $ref_ext = '')
	{
		...
		$sql .= ', f.fk_cond_reglement, c.code as cond_reglement_code, c.libelle as cond_reglement_libelle, c.libelle_invoice as cond_reglement_libelle_doc';
		...
		$sql .= ', f.generate_pdf';
		$sql .= ' FROM '.MAIN_DB_PREFIX.'invoice_fourn_rec as f';
	...
	public function fetch_lines()
	{
		...
		*/
		...
		$sql = 'SELECT l.rowid,';
		$sql .= ' l.fk_invoice_fourn, l.fk_parent_line, l.fk_product, l.ref, l.label, l.description as line_desc,';
		...
		$sql .= ' FROM '.MAIN_DB_PREFIX.'invoice_fourn_det_rec as l';
		$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'product as p ON l.fk_product = p.rowid';
		$sql .= ' WHERE l.fk_invoice_fourn = '. (int) $this->id;
		$sql .= ' ORDER BY l.rang';

		dol_syslog('InvoiceSupplierRec::fetch_lines', LOG_DEBUG);
		...
		if ($result) {
			...
			while ($i < $num) {
				...
				$line = new InvoiceSupplierLigneRec($this->db);

				$line->id                       = $objp->rowid;
				$line->fk_invoice_fourn         = $objp->fk_invoice_fourn;
	...
	public function delete(User $user, $notrigger = 0, $idwarehouse = -1)
	{
		...
		$main = MAIN_DB_PREFIX.'invoice_fourn_det_rec';
		$ef = $main."_extrafields";

		$sqlef = "DELETE FROM ".$ef." WHERE fk_object IN (SELECT rowid FROM ".$main." WHERE fk_invoice_fourn = ". (int) $rowid .")";
		$sql = "DELETE FROM ".MAIN_DB_PREFIX."invoice_fourn_det_rec WHERE fk_invoice_fourn = ". (int) $rowid;

		if ($this->db->query($sqlef) && $this->db->query($sql)) {
			$sql = "DELETE FROM ".MAIN_DB_PREFIX."invoice_fourn_rec WHERE rowid = ". (int) $rowid;
			dol_syslog($sql);
			if ($this->db->query($sql)) {
	...
	public function addline($fk_product, $ref, $label, $desc, $pu_ht, $pu_ttc, $qty, $remise_percent, $txtva, $txlocaltax1 = 0, $txlocaltax2 = 0, $price_base_type = 'HT', $type = 0, $date_start = 0, $date_end = 0, $info_bits = 0, $special_code = 0, $rang = -1, $fk_unit = null, $pu_ht_devise = 0)
	{
		...
		if ($this->suspended == self::STATUS_NOTSUSPENDED) {
			...
			$sql = 'INSERT INTO ' . MAIN_DB_PREFIX . 'invoice_fourn_det_rec (';
			...
			if ($this->db->query($sql)) {
				$lineId = $this->db->last_insert_id(MAIN_DB_PREFIX. 'invoice_fourn_det_rec');
				$this->update_price();
				$this->id = $facid;
				$this->db->commit();
				return $lineId;
	...
	public function updateline($rowid, $fk_product, $ref, $label, $desc, $pu_ht, $qty, $remise_percent, $txtva, $txlocaltax1 = 0, $txlocaltax2 = 0, $price_base_type = 'HT', $type = 0, $date_start = 0, $date_end = 0, $info_bits = 0, $special_code = 0, $rang = -1, $fk_unit = null, $pu_ht_devise = 0, $pu_ttc = 0)
	{
		...
		if ($this->status == self::STATUS_SUSPENDED) {
			...
			$sql = 'UPDATE ' . MAIN_DB_PREFIX . 'invoice_fourn_det_rec SET';
			$sql .= ' fk_invoice_fourn = ' . ((int) $facid);
	 ...
	 *
	 *  WARNING: This method changes temporarily the context $config->entity to be in correct context for each recurring invoice found.
	 *
	...
	public function createRecurringInvoices($restrictioninvoiceid = 0, $forcevalidation = 0)
	{
		...
		$sql = 'SELECT rowid FROM '.MAIN_DB_PREFIX.'invoice_fourn_rec';
		...
		$sql .= ' AND suspended = 0';
		$sql .= ' AND entity = '. (int) $config->entity; // MUST STAY = $config->entity here
		...
		if ($resql) {
			...
			while ($i < $num) {     // Loop on each template invoice. If $num = 0, test is false at first pass.
				...
				$new_fac_fourn = null;
				$invoicerec = new InvoiceSupplierRec($this->db);
				...
				if ($invoicerec->id > 0) {
					...
					$new_fac_fourn = new InvoiceSupplier($this->db);
					$new_fac_fourn->fac_rec = $invoicerec->id; // We will create $invoice from this recurring invoice
					$new_fac_fourn->fk_fac_rec_source = $invoicerec->id; // We will create $invoice from this recurring invoice

					$new_fac_fourn->type = self::TYPE_STANDARD;
					$new_fac_fourn->subtype = $invoicerec->subtype;
					$new_fac_fourn->statut = self::STATUS_DRAFT;	// deprecated
					$new_fac_fourn->status = self::STATUS_DRAFT;
					$new_fac_fourn->date = empty($invoicerec->date_when) ? $now : $invoicerec->date_when; // We could also use dol_now here but we prefer date_when so invoice has real date when we would like even if we generate later.
					$new_fac_fourn->socid = $invoicerec->socid;
					$new_fac_fourn->lines = $invoicerec->lines;
					$new_fac_fourn->ref_supplier = $invoicerec->ref_supplier;
					$new_fac_fourn->model_pdf = $invoicerec->model_pdf;
					$new_fac_fourn->fk_project = $invoicerec->fk_project;
					$new_fac_fourn->label = $invoicerec->label;
					$new_fac_fourn->libelle = $invoicerec->label;	// deprecated
					...
					if (!$error && $invoicerec->generate_pdf) {
						...
						$result = $new_fac_fourn->generateDocument($invoicerec->model_pdf, $langs);
						if ($result < 0) {
							$this->errors = $new_fac_fourn->errors;
							$this->error = $new_fac_fourn->error;
							$error++;
				...
				if (!$error && $invoiceidgenerated >= 0) {
					$invoicerec->nb_gen_done++;
					$invoicerec->date_last_gen = dol_now();
					$nextDate = $invoicerec->getNextDate();
					$invoicerec->date_when = (($nextDate === false) ? null : $nextDate);
					$invoicerec->update($user);
					$this->db->commit('createRecurringInvoices Process invoice template id=' .$invoicerec->id. ', title=' .$invoicerec->title);
					dol_syslog('createRecurringInvoices Process invoice template ' .$invoicerec->title. ' is finished with a success generation');
					$nb_create++;
					$this->output .= $langs->trans('InvoiceGeneratedFromTemplate', $new_fac_fourn->ref, $invoicerec->title)."\n";
				} else {
					$this->db->rollback('createRecurringInvoices Process invoice template error='.$error.' invoiceidgenerated='.$invoiceidgenerated.' LastStep='.$laststep.' id=' .$invoicerec->id. ', title=' .$invoicerec->title);
				}
				...
				$resHook = $hookManager->executeHooks('afterCreationOfRecurringInvoice', $parameters, $new_fac_fourn); // note: $invoice can be modified by hooks (warning: $invoice can be null)
	...
	public function getNomUrl($withPicture = 0, $option = '', $max = 0, $short = 0, $moretitle = '', $notooltip = 0, $save_lastsearch_value = -1)
	{
		...
		$url = DOL_URL_ROOT.'/fourn/invoice/card-rec.php?facid='.$this->id;
