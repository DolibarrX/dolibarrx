...
/**
 *  \file       htdocs/fourn/class/fournisseur.invoice.class.php
...
require_once DOL_DOCUMENT_ROOT.'/fourn/class/fournisseur.invoice.ligne.class.php';
 ...
 */
class InvoiceSupplier extends CommonInvoice
{
	/**
	 ...
	 */
	public $table_element = 'invoice_fourn';

	/**
	 * @var string    Name of subtable line
	 */
	public $table_element_line = 'invoice_fourn_det';
	 ...
	 */
	public $fk_element = 'fk_invoice_fourn';
	 ...
	 * Supplier invoice status
	 * @var int
	 * @see InvoiceSupplier::STATUS_DRAFT, InvoiceSupplier::STATUS_VALIDATED, InvoiceSupplier::STATUS_PAID, InvoiceSupplier::STATUS_ABANDONED
	 */
	public $status;
	...
	public $fields = array(
		...
		'fk_invoice_source' => array('type' => 'integer', 'label' => 'Fk invoice source', 'enabled' => 1, 'visible' => -1, 'position' => 140),
	...
	public function create($user)
	{
		...
		if ($this->fac_rec > 0) {
			...
			$_facrec = new InvoiceSupplierRec($this->db);
		...
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."invoice_fourn (";
		$sql .= "ref";
		$sql .= ", ref_supplier";
		$sql .= ", ref_ext";
		$sql .= ", entity";
		...
		if ($resql) {
			$this->id = $this->db->last_insert_id(MAIN_DB_PREFIX.'invoice_fourn');

			// Update ref with new one
			$this->ref = '(PROV'.$this->id.')';
			$sql = 'UPDATE '.MAIN_DB_PREFIX."invoice_fourn SET ref='".$this->db->escape($this->ref)."' WHERE rowid=".((int) $this->id);
			...
			if (!$error && empty($this->fac_rec) && count($this->lines) && is_object($this->lines[0])) {	// If this->lines is array of InvoiceLines (preferred mode)
				...
				foreach ($this->lines as $i => $val) {
					$sql = 'INSERT INTO '.MAIN_DB_PREFIX.'invoice_fourn_det (fk_invoice_fourn, special_code, fk_remise_except)';
					...
					if ($resql_insert) {
						$idligne = $this->db->last_insert_id(MAIN_DB_PREFIX.'invoice_fourn_det');
			 ...
			 */
			if (! $error && $this->fac_rec > 0 && $_facrec instanceof InvoiceSupplierRec) {
	...
	public function fetch($id = 0, $ref = '', $ref_ext = '')
	{
		...
		$sql .= ' FROM '.MAIN_DB_PREFIX.'invoice_fourn as t';
	...
	public function fetch_lines()
	{
		...
		$sql .= ', f.localtax1_tx, f.localtax2_tx, f.localtax1_type, f.localtax2_type, f.total_localtax1, f.total_localtax2, f.fk_invoice_fourn, f.fk_remise_except';
		...
		$sql .= ' FROM '.MAIN_DB_PREFIX.'invoice_fourn_det as f';
		...
		if ($resql_rows) {
			...
			if ($num_rows) {
				...
				while ($i < $num_rows) {
					...
					$line->fk_invoice_fourn = $obj->fk_invoice_fourn;
	...
	public function update($user = null, $notrigger = 0)
	{
		...
		if (isset($this->fk_invoice_source)) {
			$this->fk_invoice_source = (int) $this->fk_invoice_source;
		}
		...
		// Update request
		$sql = "UPDATE ".MAIN_DB_PREFIX."invoice_fourn SET";
		...
		$sql .= " fk_invoice_source=".($this->fk_invoice_source ? ((int) $this->fk_invoice_source) : "null").",";
	...
	public function insert_discount($idremise)
	{
		...
		if ($result > 0) {
			...
			$facligne->fk_invoice_fourn = $this->id;
			...
			if ($remise->fk_invoice_supplier_source > 0) {
				$srcinvoice = new InvoiceSupplier($this->db);
	...
	public function delete(User $user, $notrigger = 0)
	{
		...
		dol_syslog("InvoiceSupplier::delete rowid=".$rowid, LOG_DEBUG);
		...
		if (!$error) {
			$main = MAIN_DB_PREFIX.'invoice_fourn_det';
			$ef = $main."_extrafields";
			$sqlef = "DELETE FROM $ef WHERE fk_object IN (SELECT rowid FROM ".$main." WHERE fk_invoice_fourn = ".((int) $rowid).")";
			$resqlef = $this->db->query($sqlef);
			$sql = 'DELETE FROM '.MAIN_DB_PREFIX.'invoice_fourn_det WHERE fk_invoice_fourn = '.((int) $rowid);
			dol_syslog(get_class($this)."::delete", LOG_DEBUG);
			$resql = $this->db->query($sql);
			if ($resqlef && $resql) {
				$sql = 'DELETE FROM '.MAIN_DB_PREFIX.'invoice_fourn WHERE rowid = '.((int) $rowid);
				dol_syslog(get_class($this)."::delete", LOG_DEBUG);
		...
		if (!$error) {
			...
			if ($config->fournisseur->invoice->dir_output) {
				...
				$dir = $config->fournisseur->invoice->dir_output.'/'.get_exdir($this->id, 2, 0, 0, $this, 'invoice_supplier').$ref;
				$file = $dir."/".$ref.".pdf";
				if (file_exists($file)) {
	...
	public function setPaid($user, $close_code = '', $close_note = '')
	{
		...
		if ($this->paid != 1) {
			...
			dol_syslog("InvoiceSupplier::setPaid", LOG_DEBUG);
	...
	public function setUnpaid($user)
	{
		...
		$sql = 'UPDATE '.MAIN_DB_PREFIX.'invoice_fourn';
	...
	public function validate($user, $force_number = '', $idwarehouse = 0, $notrigger = 0)
	{
		...
		$resql = $this->db->query($sql);
		if ($resql) {
			// Si on incrémente le produit principal et ses composants à la validation de invoice fournisseur
			if (!$error && isModEnabled('stock') && getDolGlobalString('STOCK_CALCULATE_ON_SUPPLIER_BILL')) {
				...
				for ($i = 0; $i < $cpt; $i++) {
					if ($this->lines[$i]->fk_product > 0) {
						...
						if ($this->type == InvoiceSupplier::TYPE_CREDIT_NOTE) {
			...
			if (!$error) {
				...
				if (preg_match('/^[\(]?PROV/i', $this->ref)) {
					...
					$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'fournisseur/invoice/".get_exdir($this->id, 2, 0, 0, $this, 'invoice_supplier').$this->db->escape($this->newref)."'";
					$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'fournisseur/invoice/".get_exdir($this->id, 2, 0, 0, $this, 'invoice_supplier').$this->db->escape($this->ref)."' and entity = ".$config->entity;
					$resql = $this->db->query($sql);
					if (!$resql) {
						$error++;
						$this->error = $this->db->lasterror();
					}
					$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'fournisseur/invoice/".get_exdir($this->id, 2, 0, 0, $this, 'invoice_supplier').$this->db->escape($this->newref)."'";
					$sql .= " WHERE filepath = 'fournisseur/invoice/".get_exdir($this->id, 2, 0, 0, $this, 'invoice_supplier').$this->db->escape($this->ref)."' and entity = ".$config->entity;
					$resql = $this->db->query($sql);
					if (!$resql) {
						$error++;
						$this->error = $this->db->lasterror();
					...
					$dirsource = $config->fournisseur->invoice->dir_output.'/'.get_exdir($this->id, 2, 0, 0, $this, 'invoice_supplier').$oldref;
					$dirdest = $config->fournisseur->invoice->dir_output.'/'.get_exdir($this->id, 2, 0, 0, $this, 'invoice_supplier').$this->newref;
					if (!$error && file_exists($dirsource)) {
						...
						if (@rename($dirsource, $dirdest)) {
							...
							$listoffiles = dol_dir_list($config->fournisseur->invoice->dir_output.'/'.get_exdir($this->id, 2, 0, 0, $this, 'invoice_supplier').$this->newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $idwarehouse = -1, $notrigger = 0)
	{
		...
		if ($result) {
			...
			// Si on incremente le produit principal et ses composants a la validation de invoice fournisseur, on decremente
	...
	public function addline($desc, $pu, $txtva, $txlocaltax1, $txlocaltax2, $qty, $fk_product = 0, $remise_percent = 0, $date_start = 0, $date_end = 0, $fk_code_ventilation = 0, $info_bits = 0, $price_base_type = 'HT', $type = 0, $rang = -1, $notrigger = 0, $array_options = [], $fk_unit = null, $origin_id = 0, $pu_devise = 0, $ref_supplier = '', $special_code = 0, $fk_parent_line = 0, $fk_remise_except = 0)
	{
		...
		if ($this->status == self::STATUS_DRAFT) {
			...
			$supplierinvoiceline->fk_invoice_fourn = $this->id;
	...
	public function info($id)
	{
		...
		$sql .= ' FROM '.MAIN_DB_PREFIX.'invoice_fourn as c';
	...
	public function list_replacable_supplier_invoices($socid = 0)
	{
		...
		$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."invoice_fourn as ff ON f.rowid = ff.fk_invoice_source";
		...
		$sql .= " AND ff.fk_statut IS NULL"; // Renvoi vrai si pas invoice de replacement
		if ($socid > 0) {
	...
	public function list_qualified_avoir_supplier_invoices($socid = 0)
	{
		...
		$sql .= " AND NOT EXISTS (SELECT rowid from ".MAIN_DB_PREFIX."invoice_fourn as ff WHERE f.rowid = ff.fk_invoice_source";
		$sql .= " AND ff.type=".self::TYPE_REPLACEMENT.")";
		$sql .= " AND f.type != ".self::TYPE_CREDIT_NOTE; // Type non 2 si invoice non avoir
		if ($socid > 0) {
			$sql .= " AND f.fk_soc = ".((int) $socid);
		}
		$sql .= " ORDER BY f.ref";
	...
	public function load_board($user)
	{
		...
		if ($resql) {
			...
			$response->warning_delay = $config->invoice->fournisseur->warning_delay / 60 / 60 / 24;
			$response->label = $langs->trans("SupplierBillsToPay");
			$response->labelShort = $langs->trans("StatusToPay");

			$response->url = DOL_URL_ROOT.'/fourn/invoice/list.php?search_status=1&mainmenu=billing&leftmenu=suppliers_bills';
			$response->img = img_object($langs->trans("Bills"), "bill");

			$invoicestatic = new InvoiceSupplier($this->db);

			while ($obj = $this->db->fetch_object($resql)) {
				$invoicestatic->date_echeance = $this->db->jdate($obj->datefin);
				$invoicestatic->statut = $obj->status;	// For backward compatibility
				...
				if ($invoicestatic->hasDelay()) {
					$response->nbtodolate++;
					$response->url_late = DOL_URL_ROOT.'/fourn/invoice/list.php?search_option=late&mainmenu=billing&leftmenu=suppliers_bills';
				}
			}
	...
	public function getNomUrl($withPicture = 0, $option = '', $max = 0, $short = 0, $moretitle = '', $notooltip = 0, $save_lastsearch_value = -1, $addlinktonotes = 0)
	{
		...
		if ($option == 'withdraw') {
			$url = DOL_URL_ROOT.'/compta/invoice/prelevement.php?facid='.$this->id.'&type=bank-transfer';
		} elseif ($option == 'document') {
			$url = DOL_URL_ROOT.'/fourn/invoice/document.php?facid='.$this->id;
		} else {
			$url = DOL_URL_ROOT.'/fourn/invoice/card.php?facid='.$this->id;
		}
		...
		if ($addlinktonotes) {
			...
			if ($txttoshow) {
				...
				$result .= '<a href="'.DOL_URL_ROOT.'/fourn/invoice/note.php?id='.$this->id.'" class="classfortooltip" title="'.dol_escape_htmltag($notetoshow).'">';
	...
	public function getNextNumRef($soc, $mode = 'next')
	{
		...
		if (!getDolGlobalString('INVOICE_SUPPLIER_ADDON_NUMBER')) {
			$config->global->INVOICE_SUPPLIER_ADDON_NUMBER = 'mod_invoice_fournisseur_cactus';
		}
	...
	public function getRights()
	{
		...
		return $user->hasRight("fournisseur", "invoice");
	}
	...
	public function hasDelay()
	{
		...
		return ($status == self::STATUS_VALIDATED) && ($this->date_echeance < ($now - $config->invoice->fournisseur->warning_delay));
	}
	...
	public function sendEmailsRemindersOnSupplierInvoiceDueDate($nbdays = 0, $paymentmode = 'all', $template = '', $datetouse = 'duedate', $forcerecipient = '')
	{
		...
		$tmpinvoice = new InvoiceSupplier($this->db);

		dol_syslog(__METHOD__." start", LOG_INFO);

		// Select all action comm reminder
		$sql = "SELECT rowid as id FROM ".MAIN_DB_PREFIX."invoice_fourn as f";
