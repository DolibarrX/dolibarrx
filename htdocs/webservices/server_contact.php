...
$contact_fields = array(
	...
	'ref_contract' => array('name' => 'ref_contract', 'type' => 'xsd:string'),
		...
		if ($result > 0) {
			...
			if ($fuser->hasRight('societe', 'contact', 'lire') && !$fuser->socid
				...
				$contact_result_fields = array(
					...
					'ref_contract' => $contact->ref_contract,
...
function createContact($authentication, $contact)
{
	...
	if (!$error) {
		...
		$newobject->ref_contract = (int) $contact['ref_contract'];
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				$linescontact[] = array(
					...
					'ref_contract' => $contact->ref_contract ? $contact->ref_contract : '',
