...
if (!empty($searchCategoryContactList)) {
	...
	foreach ($searchCategoryContactList as $searchCategoryContact) {
		if (intval($searchCategoryContact) == -2) {
			$searchCategoryContactSqlList[] = "NOT EXISTS (SELECT ck.fk_socpeople FROM ".MAIN_DB_PREFIX."category_contact as ck WHERE p.rowid = ck.fk_socpeople)";
		} elseif (intval($searchCategoryContact) > 0) {
			if ($searchCategoryContactOperator == 0) {
				$searchCategoryContactSqlList[] = " EXISTS (SELECT ck.fk_socpeople FROM ".MAIN_DB_PREFIX."category_contact as ck WHERE p.rowid = ck.fk_socpeople AND ck.fk_category = ".((int) $searchCategoryContact).")";
			} else {
	...
	if ($listofcategoryid) {
		$searchCategoryContactSqlList[] = " EXISTS (SELECT ck.fk_socpeople FROM ".MAIN_DB_PREFIX."category_contact as ck WHERE p.rowid = ck.fk_socpeople AND ck.fk_category IN (".$db->sanitize($listofcategoryid)."))";
	}
