...
if (empty($resHook)) {
	$backurlforlist = DOL_URL_ROOT.'/members/list.php';
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = DOL_URL_ROOT.'/members/card.php?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
		...
		if ($result >= 0 && !count($object->errors)) {
			...
			if ($object->email && GETPOST("send_mail")) {
				...
				if (empty($labeltouse) || (int) $labeltouse === -1) {
					...
					$langs->load("errors");
					setEventMessages('<a href="'.DOL_URL_ROOT.'/members/admin/member_emails.php">'.$langs->trans('WarningMandatorySetupNotComplete').'</a>', null, 'errors');
					$error++;
				} else {
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($action == 'create') {
		...
		if ($user->hasRight('member', 'configurer')) {
			print ' <a href="'.DOL_URL_ROOT.'/members/type.php?action=create&backtopage='.urlencode($_SERVER["PHP_SELF"].'?action=create&typeid=--IDFORBACKTOPAGE--').'"><span class="fa fa-plus-circle valignmiddle paddingleft" title="'.$langs->trans("NewMemberType").'"></span></a>';
		}
		print "</td>\n";
	...
	if ($id > 0 && $action != 'edit') {
		...
		if ($object->datefin) {
			...
			if ($object->need_subscription == 0) {
				...
				if (Member::STATUS_VALIDATED == $object->status) {
		...
		if (empty($resHook)) {
			if ($action != 'editlogin' && $action != 'editthirdparty') {
				...
				// Validate
				if (Member::STATUS_DRAFT == $object->status) {
					if ($user->hasRight('member', 'creer')) {
				...
				// Reactivate
				if (Member::STATUS_RESILIATED == $object->status || Member::STATUS_EXCLUDED == $object->status) {
					if ($user->hasRight('member', 'creer')) {
		...
		if ($action != 'presend') {
			...
			$morehtmlcenter = '';
			$messagingUrl = DOL_URL_ROOT.'/members/messaging.php?rowid='.$object->id;
			$morehtmlcenter .= dolGetButtonTitle($langs->trans('ShowAsConversation'), '', 'fa fa-comments imgforviewmode', $messagingUrl, '', 1);
			$morehtmlcenter .= dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', DOL_URL_ROOT.'/members/agenda.php?id='.$object->id);
