...
/**
 *      \file       htdocs/members/subscription/list.php
 *      \ingroup    member
...
if ($num == 1 && getDolGlobalString('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/members/subscription/card.php?id='.$id);
	exit;
}
...
print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $subscription->picture, 0, $newcardbutton, '', $limit, 0, 0, 1);
...
if (!empty($arrayfields['d.fk_type']['checked'])) {
	print '<td class="liste_titre left">';
	print $form->selectarray("search_type", $adht->liste_[], $search_type, 1);
	print'</td>';
}
...
while ($i < $imaxinloop) {
	...
	if ($mode == 'kanban') {
		...
		// Output Kanban
		print $subscription->getKanbanView('', array('selected' => in_array($object->id, $arrayofselected), 'member_type' => $adht, 'member' => $member, 'bank' => ($obj->fk_account > 0 ? $accountstatic : null)));
		if ($i == ($imaxinloop - 1)) {
			print '</div>';
			print '</td></tr>';
		}
