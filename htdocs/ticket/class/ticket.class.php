...
class Ticket extends CommonObject
{
	...
	public $fields = array(
		...
		'fk_contract' => array('type' => 'integer:Contract:contract/class/contract.class.php', 'label' => 'Contract', 'visible' => -1, 'enabled' => '$config->contract->enabled', 'position' => 53, 'notnull' => -1, 'index' => 1, 'help' => "LinkToContract"),
	...
	public function getNomUrl($withPicture = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$result .= $linkstart;
		if ($withPicture) {
			$result .= img_object(($notooltip ? '' : $label), ($this->picture ?: 'generic'), (($withPicture != 2) ? 'class="paddingright"' : ''), 0, 0, $notooltip ? 0 : 1);
		}
		if ($withPicture != 2) {
			$result .= $this->ref;
		}
