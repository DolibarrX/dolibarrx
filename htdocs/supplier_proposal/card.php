...
if ($action == 'create') {
	...
	if (!empty($origin) && !empty($originid)) {
		...
		'@phan-var-force Order|Propal|OrderFournisseur|SupplierProposal $objectsrc';  // Could be other classes, but CommonObject is too generic
	...
	if (empty($resHook)) {
		...
		if (isModEnabled('project')) {
			...
			print img_picture('', 'project', 'class="picturefixedwidth"').$formproject->select_projects(($soc->id > 0 ? $soc->id : -1), $projectid, 'projectid', 0, 0, 1, 1, 0, 0, 0, '', 1, 0, 'maxwidth500');
