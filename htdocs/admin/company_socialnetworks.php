	...
	foreach ($listofnetworks as $key => $value) {
		if (!empty($value['active'])) {
			...
			if (GETPOSTISSET($key.'url') && GETPOST($key.'url', 'alpha') != '') {
				dolibarr_set_const($db, $networkconstname, GETPOST($key.'url', 'alpha'), 'chaine', 0, '', $config->entity);
				dolibarr_set_const($db, $networkconstid, GETPOST($key, 'alpha'), 'chaine', 0, '', $config->entity);
				...
				if (!empty($listofnetworks[$key]['url'])) {
					...
					dolibarr_set_const($db, $networkconstname, $url, 'chaine', 0, '', $config->entity);
				}
				dolibarr_set_const($db, $networkconstid, GETPOST($key, 'alpha'), 'chaine', 0, '', $config->entity);
			} else {
				dolibarr_del_const($db, $networkconstname, $config->entity);
				dolibarr_del_const($db, $networkconstid, $config->entity);
			}
		}
	}
