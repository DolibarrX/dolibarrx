					...
					if (file_exists($dir.'/'.$file)) {
						...
						if ($modulequalified) {
							...
							if ($config->global->EXPENSEREPORT_ADDON_PDF == "$name") {
								...
								print '<a href="'.$_SERVER["PHP_SELF"].'?action=setdoc&token='.newToken().'&value='.$name.'&scan_dir='.$module->scandir.'&label='.urlencode($module->name).'" alt="'.$langs->trans("Default").'">'.img_picture($langs->trans("Disabled"), 'off').'</a>';
							}
							print '</td>';
...
print $form->textWithPicture($langs->trans("FreeLegalTextOnExpenseReports"), $langs->trans("AddCRIfTooLong").'<br><br>'.$htmltext, 1, 'help', '', 0, 2, 'freetexttooltip').'<br>';
...
print '<tr class="oddeven"><td colspan="2">';
print $form->textWithPicture($langs->trans("WatermarkOnDraftExpenseReports"), $htmltext, 1, 'help', '', 0, 2, 'watermarktooltip').'<br>';
