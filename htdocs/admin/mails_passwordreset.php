...
if ($action == 'edit') {
	...
	if (!empty($config->use_javascript_ajax) || in_array(getDolGlobalString('MAIN_MAIL_SENDMODE_PASSWORDRESET'), array('smtps', 'swiftmailer'))) {
		...
		if (!isModEnabled('multicompany') || ($user->admin && !$user->entity)) {
			...
			$htmltext = $langs->trans("ContactSuperAdminForChange");
			print $form->textWithPicture(getDolGlobalString('MAIN_MAIL_SMTPS_ID_PASSWORDRESET'), $htmltext, 1, 'superadmin');
	...
	if (!empty($config->use_javascript_ajax) || in_array(getDolGlobalString('MAIN_MAIL_SENDMODE_PASSWORDRESET'), array('smtps', 'swiftmailer'))) {
		...
		if (!isModEnabled('multicompany') || ($user->admin && !$user->entity)) {
			...
			$htmltext = $langs->trans("ContactSuperAdminForChange");
			print $form->textWithPicture(getDolGlobalString('MAIN_MAIL_SMTPS_PW_PASSWORDRESET'), $htmltext, 1, 'superadmin');
