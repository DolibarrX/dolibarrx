...
if (isModEnabled('invoice') && $user->hasRight('invoice', 'lire')) {
	$elementList['invoice_send'] = img_picture('', 'bill', 'class="picturefixedwidth"').dol_escape_htmltag($langs->trans('MailToSendInvoice'));
}
...
foreach ($fieldlist as $field => $value) {
	if ($value == 'module') {
		...
		print $formadmin->select_language($search_lang, 'search_lang', 0, [], 1, 0, 0, 'maxwidth100');
		print '</td>';
	} elseif ($value == 'fk_user') {
		print '<td class="liste_titre">';
		print $form->select_dolusers($search_fk_user, 'search_fk_user', 1, null, 0, ($user->admin ? '' : 'hierarchyme'), [], 0, 0, 0, '', 0, '', 'maxwidth100', 1);
		print '</td>';
	} elseif ($value == 'topic') {
 ...
 *  @param		string	$context		'add'=Output field for the "add form", 'edit'=Output field for the "edit form", 'preview'=show in readonly the template, 'hide'=Output field for the "add form" but we do not want it to be rendered
 *	@return		void
 */
	...
	foreach ($fieldlist as $field => $value) {
		...
		if ($value == 'module') {
			...
			if ($user->admin && $context != 'preview') {
				print $form->select_dolusers(GETPOSTISSET('fk_user') ? GETPOSTINT('fk_user') : (empty($obj->$value) ? '' : $obj->$value), 'fk_user', 1, [], 0, ($user->admin ? '' : 'hierarchyme'), [], 0, 0, 0, '', 0, '', 'minwidth75 maxwidth100');
			} else {
			...
			if (getDolGlobalInt('MAIN_MULTILANGS') && $context != 'preview') {
				...
				print $formadmin->select_language($selectedlang, 'langcode', 0, [], 1, 0, 0, 'maxwidth100');
			} else {
				if (!empty($obj->lang)) {
