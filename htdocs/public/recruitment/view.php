...
if (!$config->global->RECRUITMENT_ENABLE_PUBLIC_INTERFACE) {
	$langs->load("errors");
...
if ($action != 'dosubmit') {
	if ($found && !$error) {
		...
		print img_picture("", "email").'<input type="text" class="flat minwidth100 --success" name="email" value="'.$email.'">';
		print '</td></tr>'."\n";

		print '<tr><td class="titlefieldcreate left">'.$langs->trans("Phone").'</td><td class="left">';
		print img_picture("", "phone").'<input type="text" class="flat minwidth100 --success" name="phone" value="'.$phone.'">';
		print '</td></tr>'."\n";
