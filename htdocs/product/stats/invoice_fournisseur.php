...
/**
 * \file 		htdocs/product/stats/invoice_fournisseur.php
...
$supplierinvoicestatic = new InvoiceSupplier($db);
...
if ($id > 0 || !empty($ref)) {
	...
	llxHeader("", "", $langs->trans("CardProduct".$product->type), '', 0, 0, '', '', 'mod-product page-stats_invoice_fournisseur');
	...
	if ($result > 0) {
		...
		if ($user->hasRight('fournisseur', 'invoice', 'lire')) {
			...
			$sql .= ", ".MAIN_DB_PREFIX."invoice_fourn_det as d";
			if (!$user->hasRight('societe', 'client', 'voir')) {
				$sql .= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
			}
			$sql .= " WHERE f.fk_soc = s.rowid";
			$sql .= " AND f.entity IN (".getEntity('invoice_fourn').")";
			$sql .= " AND d.fk_invoice_fourn = f.rowid";
