...
/**
 *	\file       htdocs/product/stats/invoicerec.php
 *	\ingroup    product service template invoice
...
$invoicestatic = new InvoiceRec($db);
...
if ($id > 0 || !empty($ref)) {
	...
	llxHeader('', $title, $helpurl, '', 0, 0, '', '', 'mod-product page-stats_invoicerec');
	...
	if ($result > 0) {
		...
		if ($showmessage && $nboflines > 1) {
			...
			$sql .= ", ".MAIN_DB_PREFIX."invoicedet_rec as d";
			if (!$user->hasRight('societe', 'client', 'voir')) {
			...
			if ($result) {
				...
				if ($num > 0) {
					while ($i < min($num, $limit)) {
						...
						if ($objp->type == InvoiceRec::TYPE_CREDIT_NOTE) {
							$objp->qty = -($objp->qty);
						}
