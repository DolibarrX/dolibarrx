...
/**
 *	\file       htdocs/product/stats/invoice.php
...
$search_array_options = $extrafields->getOptionalsFromPost('invoice', '', 'search_');
...
if ($id > 0 || !empty($ref)) {
	...
	llxHeader('', $title, $helpurl, '', 0, 0, '', '', '', 'mod-product page-stats_invoice');
	...
	if ($result > 0) {
		...
		if ($showmessage && $nboflines > 1) {
		...
		} elseif ($user->hasRight('invoice', 'lire')) {
			...
			if (!empty($extrafields->attributes['invoice']['label'])) {
				foreach ($extrafields->attributes['invoice']['label'] as $key => $val) {
					$sql .= ($extrafields->attributes['invoice']['type'][$key] != 'separate' ? ", ef.".$key." as options_".$key : '');
				}
			}
			...
			if (isset($extrafields->attributes['invoice']['label']) && is_array($extrafields->attributes['invoice']['label']) && count($extrafields->attributes['invoice']['label'])) {
				$sql .= " LEFT JOIN ".MAIN_DB_PREFIX.'invoice'."_extrafields as ef on (f.rowid = ef.fk_object)";
			}
