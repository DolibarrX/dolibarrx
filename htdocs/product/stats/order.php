...
/**
 *       \file       htdocs/product/stats/order.php
 *       \ingroup    product service order
 *       \brief      Page des stats des orders clients pour un produit
 */
...
if ($id > 0 || !empty($ref)) {
	...
	llxHeader("", "", $langs->trans("CardProduct".$product->type), '', 0, 0, '', '', 'mod-product page-stats_order');
	...
	if ($result > 0) {
		...
		if ($user->hasRight('order', 'lire')) {
			...
			$sql .= ", ".MAIN_DB_PREFIX."orderdet as d";
			if (!$user->hasRight('societe', 'client', 'voir')) {
			...
			if ($result) {
				...
				print_liste_field_titre("OrderDate", $_SERVER["PHP_SELF"], "c.date_order", "", $option, '', $sortfield, $sortorder, 'center ');
