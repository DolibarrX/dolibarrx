...
/**
 *	\file       htdocs/order/list_det.php
 *	\ingroup    order
...
$arrayfields = array(
	...
	'p.ref' => array('label' => "ProjectRef", 'checked' => -1, 'enabled' => (empty($config->project->enabled) ? 0 : 1), 'position' => 20),
	'p.title' => array('label' => "ProjectLabel", 'checked' => 0, 'enabled' => (empty($config->project->enabled) ? 0 : 1), 'position' => 25),
	...
	'c.date_order' => array('label' => "OrderDateShort", 'checked' => 1, 'position' => 60),
	...
	'c.multicurrency_code' => array('label' => 'Currency', 'checked' => 0, 'enabled' => (empty($config->multicurrency->enabled) ? 0 : 1), 'position' => 90),
	'c.multicurrency_tx' => array('label' => 'CurrencyRate', 'checked' => 0, 'enabled' => (empty($config->multicurrency->enabled) ? 0 : 1), 'position' => 95),
	'c.multicurrency_total_ht' => array('label' => 'MulticurrencyAmountHT', 'checked' => 0, 'enabled' => (empty($config->multicurrency->enabled) ? 0 : 1), 'position' => 100),
	'c.multicurrency_total_vat' => array('label' => 'MulticurrencyAmountVAT', 'checked' => 0, 'enabled' => (empty($config->multicurrency->enabled) ? 0 : 1), 'position' => 105),
	'c.multicurrency_total_ttc' => array('label' => 'MulticurrencyAmountTTC', 'checked' => 0, 'enabled' => (empty($config->multicurrency->enabled) ? 0 : 1), 'position' => 110),
...
if (($search_categ_cus > 0) || ($search_categ_cus == -2)) {
	$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX."category_societe as cc ON s.rowid = cc.fk_soc"; // We'll need this table joined to the select in order to filter by categ
}

// Détail order
$sql .= ', '.MAIN_DB_PREFIX.'orderdet as cdet';
$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'order as c ON cdet.fk_order=c.rowid';
...
if ($search_user > 0) {
	$sql .= " AND ec.fk_c_type_contact = tc.rowid AND tc.element='order' AND tc.source='internal' AND ec.element_id = c.rowid AND ec.fk_socpeople = ".((int) $search_user);
}
if ($search_total_ht != '') {
...
if (!empty($searchCategoryProductList)) {
	...
	foreach ($searchCategoryProductList as $searchCategoryProject) {
		if (intval($searchCategoryProject) == -2) {
			$searchCategoryProjectSqlList[] = "NOT EXISTS (SELECT cp.fk_product FROM ".MAIN_DB_PREFIX."category_product as cp WHERE cdet.fk_product = cp.fk_product)";
		} elseif (intval($searchCategoryProject) > 0) {
			if ($searchCategoryProductOperator == 0) {
				$searchCategoryProjectSqlList[] = " EXISTS (SELECT cp.fk_product FROM ".MAIN_DB_PREFIX."category_product as cp WHERE cdet.fk_product = cp.fk_product AND cp.fk_category = ".((int) $searchCategoryProject).")";
			} else {
	...
	if ($listofcategoryid) {
		$searchCategoryProjectSqlList[] = " EXISTS (SELECT cp.fk_product FROM ".MAIN_DB_PREFIX."category_product as cp WHERE cdet.fk_product = cp.fk_product AND cp.fk_category IN (".$db->sanitize($listofcategoryid)."))";
	}
...
if ($resql) {
	...
	llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-order page-list_det');
	...
	while ($i < $imaxinloop) {
		...
		$generic_order->total_ht = $obj->c_total_ht;
		$generic_order->total_tva = $obj->c_total_tva;
		$generic_order->total_ttc = $obj->c_total_ttc;
		...
		if (!empty($arrayfields['c.ref']['checked'])) {
			...
			print $generic_order->getNomUrl(1, ($search_status != 2 ? 0 : $obj->fk_statut), 0, 0, 0, 1, 1);
		...
		if (!empty($arrayfields['shippable']['checked'])) {
			...
			if (!empty($show_shippable_command) && isModEnabled('stock')) {
				...
				if (($obj->fk_statut > $generic_order::STATUS_DRAFT) && ($obj->fk_statut < $generic_order::STATUS_CLOSED)) {
					...
					if (isset($generic_order->expeditions[$obj->rowid])) {
						$reliquat =  $obj->qty - $generic_order->expeditions[$obj->rowid];
					} else {
						$reliquat = $obj->qty;
					}
							...
							if (getDolGlobalString('STOCK_CALCULATE_ON_SHIPMENT') || getDolGlobalString('STOCK_CALCULATE_ON_SHIPMENT_CLOSE')) {    // What about other options ?
								if (isModEnabled('order')) {
									if (empty($productstat_cache[$obj->fk_product]['stats_order_customer'])) {
										...
										$productstat_cache[$obj->fk_product]['stats_order_customer'] = $generic_product->stats_order['qty'];
									} else {
										$generic_product->stats_order['qty'] = $productstat_cache[$obj->fk_product]['stats_order_customer'];
									}
								...
								if (isModEnabled('supplier_order')) {
									if (empty($productstat_cache[$obj->fk_product]['stats_order_supplier'])) {
										...
										$productstat_cache[$obj->fk_product]['stats_order_supplier'] = $generic_product->stats_order_fournisseur['qty'];
									} else {
										$generic_product->stats_order_fournisseur['qty'] = $productstat_cache[$obj->fk_product]['stats_order_supplier'];
									}
