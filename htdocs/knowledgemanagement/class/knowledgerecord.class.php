...
class KnowledgeRecord extends CommonObject
{
	 ...
	 */
	public $picture = 'knowledgemanagement';
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($config->global->MAIN_USE_ADVANCED_PERMS) && $user->hasRight('knowledgemanagement', 'knowledgerecord', 'write'))
		 || (!empty($config->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->knowledgemanagement->knowledgerecord->knowledgerecord_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'knowledgerecord/".$this->db->escape($this->ref)."' and entity = ".$config->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$sql .= " WHERE filepath = 'knowledgerecord/".$this->db->escape($this->ref)."' and entity = ".$config->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $config->knowledgemanagement->dir_output.'/knowledgerecord/'.$oldref;
				$dirdest = $config->knowledgemanagement->dir_output.'/knowledgerecord/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($config->knowledgemanagement->dir_output.'/knowledgerecord/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($config->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->knowledgemanagement->write))
		 || (!empty($config->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->knowledgemanagement->knowledgemanagement_advance->validate))))
		 {
	...
	public function getTooltipContentArray($params)
	{
		...
		$datas['lang'] = '<br><b>'.$langs->trans('Language').':</b> ' . picture_from_langcode($this->lang, 'class="paddingrightonly saturatemedium opacitylow"') . $labellang;
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('KNOWLEDGEMANAGEMENT_KNOWLEDGERECORD_ADDON')) {
			$config->global->KNOWLEDGEMANAGEMENT_KNOWLEDGERECORD_ADDON = 'mod_knowledgerecord_standard';
		}
	...
	public function getKanbanView($option = '', $arraydata = null)
	{
		...
		if (property_exists($this, 'lang') && !empty($this->lang)) {
			...
			$return .= '<br>'.picture_from_langcode($this->lang, 'class="paddingrightonly saturatemedium opacitylow paddingrightonly"');
		}
