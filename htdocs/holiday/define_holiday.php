...
$typeleaves = $holiday->getTypes(1, 1);
$result = $holiday->updateBalance(); // Create users into table holiday if they do not exists. TODO Remove this whif we use field into table user.
if ($result < 0) {
	setEventMessages($holiday->error, $holiday->errors, 'errors');
}
...
if ($permissiontosetup) {
	$arrayofmassactions['preincreaseholiday'] = img_picture('', 'add', 'class="picturefixedwidth"').$langs->trans("IncreaseHolidays");
}
