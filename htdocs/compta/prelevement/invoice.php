...
/**
 *     \file       htdocs/compta/prelevement/invoices.php
 *     \ingroup    prelevement
...
if ($resql) {
	...
	if ($salaryBonPl) {
		...
		$invoicetmpcustomer = new Invoice($db);
		$invoicetmpsupplier = new InvoiceSupplier($db);
	}
	...
	while ($i < $imaxinloop) {
		...
		} elseif ($invoicetmpcustomer instanceof Invoice && $invoicetmpsupplier instanceof InvoiceSupplier) {
		...
		if ($object->type == 'bank-transfer' && !$salaryBonPl) {
			$labeltoshow = '';
			if ($invoicetmp instanceof Invoice) {
