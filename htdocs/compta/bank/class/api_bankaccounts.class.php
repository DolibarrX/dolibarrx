...
class BankAccounts extends DolibarrApi
{
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $category = 0, $sqlfilters = '', $properties = '')
	{
		...
		if ($category > 0) {
			$sql .= ", " . MAIN_DB_PREFIX . "category_account as c";
		}
		$sql .= ' WHERE t.entity IN (' . getEntity('bank_account') . ')';
		// Select accounts of given category
		if ($category > 0) {
			$sql .= " AND c.fk_category = " . ((int) $category) . " AND c.fk_account = t.rowid";
		}
		// Add sql filters
		if ($sqlfilters) {
			$errormessage = '';
	...
	{
		if (!DolibarrApiAccess::$user->hasRight('bank', 'configurer')) {
			throw new RestException(403);
		}
	...
	{
		if (!DolibarrApiAccess::$user->hasRight('bank', 'modifier')) {
			throw new RestException(403);
		}
	...
	{
		if (!DolibarrApiAccess::$user->rights->bank->modifier) {
			throw new RestException(403);
		}
