...
if (!$user->hasRight('bank', 'transfer')) {
	accessforbidden();
}
...
if ($action == 'add' && $user->hasRight('bank', 'transfer')) {
	$langs->load('errors');
	$i = 1;
	...
	while ($n < $MAXLINESFORTRANSFERT) {
		if ($tabnum[$n] === 1) {
			...
			if (!$error) {
				...
				if (!$error) {
					$mesg = $langs->trans("TransferFromToDone", '{s1}', '{s2}', $amount[$n], $langs->transnoentitiesnoconv("Currency".$config->currency));
