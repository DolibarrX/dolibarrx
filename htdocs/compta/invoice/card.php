...
/**
 * \file 	htdocs/compta/invoice/card.php
 * \ingroup invoice
...
// Permissions
$usercanread = $user->hasRight("invoice", "lire");
$usercancreate = $user->hasRight("invoice", "creer");
$usercanissuepayment = $user->hasRight("invoice", "paiement");
$usercandelete = $user->hasRight("invoice", "supprimer") || ($usercancreate && isset($object->status) && $object->status == $object::STATUS_DRAFT);
$usercancreatecontract = $user->hasRight("contract", "creer");

// Advanced Permissions
$usercanvalidate = ((!getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && $usercancreate) || (getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('invoice', 'invoice_advance', 'validate')));
$usercansend = (!getDolGlobalString('MAIN_USE_ADVANCED_PERMS') || (getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('invoice', 'invoice_advance', 'send')));
$usercanreopen = ((!getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && $usercancreate) || (getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('invoice', 'invoice_advance', 'reopen')));
if (getDolGlobalString('INVOICE_DISALLOW_REOPEN')) {
	$usercanreopen = false;
}
$usercanunvalidate = ((!getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && !empty($usercancreate)) || (getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('invoice', 'invoice_advance', 'unvalidate')));
...
$result = restrictedArea($user, 'invoice', $object->id, '', '', 'fk_soc', 'rowid', $isdraft);
...
if (empty($resHook)) {
	$backurlforlist = DOL_URL_ROOT.'/compta/invoice/list.php';
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = DOL_URL_ROOT.'/compta/invoice/card.php?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $permissionToAdd) {
		if (!($socid > 0)) {
			...
			'@phan-var-force Invoice $objectutil';
		...
		if ($object->status == Invoice::STATUS_CLOSED || ($object->status == Invoice::STATUS_ABANDONED && ($object->close_code != 'replaced' || $object->getIdReplacingInvoice() == 0)) || ($object->status == Invoice::STATUS_VALIDATED && $object->paye == 1)) {    // ($object->status == 1 && $object->paye == 1) should not happened but can be found when data are corrupted
		...
		if (($usercandelete && $isErasable > 0)
			...
			if ($result > 0) {
				header('Location: '.DOL_URL_ROOT.'/compta/invoice/list.php?restore_lastsearch_values=1');
				exit();
			} else {
				setEventMessages($object->error, $object->errors, 'errors');
				$action = '';
		...
		// Check for warehouse
		if ($object->type != Invoice::TYPE_DEPOSIT && getDolGlobalString('STOCK_CALCULATE_ON_BILL')) {
			$qualified_for_stock_change = 0;
			if (!getDolGlobalString('STOCK_SUPPORTS_SERVICES')) {
				$qualified_for_stock_change = $object->hasProductsOrServices(2);
			} else {
		...
		$canconvert = 0;
		if ($object->type == Invoice::TYPE_DEPOSIT && empty($discountcheck->id)) {
		...
		if (($object->type == Invoice::TYPE_CREDIT_NOTE || $object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_SITUATION) && $object->paye == 0 && empty($discountcheck->id)) {
		...
		if ($canconvert) {
			...
			if (getDolGlobalString('INVOICE_ALLOW_REUSE_OF_CREDIT_WHEN_PARTIALLY_REFUNDED') && $object->type == Invoice::TYPE_CREDIT_NOTE) {
			...
			} elseif ($object->type == Invoice::TYPE_DEPOSIT) {
				$discount->description = '(DEPOSIT)';
			} elseif ($object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_REPLACEMENT || $object->type == Invoice::TYPE_SITUATION) {
			...
			$discount->fk_invoice_source = $object->id;

			$error = 0;

			if ($object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_REPLACEMENT || $object->type == Invoice::TYPE_SITUATION) {
				// If we're on a standard invoice, we have to get excess received to create a discount in TTC without VAT

				// Total payments
				$sql = 'SELECT SUM(pf.amount) as total_paiements';
				$sql .= ' FROM '.MAIN_DB_PREFIX.'paiement_invoice as pf, '.MAIN_DB_PREFIX.'paiement as p';
			...
			if ($object->type == Invoice::TYPE_CREDIT_NOTE || $object->type == Invoice::TYPE_DEPOSIT) {
			...
			if (empty($error)) {
				if ($object->type != Invoice::TYPE_DEPOSIT) {
					// Set invoice as paid
					$result = $object->setPaid($user);
					if ($result >= 0) {
						$db->commit();
		...
		// Delete payment
		$object->fetch($id);
		if ($object->status == Invoice::STATUS_VALIDATED && $object->paye == 0) {
		...
		// Replacement invoice
		if (GETPOST('type') == Invoice::TYPE_REPLACEMENT) {
			...
			if (!$error) {
				...
				$object->fk_invoice_source = GETPOSTINT('fac_replacement');
				$object->type              = Invoice::TYPE_REPLACEMENT;
		...
		if (GETPOST('type') == Invoice::TYPE_CREDIT_NOTE) {
			...
			if (!$error) {
				...
				// Special properties of replacement invoice
				$object->fk_invoice_source = $sourceinvoice > 0 ? $sourceinvoice : '';
				$object->type = Invoice::TYPE_CREDIT_NOTE;

				$invoice_source = new Invoice($db); // fetch origin object
				if ($invoice_source->fetch($object->fk_invoice_source) > 0) {
					if ($invoice_source->type == Invoice::TYPE_SITUATION) {
						$object->situation_counter = $invoice_source->situation_counter;
						$object->situation_cycle_ref = $invoice_source->situation_cycle_ref;
						$invoice_source->fetchPreviousNextSituationInvoice();
					}
				}
				...
				if ($id < 0) {
					...
					if ($object->copy_linked_contact($invoice_source, 'internal') < 0) {
						$error++;
					} elseif ($invoice_source->socid == $object->socid) {
						// copy external contacts if same company
						if ($object->copy_linked_contact($invoice_source, 'external') < 0) {
							$error++;
						}
					}
				}
				...
				if (GETPOSTINT('invoiceAvoirWithLines') == 1 && $id > 0) {
					if (!empty($invoice_source->lines)) {
						$fk_parent_line = 0;

						foreach ($invoice_source->lines as $line) {
							// Extrafields
							...
							if ($invoice_source->type == Invoice::TYPE_SITUATION) {
								...
								if (!empty($invoice_source->tab_previous_situation_invoice)) {
									// search the last standard invoice in cycle and the possible credit note between this last and invoice_source
									// TODO Move this out of loop of $invoice_source->lines
									$tab_jumped_credit_notes = [];
									$lineIndex = count($invoice_source->tab_previous_situation_invoice) - 1;
									$searchPreviousInvoice = true;
									while ($searchPreviousInvoice) {
										if ($invoice_source->tab_previous_situation_invoice[$lineIndex]->type == Invoice::TYPE_SITUATION || $lineIndex < 1) {
											$searchPreviousInvoice = false; // find, exit;
											break;
										} else {
											if ($invoice_source->tab_previous_situation_invoice[$lineIndex]->type == Invoice::TYPE_CREDIT_NOTE) {
												$tab_jumped_credit_notes[$lineIndex] = $invoice_source->tab_previous_situation_invoice[$lineIndex]->id;
											}
									...
									foreach ($invoice_source->tab_previous_situation_invoice[$lineIndex]->lines as $prevLine) {
									...
									//print 'New line based on invoice id '.$invoice_source->tab_previous_situation_invoice[$lineIndex]->id.' fk_prev_id='.$source_fk_prev_id.' will be fk_prev_id='.$line->fk_prev_id.' '.$line->total_ht.' '.$line->situation_percent.'<br>';
									...
									foreach ($tab_jumped_credit_notes as $index => $creditnoteid) {
										foreach ($invoice_source->tab_previous_situation_invoice[$index]->lines as $prevLine) {
				...
				if (GETPOSTINT('invoiceAvoirWithPaymentRestAmount') == 1 && $id > 0) {
					if ($invoice_source->fetch($object->fk_invoice_source) > 0) {
						$totalpaid = $invoice_source->getSommePaiement();
						$totalcreditnotes = $invoice_source->getSumCreditNotesUsed();
						$totaldeposits = $invoice_source->getSumDepositsUsed();
						$remain_to_pay = abs($invoice_source->total_ttc - $totalpaid - $totalcreditnotes - $totaldeposits);
				...
				if (!empty($object->fk_invoice_source) && $id > 0) {
					$invoice_source->fetch($object->fk_invoice_source);
					$invoice_source->fetchObjectLinked();

					if (!empty($invoice_source->linkedObjectsIds)) {
						foreach ($invoice_source->linkedObjectsIds as $sourcetype => $TIds) {
		...
		if ((GETPOST('type') == Invoice::TYPE_STANDARD || GETPOST('type') == Invoice::TYPE_DEPOSIT) && GETPOSTINT('fac_rec') > 0) {
			if (empty($dateinvoice)) {
				$error++;
				setEventMessages($langs->trans("ErrorFieldRequired", $langs->transnoentitiesnoconv("Date")), null, 'errors');
				$action = 'create';
		...
		if ((GETPOST('type') == Invoice::TYPE_STANDARD || GETPOST('type') == Invoice::TYPE_DEPOSIT || GETPOST('type') == Invoice::TYPE_PROFORMA || (GETPOST('type') == Invoice::TYPE_SITUATION && !GETPOST('situations'))) && GETPOST('fac_rec') <= 0) {
			...
			if (!$error) {
				...
				if (GETPOST('type') == Invoice::TYPE_SITUATION) {
				...
				if (!empty($origin) && !empty($originid)) {
					...
					if ($id > 0) {
						...
						if (GETPOST('type') == Invoice::TYPE_DEPOSIT && in_array($typeamount, array('amount', 'variable'))) {
							...
							foreach ($amountdeposit as $tva => $amount) {
								...
								if ($typeamount == 'amount') {
									$descline .= ' ('.price($valuedeposit, 0, $langs, 0, - 1, - 1, (!empty($object->multicurrency_code) ? $object->multicurrency_code : $config->currency)).')';
								} elseif ($typeamount == 'variable') {
								...
								$result = $object->addline(
									...
									0,
									'',
									(!empty($config->global->MAIN_DEPOSIT_MULTI_TVA) ? 0 : 1)
								);
							}
						...
						if (GETPOST('type') != Invoice::TYPE_DEPOSIT || (GETPOST('type') == Invoice::TYPE_DEPOSIT && $typeamount == 'variablealllines')) {
							$lines = [];
							...
							if ($result > 0) {
								...
								if (GETPOST('type') == Invoice::TYPE_STANDARD && $valuestandardinvoice > 0 && $valuestandardinvoice < 100) {
									if (is_array($lines)) {
								...
								if (GETPOST('type') == Invoice::TYPE_DEPOSIT && $typeamount == 'variablealllines') {
									if (is_array($lines)) {
										foreach ($lines as $line) {
											// We keep ->subprice and ->pa_ht, but we change the qty
		...
		// Situation invoices
		if (GETPOST('type') == Invoice::TYPE_SITUATION && GETPOST('situations')) {
			if (empty($dateinvoice)) {
				$error++;
			...
			if (!$error) {
				...
				$object->fk_invoice_source = GETPOSTINT('situations');
				$object->type = Invoice::TYPE_SITUATION;
				...
				if ($id <= 0) {
					...
					$nextSituationInvoice = new Invoice($db);
		...
		if (!$error && ($qty >= 0) && (!empty($product_desc) || (!empty($idprod) && $idprod > 0))) {
			...
			if ($usermustrespectpricemin && ($object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_REPLACEMENT)) {
		...
		if ($object->type == Invoice::TYPE_CREDIT_NOTE && $object->situation_cycle_ref > 0) {
		...
		if (empty($productid) && (($pu_ht < 0 && !getDolGlobalString('FACTURE_ENABLE_NEGATIVE_LINES')) || $pu_ht == '') && (($pu_ht_devise < 0 && !getDolGlobalString('FACTURE_ENABLE_NEGATIVE_LINES')) || $pu_ht_devise == '') && $pu_ttc === '' && $pu_ttc_devise === '' && $object->type != Invoice::TYPE_CREDIT_NOTE) { 	// Unit price can be 0 but not ''
		...
		// Invoice situation
		if (getDolGlobalInt('INVOICE_USE_SITUATION') == 2) {
			$previousprogress = $line->getAllPrevProgress($line->fk_invoice);
		...
		if (in_array($object->status, array(Invoice::STATUS_CLOSED, Invoice::STATUS_VALIDATED))
			&& $object->type == Invoice::TYPE_SITUATION
			&& $usercancreate
			&& !$objectidnext
			...
			if ($newCycle > 1) {
				...
				if (count($object->tab_next_situation_invoice) > 0) {
					foreach ($object->tab_next_situation_invoice as $next_invoice) {
						if ($next_invoice->type == Invoice::TYPE_CREDIT_NOTE
							&& $next_invoice->situation_counter == $object->situation_counter
							&& $next_invoice->fk_invoice_source == $object->id
						) {
					...
					if (count($linkedCreditNotesList) > 0) {
						...
						foreach ($object->lines as $line) {
							...
							if (!empty($object->tab_previous_situation_invoice)) {
								...
								$searchPreviousInvoice = true;
								while ($searchPreviousInvoice) {
									if ($object->tab_previous_situation_invoice[$lineIndex]->type == Invoice::TYPE_SITUATION || $lineIndex < 1) {
										$searchPreviousInvoice = false; // find, exit;
										break;
									} else {
										$lineIndex--; // go to previous invoice in cycle
	...
	} elseif ($action == 'import_lines_from_object' && $usercancreate && $object->status == Invoice::STATUS_DRAFT
		&& ($object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_REPLACEMENT || $object->type == Invoice::TYPE_DEPOSIT || $object->type == Invoice::TYPE_PROFORMA || $object->type == Invoice::TYPE_SITUATION)) {
		...
		if (!empty($importLines) && is_array($importLines) && !empty($fromElement) && ctype_alpha($fromElement) && !empty($fromElementid)) {
			...
			foreach ($importLines as $lineId) {
				if ($lineClassName === '') {
					...
					if ($error === 0) {
						// Log only once
						dol_syslog('compta/invoice/card - No lineClassName - skip import', LOG_ERR);
					}
				...
				if (intval($fromElementid) > 0 && $originLine->fetch($lineId) > 0) {
					...
					if ($object->type == Invoice::TYPE_SITUATION) {
						$situation_percent = 0;
					} else {
...
if ($action == 'create') {
	...
	$extrafields->fetch_name_optionals_label($invoicestatic->table_element);
	...
	if (!empty($origin) && !empty($originid)) {
		...
		if ($element == 'project') {
			...
			if ($element == 'expedition') {
				...
				dol_syslog("Is type Invoice|Order or Expedition: $element...expesrc($classname)=".get_class($expesrc));
	...
	if (empty($resHook)) {
		...
		$exampletemplateinvoice = new InvoiceRec($db);
		$invoice_predefined = new InvoiceRec($db);
		...
		if ($soc->id > 0 && (!GETPOSTINT('fac_rec') || !empty($invoice_predefined->frequency))) {
			...
			print '<span class="amount">'.price($outstandingBills, 0, $langs, 0, 0, -1, $config->currency).'</span>';
		...
		if (empty($origin) && empty($originid) && GETPOSTINT('fac_rec') > 0) {
			...
			$sql .= ' FROM '.MAIN_DB_PREFIX.'invoice_rec as r';
		...
		if ($socid > 0) {
			if (getDolGlobalString('INVOICE_USE_SITUATION')) {
				...
				if ($opt == ('<option value ="0" selected>'.$langs->trans('NoSituations').'</option>') || (GETPOST('origin') && GETPOST('origin') != 'invoice' && GETPOST('origin') != 'order')) {
					$tmp .= ' disabled';
				}
				$tmp .= '> ';
			...
			if (!getDolGlobalString('INVOICE_DISABLE_REPLACEMENT')) {
				// Invoice type
				$facids = $invoicestatic->list_replacable_invoices($soc->id);
				if ($facids < 0) {
					dol_print_error($db, $invoicestatic->error, $invoicestatic->errors);
					exit();
				}
				$options = "";
				if (is_array($facids)) {
					foreach ($facids as $facparam) {
						...
						$options .= ' ('.$invoicestatic->LibStatut($facparam['paid'], $facparam['status'], 0, $facparam['alreadypaid']).')';
						$options .= '</option>';
					}
				}
		...
		if (empty($origin)) {
			if ($socid > 0) {
				...
				if (!getDolGlobalString('INVOICE_DISABLE_CREDIT_NOTE')) {
					...
					$facids = $invoicestatic->list_qualified_avoir_invoices($soc->id);
					if ($facids < 0) {
						dol_print_error($db, $invoicestatic->error, $invoicestatic->errors);
						exit;
					}
					$optionsav = "";
					$newinvoice_static = new Invoice($db);
		...
		if (getDolGlobalString('INVOICE_USE_DEFAULT_DOCUMENT')) { // Hidden conf
			...
			$listtType = array(Invoice::TYPE_STANDARD, Invoice::TYPE_REPLACEMENT, Invoice::TYPE_CREDIT_NOTE, Invoice::TYPE_DEPOSIT, Invoice::TYPE_SITUATION);
			$jsListType = '';
		...
		if (getDolGlobalString('INVOICE_USE_RETAINED_WARRANTY')) {
			...
			if (empty($retained_warranty)) {
				if ($objectsrc !== null && property_exists($objectsrc, 'retained_warranty') && !empty($objectsrc->retained_warranty)) { // use previous situation value
					// Invoice->retained_warranty  (does not exist on Expedition)
		...
		$liste = ModelePDFInvoices::liste_modeles($db);
		...
		if (!empty($origin) && !empty($originid) && is_object($objectsrc)) {
			...
			$objectsrc->fetchObjectLinked($originid, $origin, null, 'invoice');
			if (isset($objectsrc->linkedObjects['invoice']) && is_array($objectsrc->linkedObjects['invoice']) && count($objectsrc->linkedObjects['invoice']) >= 1) {
				setEventMessages('WarningBillExist', null, 'warnings');
				echo ' - '.$langs->trans('LatestRelatedBill').' '.end($objectsrc->linkedObjects['invoice'])->getNomUrl(1);
			}
			echo '</td></tr>';
	...
	if (isModEnabled('multicurrency')) {
		...
		if ($resteapayer == 0 && $multicurrency_resteapayer != 0 && $object->multicurrency_code != $config->currency) {
	...
	$head = invoice_prepare_head($object);
	...
	if ($action == 'converttoreduc') {
		if ($object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_SITUATION) {
			$type_fac = 'ExcessReceived';
		} elseif ($object->type == Invoice::TYPE_CREDIT_NOTE) {
			$type_fac = 'CreditNote';
		} elseif ($object->type == Invoice::TYPE_DEPOSIT) {
			$type_fac = 'Deposit';
		} else {
	...
	if ($action == 'delete') {
		...
		if ($object->type != Invoice::TYPE_DEPOSIT && getDolGlobalString('STOCK_CALCULATE_ON_BILL') && $object->status >= 1) {
			$qualified_for_stock_change = 0;
			if (!getDolGlobalString('STOCK_SUPPORTS_SERVICES')) {
				$qualified_for_stock_change = $object->hasProductsOrServices(2);
			} else {
			...
			if ($qualified_for_stock_change) {
				...
				$label = $object->type == Invoice::TYPE_CREDIT_NOTE ? $langs->trans("SelectWarehouseForStockDecrease") : $langs->trans("SelectWarehouseForStockIncrease");
				$forcecombo = 0;
	...
	if ($action == 'valid') {
		...
		if ($object->type != Invoice::TYPE_DEPOSIT && getDolGlobalString('STOCK_CALCULATE_ON_BILL')) {
			...
			if ($qualified_for_stock_change) {
				...
				if (count($warehouse_array) == 1) {
					$label = $object->type == Invoice::TYPE_CREDIT_NOTE ? $langs->trans("WarehouseForStockIncrease", current($warehouse_array)) : $langs->trans("WarehouseForStockDecrease", current($warehouse_array));
					$value = '<input type="hidden" id="idwarehouse" name="idwarehouse" value="'.key($warehouse_array).'">';
				} else {
					$label = $object->type == Invoice::TYPE_CREDIT_NOTE ? $langs->trans("SelectWarehouseForStockIncrease") : $langs->trans("SelectWarehouseForStockDecrease");
		...
		if ($object->type != Invoice::TYPE_CREDIT_NOTE && $object->total_ttc < 0) { 		// Can happen only if getDolGlobalString('FACTURE_ENABLE_NEGATIVE') is on
		...
		if (!$error) {
			$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?facid='.$object->id, $langs->trans('ValidateBill'), $text, 'confirm_valid', $formquestion, (($object->type != Invoice::TYPE_CREDIT_NOTE && $object->total_ttc < 0) ? "no" : "yes"), 2, 240);
		}
	}
	...
	if ($action == 'modif') {
		...
		if ($object->type != Invoice::TYPE_DEPOSIT && getDolGlobalString('STOCK_CALCULATE_ON_BILL')) {
			...
			if ($qualified_for_stock_change) {
				...
				$warehouse_array = $warehouse->list_[];
				if (count($warehouse_array) == 1) {
					$label = $object->type == Invoice::TYPE_CREDIT_NOTE ? $langs->trans("WarehouseForStockDecrease", current($warehouse_array)) : $langs->trans("WarehouseForStockIncrease", current($warehouse_array));
					$value = '<input type="hidden" id="idwarehouse" name="idwarehouse" value="'.key($warehouse_array).'">';
				} else {
					$label = $object->type == Invoice::TYPE_CREDIT_NOTE ? $langs->trans("SelectWarehouseForStockDecrease") : $langs->trans("SelectWarehouseForStockIncrease");
					$value = $formproduct->selectWarehouses(GETPOST('idwarehouse') ? GETPOST('idwarehouse') : 'ifone', 'idwarehouse', '', 1);
	...
	if ($action == 'canceled') {
		...
		if ($objectidnext) {
			$invoicereplacement = new Invoice($db);
			$invoicereplacement->fetch($objectidnext);
			$statusreplacement = $invoicereplacement->status;
		}
	...
	$linkback = '<a href="'.DOL_URL_ROOT.'/compta/invoice/list.php?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	if (!getDolGlobalString('MAIN_DISABLE_OTHER_LINK') && $object->thirdparty->id > 0) {
		$morehtmlref .= ' (<a href="'.DOL_URL_ROOT.'/compta/invoice/list.php?socid='.$object->thirdparty->id.'&search_societe='.urlencode($object->thirdparty->name).'">'.$langs->trans("OtherBills").'</a>)';
	}
	// Project
	if (isModEnabled('project')) {
		$langs->load("projects");
	...
	if (empty($resHook)) {
		...
		if ($object->subtype > 0) {
			print ' '.$object->getSubtypeLabel('invoice');
		}
		...
		if ($object->type == Invoice::TYPE_REPLACEMENT) {
			$facreplaced = new Invoice($db);
			$facreplaced->fetch($object->fk_invoice_source);
			print ' <span class="opacitymediumbycolor paddingleft">'.$langs->transnoentities("ReplaceInvoice", $facreplaced->getNomUrl(1, '', 32)).'</span>';
		}
		if ($object->type == Invoice::TYPE_CREDIT_NOTE && !empty($object->fk_invoice_source)) {
			$facusing = new Invoice($db);
			$facusing->fetch($object->fk_invoice_source);
		...
		print '</td>';
		if ($object->type != Invoice::TYPE_CREDIT_NOTE && $action != 'editconditions' && $usercancreate) {
		...
		print '</td>';
		if ($object->type != Invoice::TYPE_CREDIT_NOTE && $action != 'editpaymentterm' && $usercancreate) {
		...
		if (isModEnabled('incoterm')) {
			...
			if ($usercancreate) {
				print '<a class="editfielda" href="'.DOL_URL_ROOT.'/compta/invoice/card.php?facid='.$object->id.'&action=editincoterm&token='.newToken().'">'.img_edit().'</a>';
			} else {
				print '&nbsp;';
			}
			print '</td></tr></table>';
		...
		if (!empty($object->retained_warranty) || getDolGlobalString('INVOICE_USE_RETAINED_WARRANTY')) {
			...
			if ($displayWarranty) {
				...
				print '</td>';
				if ($action != 'editretainedwarranty' && $user->hasRight('invoice', 'creer') && $object->status == Invoice::STATUS_DRAFT) {
				...
				print '</tr></table>';
				print '</td><td>';
				if ($action == 'editretainedwarranty' && $object->status == Invoice::STATUS_DRAFT) {
				...
				print '</td>';
				if ($action != 'editretainedwarrantypaymentterms' && $user->hasRight('invoice', 'creer') && $object->status == Invoice::STATUS_DRAFT) {
				...
				if ($action == 'editretainedwarrantypaymentterms' && $object->status == Invoice::STATUS_DRAFT) {
				...
				print '</td>';
				if ($action != 'editretainedwarrantydatelimit' && $user->hasRight('invoice', 'creer') && $object->status == Invoice::STATUS_DRAFT) {
				...
				if ($action == 'editretainedwarrantydatelimit' && $object->status == Invoice::STATUS_DRAFT) {
					//date('Y-m-d',$object->date_lim_reglement)
					print '<form method="POST" action="'.$_SERVER['PHP_SELF'].'?facid='.$object->id.'">';
		...
		print '<td class="nowrap amountcard right">' . price($sign * $object->total_ht, 0, $langs, 0, -1, -1, $config->currency) . '</td>';
		...
		print '<td class="nowrap amountcard right">' . price($sign * $object->total_tva, 0, $langs, 0, -1, -1, $config->currency) . '</td>';
		if (isModEnabled("multicurrency") && ($object->multicurrency_code && $object->multicurrency_code != $config->currency)) {
		...
		if (($mysoc->localtax1_assuj == "1" && $mysoc->useLocalTax(1)) || $object->total_localtax1 != 0) {
			...
			print '<td class="nowrap amountcard right">' . price($sign * $object->total_localtax1, 0, $langs, 0, -1, -1, $config->currency) . '</td>';
			if (isModEnabled("multicurrency") && ($object->multicurrency_code && $object->multicurrency_code != $config->currency)) {
		...
		if (($mysoc->localtax2_assuj == "1" && $mysoc->useLocalTax(2)) || $object->total_localtax2 != 0) {
			...
			print '<td class="nowrap amountcard right">' . price($sign * $object->total_localtax2, 0, $langs, 0, -1, -1, $config->currency) . '</td>';
			if (isModEnabled("multicurrency") && ($object->multicurrency_code && $object->multicurrency_code != $config->currency)) {
		...
		if ($selleruserevenustamp) {
			...
			if ($action == 'editrevenuestamp') {
				...
				print price($object->revenuestamp, 1, '', 1, -1, -1, $config->currency);
			}
			print '</td></tr>';
		}
		...
		print '<td class="nowrap amountcard right">' . price($sign * $object->total_ttc, 0, $langs, 0, -1, -1, $config->currency) . '</td>';
		if (isModEnabled("multicurrency") && ($object->multicurrency_code && $object->multicurrency_code != $config->currency)) {
		...
		if (($object->situation_cycle_ref > 0) && getDolGlobalString('INVOICE_USE_SITUATION')) {
			...
			if (count($object->tab_previous_situation_invoice) > 0) {
				...
				foreach ($object->tab_previous_situation_invoice as $prev_invoice) {
					...
					$current_situation_counter[] = (($prev_invoice->type == Invoice::TYPE_CREDIT_NOTE) ? -1 : 1) * $prev_invoice->situation_counter;
					print '<tr class="oddeven">';
					print '<td>'.$prev_invoice->getNomUrl(1).'</td>';
					print '<td></td>';
					print '<td align="center" >'.(($prev_invoice->type == Invoice::TYPE_CREDIT_NOTE) ? $langs->trans('situationInvoiceShortcode_AS') : $langs->trans('situationInvoiceShortcode_S')).$prev_invoice->situation_counter.'</td>';
					if (isModEnabled("bank")) {
						print '<td class="right"></td>';
					}
			...
			$current_situation_counter[] = (($object->type == Invoice::TYPE_CREDIT_NOTE) ? -1 : 1) * $object->situation_counter;

			print '<tr class="oddeven">';
			print '<td>'.$object->getNomUrl(1).'</td>';
			print '<td></td>';
			print '<td class="center">'.(($object->type == Invoice::TYPE_CREDIT_NOTE) ? $langs->trans('situationInvoiceShortcode_AS') : $langs->trans('situationInvoiceShortcode_S')).$object->situation_counter.'</td>';
			if (isModEnabled("bank")) {
				print '<td class="right"></td>';
			}
			...
			if (count($object->tab_next_situation_invoice) > 0) {
				...
				foreach ($object->tab_next_situation_invoice as $next_invoice) {
					...
					print '<td></td>';
					print '<td class="center">'.(($next_invoice->type == Invoice::TYPE_CREDIT_NOTE) ? $langs->trans('situationInvoiceShortcode_AS') : $langs->trans('situationInvoiceShortcode_S')).$next_invoice->situation_counter.'</td>';
					if (isModEnabled("bank")) {
						print '<td class="right"></td>';
					}
		...
		print '<tr class="liste_titre">';
		print '<td class="liste_titre">'.($object->type == Invoice::TYPE_CREDIT_NOTE ? $langs->trans("PaymentsBack") : $langs->trans('Payments')).'</td>';
		...
		$sql .= ' WHERE pf.fk_invoice = '.((int) $object->id).' AND pf.fk_paiement = p.rowid';
		$sql .= ' AND p.entity IN ('.getEntity('invoice').')';
		...
		if ($result) {
			...
			if ($num > 0) {
				while ($i < $num) {
					...
					if ($object->status == Invoice::STATUS_VALIDATED && $object->paye == 0 && $user->socid == 0 && !$paiement->isReconciled()) {
		...
		if ($object->type != Invoice::TYPE_CREDIT_NOTE) {
			...
			if ($resql) {
				...
				while ($i < $num) {
					...
					$invoice->fetch($obj->fk_invoice_source);
					print '<tr><td colspan="'.$nbcols.'" class="right">';
					print '<span class="opacitymedium">';
			...
			if (($object->status == Invoice::STATUS_CLOSED || $object->status == Invoice::STATUS_ABANDONED) && $object->close_code == 'discount_vat') {
			...
			if (($object->status == Invoice::STATUS_CLOSED || $object->status == Invoice::STATUS_ABANDONED) && $object->close_code == 'badcustomer') {
				print '<tr><td colspan="'.$nbcols.'" class="nowrap right">';
				print '<span class="opacitymedium">';
			...
			if (($object->status == Invoice::STATUS_CLOSED || $object->status == Invoice::STATUS_ABANDONED) && $object->close_code == 'product_returned') {
				print '<tr><td colspan="'.$nbcols.'" class="nowrap right">';
				print '<span class="opacitymedium">';
			...
			if (($object->status == Invoice::STATUS_CLOSED || $object->status == Invoice::STATUS_ABANDONED) && $object->close_code == 'abandon') {
				print '<tr><td colspan="'.$nbcols.'" class="nowrap right">';
			...
			if ($object->multicurrency_code != $config->currency || $object->multicurrency_tx != 1) {
				print '<tr><td colspan="'.$nbcols.'" class="right">';
				print '<span class="opacitymedium">';
				...
				//print (empty($object->multicurrency_code) ? $config->currency : $object->multicurrency_code).' ';
				print price(price2num($object->multicurrency_tx * $resteapayeraffiche, 'MT'), 1, $langs, 1, -1, -1, (empty($object->multicurrency_code) ? $config->currency : $object->multicurrency_code)).'</td><td>&nbsp;</td></tr>';
			}
			...
			if ($object->multicurrency_code != $config->currency || $object->multicurrency_tx != 1) {
				...
				print '</span>';
				print '</td>';
				print '<td class="right'.($resteapayeraffiche ? ' amountremaintopayback' : (' '.$cssforamountpaymentcomplete)).'">'.(!empty($object->multicurrency_code) ? $object->multicurrency_code : $config->currency).' '.price(price2num($sign * $object->multicurrency_tx * $resteapayeraffiche, 'MT')).'</td><td>&nbsp;</td></tr>';
			}
	...
	if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline') {
		...
		if (empty($resHook)) {
			...
			if ($object->status == Invoice::STATUS_VALIDATED) {
			...
			// Reopen an invoice
			if ((($object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_REPLACEMENT)
				|| ($object->type == Invoice::TYPE_CREDIT_NOTE && empty($discount->id))
				|| ($object->type == Invoice::TYPE_DEPOSIT && empty($discount->id))
				|| ($object->type == Invoice::TYPE_SITUATION && empty($discount->id)))
				&& ($object->status == Invoice::STATUS_CLOSED || $object->status == Invoice::STATUS_ABANDONED || ($object->status == 1 && $object->paye == 1))   // Condition ($object->status == 1 && $object->paye == 1) should not happened but can be found due to corrupted data
			...
			if (getDolGlobalString('CONTRACT_CREATE_FROM_INVOICE')) {
				if (isModEnabled('contract') && $object->status == Invoice::STATUS_VALIDATED) {
			...
			// Validate
			if ($object->status == Invoice::STATUS_DRAFT && count($object->lines) > 0
				&& ((($object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_REPLACEMENT || $object->type == Invoice::TYPE_DEPOSIT || $object->type == Invoice::TYPE_PROFORMA || $object->type == Invoice::TYPE_SITUATION) && (getDolGlobalString('FACTURE_ENABLE_NEGATIVE') || $object->total_ttc >= 0))
					|| ($object->type == Invoice::TYPE_CREDIT_NOTE && $object->total_ttc <= 0))) {
				if ($usercanvalidate) {
					unset($params['attr']['title']);
			...
			// Send by mail
			if (empty($user->socid)) {
				if (($object->status == Invoice::STATUS_VALIDATED || $object->status == Invoice::STATUS_CLOSED) || getDolGlobalString('FACTURE_SENDBYEMAIL_FOR_ALL_STATUS')) {
					if ($objectidnext) {
			...
			if ($object->status > Invoice::STATUS_DRAFT && $object->paye == 0 && $num == 0) {
				if ($resteapayer > 0) {
					if ($usercancreatewithdrarequest) {
						if (!$objectidnext && $object->close_code != 'replaced') { 				// Not replaced by another invoice
							print '<a class="butAction" href="'.DOL_URL_ROOT.'/compta/invoice/prelevement.php?facid='.$object->id.'" title="'.dol_escape_htmltag($langs->trans("MakeWithdrawRequest")).'">'.$langs->trans("MakeWithdrawRequest").'</a>';
						} else {
			...
			// Create payment
			if ($object->type != Invoice::TYPE_CREDIT_NOTE && $object->status == 1 && $object->paye == 0 && $usercanissuepayment) {
				if ($objectidnext) {
					...
					if ($object->type == Invoice::TYPE_DEPOSIT && $resteapayer == 0) {
			...
			if ($object->type == Invoice::TYPE_CREDIT_NOTE || $object->type == Invoice::TYPE_DEPOSIT || $object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_SITUATION) {
				// For credit note only
				if ($object->type == Invoice::TYPE_CREDIT_NOTE && $object->status == Invoice::STATUS_VALIDATED && $object->paye == 0 && $usercanissuepayment) {
					if ($resteapayer == 0) {
				...
				if (($object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_SITUATION) && $object->status == Invoice::STATUS_VALIDATED && empty($object->paye) && $resteapayer < 0 && $usercancreate && empty($discount->id)) {
					print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER["PHP_SELF"].'?facid='.$object->id.'&action=converttoreduc&token='.newToken().'">'.$langs->trans('ConvertExcessReceivedToReduc').'</a>';
				}
				// For credit note
				if ($object->type == Invoice::TYPE_CREDIT_NOTE && $object->status == Invoice::STATUS_VALIDATED && $object->paye == 0 && $usercancreate
					&& (getDolGlobalString('INVOICE_ALLOW_REUSE_OF_CREDIT_WHEN_PARTIALLY_REFUNDED') || $sumofpayment == 0) && $object->total_ht < 0
				) {
					print '<a class="butAction classfortooltip'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER["PHP_SELF"].'?facid='.$object->id.'&action=converttoreduc&token='.newToken().'" title="'.dol_escape_htmltag($langs->trans("ConfirmConvertToReduc2")).'">'.$langs->trans('ConvertToReduc').'</a>';
				}
				// For down payment invoice (deposit)

				if ($object->type == Invoice::TYPE_DEPOSIT && $usercancreate && $object->status > Invoice::STATUS_DRAFT && empty($discount->id)) {
					...
					if (price2num($object->total_ttc, 'MT') == price2num($sumofpaymentall, 'MT') || getDolGlobalInt('DEPOSIT_AS_CREDIT_AVAILABLE_EVEN_UNPAID') || ($object->type == Invoice::STATUS_ABANDONED && in_array($object->close_code, array('bankcharge', 'discount_vat', 'other')))) {
						print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER["PHP_SELF"].'?facid='.$object->id.'&action=converttoreduc&token='.newToken().'">'.$langs->trans('ConvertToReduc').'</a>';
					} else {
			...
			// Classify paid
			if ($object->status == Invoice::STATUS_VALIDATED && $object->paye == 0 && $usercanissuepayment && (
				($object->type != Invoice::TYPE_CREDIT_NOTE && $object->type != Invoice::TYPE_DEPOSIT && ($resteapayer <= 0 || (getDolGlobalString('INVOICE_CAN_SET_PAID_EVEN_IF_PARTIALLY_PAID') && $object->total_ttc == $resteapayer))) ||
				($object->type == Invoice::TYPE_CREDIT_NOTE && $resteapayer >= 0) ||
				($object->type == Invoice::TYPE_DEPOSIT && $object->total_ttc > 0)
			)
			) {
				if ($object->type == Invoice::TYPE_DEPOSIT && price2num($object->total_ttc, 'MT') != price2num($sumofpaymentall, 'MT')) {
			...
			if ($object->status == Invoice::STATUS_VALIDATED && $object->paye == 0 && $resteapayer > 0 && (!getDolGlobalString('INVOICE_CAN_SET_PAID_EVEN_IF_PARTIALLY_PAID') || $resteapayer != $object->total_ttc) && $usercanissuepayment) {
				if ($totalpaid > 0 || $totalcreditnotes > 0) {
					// If one payment or one credit note was linked to this invoice
					print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER['PHP_SELF'].'?facid='.$object->id.'&action=paid&token='.newToken().'">'.$langs->trans('ClassifyPaidPartially').'</a>';
				} else {
					if (!getDolGlobalString('INVOICE_CAN_NEVER_BE_CANCELED')) {
						if ($objectidnext) {
							...
							print '<a class="butAction'.($config->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER['PHP_SELF'].'?facid='.$object->id.'&action=canceled">'.$langs->trans('ClassifyCanceled').'</a>';
						}
					}
				}
			}
			...
			if (($object->type == Invoice::TYPE_STANDARD || ($object->type == Invoice::TYPE_DEPOSIT && !getDolGlobalString('FACTURE_DEPOSITS_ARE_JUST_PAYMENTS')) || $object->type == Invoice::TYPE_PROFORMA) && $object->status > 0 && $usercancreate) {
				if (!$objectidnext) {
			...
			// Clone
			if (($object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_DEPOSIT || $object->type == Invoice::TYPE_PROFORMA) && $usercancreate) {
				unset($params['attr']['title']);
			...
			if (($object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_DEPOSIT || $object->type == Invoice::TYPE_PROFORMA) && $object->status == 0 && $usercancreate) {
				if (!$objectidnext && count($object->lines) > 0) {
					unset($params['attr']['title']);
					print dolGetButtonAction($langs->trans('ChangeIntoRepeatableInvoice'), '', 'default', DOL_URL_ROOT.'/compta/invoice/card-rec.php?facid='.$object->id.'&action=create', '', true, $params);
				}
			}
			...
			if ($usercancreate && ($object->type == 5) && ($object->status == 1 || $object->status == 2)) {
				if ($object->is_last_in_cycle() && $object->situation_final != 1) {
					print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?action=create&type=5&origin=invoice&originid='.$object->id.'&socid='.$object->socid.'" >'.$langs->trans('CreateNextSituationInvoice').'</a>';
				} elseif (!$object->is_last_in_cycle()) {
	...
	if ($action != 'prerelance' && $action != 'presend') {
		...
		$filedir = $config->invoice->multidir_output[$object->entity].'/'.dol_sanitizeFileName($object->ref);
		...
		if ($usercancreate
			&& $object->status == Invoice::STATUS_DRAFT
			&& ($object->type == Invoice::TYPE_STANDARD || $object->type == Invoice::TYPE_REPLACEMENT || $object->type == Invoice::TYPE_DEPOSIT || $object->type == Invoice::TYPE_PROFORMA || $object->type == Invoice::TYPE_SITUATION)) {
		...
		if ($object->status != Invoice::STATUS_DRAFT && $useonlinepayment) {
		...
		$morehtmlcenter .= dolGetButtonTitle($langs->trans('FullConversation'), '', 'fa fa-comments imgforviewmode', DOL_URL_ROOT.'/compta/invoice/messaging.php?id='.$object->id);
		$morehtmlcenter .= dolGetButtonTitle($langs->trans('FullList'), '', 'fa fa-bars imgforviewmode', DOL_URL_ROOT.'/compta/invoice/agenda.php?id='.$object->id);
		$morehtmlcenter .= '</div>';
	...
	// Presend form
	$modelmail = 'invoice_send';
	$defaulttopic = 'SendBillRef';
	$diroutput = $config->invoice->multidir_output[$object->entity];
	$trackid = 'inv'.$object->id;
