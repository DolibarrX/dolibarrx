...
/**
 *	\file       htdocs/compta/invoice/prelevement.php
 *	\ingroup    invoice
...
if ($type == 'bank-transfer') {
	$result = restrictedArea($user, 'fournisseur', $id, 'invoice_fourn', 'invoice', 'fk_soc', $fieldid, $isdraft);
	if (!$user->hasRight('fournisseur', 'invoice', 'lire')) {
		accessforbidden();
	}
} else {
	$result = restrictedArea($user, 'invoice', $id, '', '', 'fk_soc', $fieldid, $isdraft);
...
if ($type == 'bank-transfer') {
	$usercancreate = ($user->rights->fournisseur->invoice->creer || $user->rights->supplier_invoice->creer);
} else {
...
if (empty($resHook)) {
	...
	if ($action == 'sepastripedirectdebit' && $usercancreate) {
		$result = $object->makeStripeSepaRequest($user, GETPOSTINT('did'), 'direct-debit', 'invoice');
		if ($result < 0) {
			setEventMessages($object->error, $object->errors, 'errors');
		} else {
...
if ($object->id > 0) {
	...
	if ($type == 'bank-transfer') {
		if (!getDolGlobalString('MAIN_DISABLE_OTHER_LINK') && $object->thirdparty->id > 0) {
			$morehtmlref .= ' <div class="inline-block valignmiddle">(<a class="valignmiddle" href="'.DOL_URL_ROOT.'/fourn/invoice/list.php?socid='.$object->thirdparty->id.'&search_company='.urlencode($object->thirdparty->name).'">'.$langs->trans("OtherBills").'</a>)</div>';
		}
	} else {
		if (!getDolGlobalString('MAIN_DISABLE_OTHER_LINK') && $object->thirdparty->id > 0) {
			$morehtmlref .= ' <div class="inline-block valignmiddle">(<a class="valignmiddle" href="'.DOL_URL_ROOT.'/compta/invoice/list.php?socid='.$object->thirdparty->id.'&search_company='.urlencode($object->thirdparty->name).'">'.$langs->trans("OtherBills").'</a>)</div>';
		}
	}
	// Project
	...
	if ($object->type == $object::TYPE_CREDIT_NOTE && !empty($object->fk_invoice_source)) {
		if ($type == 'bank-transfer') {
	...
	*/
	...
	print '</td>';
	if ($object->type != $object::TYPE_CREDIT_NOTE && $action != 'editinvoicedate' && $object->status == $object::STATUS_DRAFT && $user->hasRight('invoice', 'creer')) {
	...
	print '</td>';
	if ($object->type != $object::TYPE_CREDIT_NOTE && $action != 'editconditions' && $object->status == $object::STATUS_DRAFT && $user->hasRight('invoice', 'creer')) {
	...
	print '</td>';
	if ($object->type != $object::TYPE_CREDIT_NOTE && $action != 'editpaymentterm' && $object->status == $object::STATUS_DRAFT && $user->hasRight('invoice', 'creer')) {
	...
	print '</td>';
	if ($action != 'editmode' && $object->status == $object::STATUS_DRAFT && $user->hasRight('invoice', 'creer')) {
	...
	print '<td>';
	if (($action != 'editbankaccount') && $user->hasRight('order', 'creer') && $object->status == $object::STATUS_DRAFT) {
	...
	if ($selleruserevenustamp) { 	// Test company use revenue stamp
		...
		print '</td>';
		if ($action != 'editrevenuestamp' && $object->status == $object::STATUS_DRAFT && $user->hasRight('invoice', 'creer')) {
	...
	print '<tr><td>'.$langs->trans('RemainderToPay').'</td><td class="nowrap right">'.price($resteapayer, 1, '', 1, - 1, - 1, $config->currency).'</td>';
	if (isModEnabled("multicurrency") && ($object->multicurrency_code && $object->multicurrency_code != $config->currency)) {
		print '<td></td>';
	}
	print '</tr>';
	...
	if ($type == 'bank-transfer') {
		$sql .= " WHERE fk_invoice_fourn = ".((int) $object->id);
	} else {
		...
		if ($resteapayer > 0) {
			if ($user_perms) {
				...
				if ($object->element == 'invoice' && $object->fk_fac_rec_source) {
					...
					$invoicerec->fetch($object->fk_fac_rec_source);
					if ($invoicerec->fk_societe_rib) {
						$companyBankAccount = new CompanyBankAccount($db);
						$res = $companyBankAccount->fetch($invoicerec->fk_societe_rib);
		...
		while ($i < $num) {
			...
			if ($type != 'bank-transfer') {
				if (getDolGlobalString('STRIPE_SEPA_DIRECT_DEBIT')) {
					...
					print '<a href="'.$_SERVER["PHP_SELF"].'?action=sepastripedirectdebit&paymentservice=stripesepa&token='.newToken().'&did='.$obj->rowid.'&id='.$object->id.'&type='.urlencode($type).'">'.img_picture('', 'stripe', 'class="picturefixedwidth"').$langs->trans("RequestDirectDebitWithStripe").'</a>';
				}
			} else {
				if (getDolGlobalString('STRIPE_SEPA_CREDIT_TRANSFER')) {
					...
					print '<a href="'.$_SERVER["PHP_SELF"].'?action=sepastripecredittransfer&paymentservice=stripesepa&token='.newToken().'&did='.$obj->rowid.'&id='.$object->id.'&type='.urlencode($type).'">'.img_picture('', 'stripe', 'class="picturefixedwidth"').$langs->trans("RequestDirectDebitWithStripe").'</a>';
				}
			}
			print '</td>';
