...
/**
 *	\file       htdocs/compta/invoice/class/invoice-rec.class.php
 *	\ingroup    invoice
 ...
 */
class InvoiceRec extends CommonInvoice
{
	const TRIGGER_PREFIX = 'BILLREC';
	/**
	 * @var string ID to identify managed object
	 */
	public $element = 'invoicerec';

	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'invoice_rec';

	/**
	 * @var string    Name of subtable line
	 */
	public $table_element_line = 'invoicedet_rec';
	...
	public function create($user, $facid, $notrigger = 0, $onlylines = [])
	{
		...
		$facsrc = new Invoice($this->db);
		$result = $facsrc->fetch($facid);
		if ($result > 0) {
			$this->socid = $facsrc->socid;

			$sql = "INSERT INTO ".MAIN_DB_PREFIX."invoice_rec (";
			$sql .= "titre";
			$sql .= ", fk_soc";
			$sql .= ", subtype";
			$sql .= ", entity";
			...
			if ($this->db->query($sql)) {
				$this->id = $this->db->last_insert_id(MAIN_DB_PREFIX."invoice_rec");
				...
				for ($i = 0; $i < $num; $i++) {
					...
					'@phan-var-force InvoiceLineRec $facline';
	...
	public function update(User $user, $notrigger = 0)
	{
		...
		$sql = "UPDATE ".MAIN_DB_PREFIX."invoice_rec SET";
	...
	public function fetch($rowid, $ref = '', $ref_ext = '', $noextrafields = 0, $nolines = 0)
	{
		...
		//$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."element_element as el ON el.fk_target = f.rowid AND el.targettype = 'invoice'";
		...
		if ($result) {
			if ($this->db->num_rows($result)) {
				...
				dol_syslog('Invoice::Fetch Error '.$this->error, LOG_ERR);
				return -2;
			}
		} else {
	...
	public function fetch_lines()
	{
		...
		dol_syslog('InvoiceRec::fetch_lines', LOG_DEBUG);

		$sql = 'SELECT l.rowid, l.fk_invoice, l.fk_product, l.fk_parent_line, l.product_type, l.label as custom_label, l.description, l.product_type, l.price, l.qty, l.vat_src_code, l.tva_tx, ';
		...
		$sql .= ' FROM '.MAIN_DB_PREFIX.'invoicedet_rec as l';
		...
		if ($result) {
			...
			while ($i < $num) {
				...
				$line = new InvoiceLineRec($this->db);
	...
	public function delete(User $user, $notrigger = 0, $idwarehouse = -1)
	{
		...
		$main = MAIN_DB_PREFIX.'invoicedet_rec';
		$ef = $main."_extrafields";

		$sqlef = "DELETE FROM $ef WHERE fk_object IN (SELECT rowid FROM ".$main." WHERE fk_invoice = ".((int) $rowid).")";
		$sql = "DELETE FROM ".MAIN_DB_PREFIX."invoicedet_rec WHERE fk_invoice = ".((int) $rowid);

		if ($this->db->query($sqlef) && $this->db->query($sql)) {
			$sql = "DELETE FROM ".MAIN_DB_PREFIX."invoice_rec WHERE rowid = ".((int) $rowid);
			dol_syslog($sql);
			if ($this->db->query($sql)) {
	...
	public function addline($desc, $pu_ht, $qty, $txtva, $txlocaltax1 = 0, $txlocaltax2 = 0, $fk_product = 0, $remise_percent = 0, $price_base_type = 'HT', $info_bits = 0, $fk_remise_except = 0, $pu_ttc = 0, $type = 0, $rang = -1, $special_code = 0, $label = '', $fk_unit = null, $pu_ht_devise = 0, $date_start_fill = 0, $date_end_fill = 0, $fk_fournprice = null, $pa_ht = 0, $fk_parent_line = 0)
	{
		...
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."invoicedet_rec (";
		$sql .= "fk_invoice";
		$sql .= ", fk_parent_line";
		$sql .= ", label";
		$sql .= ", description";
		...
		if ($this->db->query($sql)) {
			$lineId = $this->db->last_insert_id(MAIN_DB_PREFIX."invoicedet_rec");
			$this->id = $facid;
			$this->update_price(1);
			return $lineId;
		} else {
	...
	public function updateline($rowid, $desc, $pu_ht, $qty, $txtva, $txlocaltax1 = 0, $txlocaltax2 = 0, $fk_product = 0, $remise_percent = 0, $price_base_type = 'HT', $info_bits = 0, $fk_remise_except = 0, $pu_ttc = 0, $type = 0, $rang = -1, $special_code = 0, $label = '', $fk_unit = null, $pu_ht_devise = 0, $notrigger = 0, $date_start_fill = 0, $date_end_fill = 0, $fk_fournprice = null, $pa_ht = 0, $fk_parent_line = 0)
	{
		...
		$sql = "UPDATE ".MAIN_DB_PREFIX."invoicedet_rec SET ";
		$sql .= "fk_invoice = ".((int) $facid);
	...
	public function createRecurringInvoices($restrictioninvoiceid = 0, $forcevalidation = 0, $notrigger = 0)
	{
		...
		$sql = 'SELECT rowid FROM '.MAIN_DB_PREFIX.'invoice_rec';
		...
		if ($resql) {
			...
			while ($i < $num) {     // Loop on each template invoice. If $num = 0, test is false at first pass.
				...
				$invoice = null;
				$invoicerec = new InvoiceRec($this->db);
				$invoicerec->fetch($line->rowid);

				if ($invoicerec->id > 0) {
					// Set entity context
					$config->entity = $invoicerec->entity;

					dol_syslog("createRecurringInvoices Process invoice template id=".$invoicerec->id.", ref=".$invoicerec->ref.", entity=".$invoicerec->entity);

					$invoice = new Invoice($this->db);
					$invoice->fac_rec = $invoicerec->id; // We will create $invoice from this recurring invoice
					$invoice->fk_fac_rec_source = $invoicerec->id; // We will create $invoice from this recurring invoice

					$invoice->type = self::TYPE_STANDARD;
					$invoice->subtype = $invoicerec->subtype;
					$invoice->statut = self::STATUS_DRAFT;	// deprecated
					$invoice->status = self::STATUS_DRAFT;
					$invoice->date = (empty($invoicerec->date_when) ? $now : $invoicerec->date_when); // We could also use dol_now here but we prefer date_when so invoice has real date when we would like even if we generate later.
					$invoice->socid = $invoicerec->socid;
					if (!empty($invoicerec->fk_multicurrency)) {
						$invoice->fk_multicurrency = $invoicerec->fk_multicurrency;
						$invoice->multicurrency_code = $invoicerec->multicurrency_code;
						$invoice->multicurrency_tx = $invoicerec->multicurrency_tx;
					}

					if (isset($invoice->array_options) && isset($invoicerec->array_options)) {
						foreach ($invoicerec->array_options as $key => $value) {
					...
					$invoiceidgenerated = $invoice->create($user);
					...
					if (!$error && ($invoicerec->auto_validate || $forcevalidation)) {
					...
					if (!$error && $invoicerec->generate_pdf) {
						// We refresh the object in order to have all necessary data (like date_lim_reglement)
						$invoice->fetch($invoice->id);
						$result = $invoice->generateDocument($invoicerec->model_pdf, $langs);
						if ($result <= 0) {
							$this->errors = $invoice->errors;
							$this->error = $invoice->error;
							$error++;
				...
				if (!$error && $invoiceidgenerated >= 0) {
					$this->db->commit("createRecurringInvoices Process invoice template id=".$invoicerec->id.", ref=".$invoicerec->ref);
					dol_syslog("createRecurringInvoices Process invoice template ".$invoicerec->ref." is finished with a success generation");
					$nb_create++;
					$this->output .= $langs->trans("InvoiceGeneratedFromTemplate", $invoice->ref, $invoicerec->ref)."\n";
				} else {
					$this->output .= $langs->trans("InvoiceGeneratedFromTemplateError", $invoice->ref, $invoicerec->ref, $this->error)."\n";
					$this->db->rollback("createRecurringInvoices Process invoice template id=".$invoicerec->id.", ref=".$invoicerec->ref);
				}
				...
				$parameters = array(
					...
					'invoicerec' => $invoicerec, // it's an object which PHP passes by "reference", so modifiable by hooks.
					'this'       => $this, // it's an object which PHP passes by "reference", so modifiable by hooks.
				);
				$resHook = $hookManager->executeHooks('afterCreationOfRecurringInvoice', $parameters, $invoice); // note: $invoice can be modified by hooks (warning: $invoice can be null)
	...
	public function getNomUrl($withPicture = 0, $option = '', $max = 0, $short = 0, $moretitle = '', $notooltip = 0, $save_lastsearch_value = -1)
	{
		...
		$url = DOL_URL_ROOT.'/compta/invoice/card-rec.php?facid='.$this->id;
	...
	public function info($id)
	{
		...
		$sql .= ' FROM '.MAIN_DB_PREFIX.'invoice_rec as c';
 ...
 *  Saved into database table llx_invoicedet_rec
 */
class InvoiceLineRec extends CommonInvoiceLine
{
	/**
	 * @var string ID to identify managed object
	 */
	public $element = 'invoicedetrec';

	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'invoicedet_rec';

	/**
	 * @see CommonObjectLine
	 */
	public $parent_element = 'invoicerec';

	/**
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = 'fk_invoice';

	//!From llx_invoicedet_rec
	/**
	 * @var int Id invoice
	 */
	public $fk_invoice;
	...
	{
		$sql = 'SELECT l.rowid, l.fk_invoice, l.fk_parent_line, l.fk_product, l.product_type, l.label as custom_label, l.description, l.product_type, l.price, l.qty, l.vat_src_code, l.tva_tx,';
		$sql .= ' l.localtax1_tx, l.localtax2_tx, l.localtax1_type, l.localtax2_type, l.remise, l.remise_percent, l.subprice,';
		...
		if ($result) {
			...
			$this->id				= $objp->rowid;
			$this->fk_invoice		= $objp->fk_invoice;
	...
	public function update(User $user, $notrigger = 0)
	{
		...
		$sql = "UPDATE ".MAIN_DB_PREFIX."invoicedet_rec SET";
		$sql .= " fk_invoice = ".((int) $this->fk_invoice);
