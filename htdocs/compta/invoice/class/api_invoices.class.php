...
class Invoices extends DolibarrApi
{
	...
	/**
	 * @var Invoice $invoice {@type Invoice}
	 */
	private $invoice;

	/**
	 * @var InvoiceRec $template_invoice {@type InvoiceRec}
	 */
	...
	public function __construct()
	{
		...
		$this->template_invoice = new InvoiceRec($this->db);
	}
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $thirdparty_ids = '', $status = '', $sqlfilters = '', $properties = '', $pagination_data = false, $loadlinkedobjects = 0)
	{
		...
		$sql = "SELECT t.rowid";
		$sql .= " FROM ".MAIN_DB_PREFIX."invoice AS t";
		$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."invoice_extrafields AS ef ON (ef.fk_object = t.rowid)"; // Modification VMR Global Solutions to include extrafields as search parameters in the API GET call, so we will be able to filter on extrafields
		...
		if ($result) {
			...
			while ($i < $min) {
				...
				$invoice_static = new Invoice($this->db);
	...
	{
		if (!DolibarrApiAccess::$user->hasRight('invoice', 'creer')) {
	...
	*/
	...
	public function deleteLine($id, $lineid)
	{
		...
		if (!DolibarrApi::_checkAccessToResource('invoice', $id)) {
			throw new RestException(403, 'Access not allowed for login '.DolibarrApiAccess::$user->login);
		}
	...
	{
		if (!DolibarrApiAccess::$user->hasRight('invoice', 'supprimer')) {
			throw new RestException(403);
		}
		$result = $this->invoice->fetch($id);
		if (!$result) {
	...
	public function markAsCreditAvailable($id)
	{
		...
		$canconvert = 0;
		if ($this->invoice->type == Invoice::TYPE_DEPOSIT && empty($discountcheck->id)) {
		...
		if (($this->invoice->type == Invoice::TYPE_CREDIT_NOTE || $this->invoice->type == Invoice::TYPE_STANDARD) && $this->invoice->paye == 0 && empty($discountcheck->id)) {
		...
		if ($canconvert) {
			...
			if ($this->invoice->type == Invoice::TYPE_CREDIT_NOTE) {
				$discount->description = '(CREDIT_NOTE)';
			} elseif ($this->invoice->type == Invoice::TYPE_DEPOSIT) {
				$discount->description = '(DEPOSIT)';
			} elseif ($this->invoice->type == Invoice::TYPE_STANDARD || $this->invoice->type == Invoice::TYPE_REPLACEMENT || $this->invoice->type == Invoice::TYPE_SITUATION) {
			...
			$discount->fk_invoice_source = $this->invoice->id;

			$error = 0;

			if ($this->invoice->type == Invoice::TYPE_STANDARD || $this->invoice->type == Invoice::TYPE_REPLACEMENT || $this->invoice->type == Invoice::TYPE_SITUATION) {
				...
				$sql .= ' WHERE pf.fk_invoice = '.((int) $this->invoice->id);
				...
				$sql .= " WHERE fk_invoice = ".((int) $this->invoice->id);
				$resql = $this->db->query($sql);
				if (!empty($resql)) {
			...
			if ($this->invoice->type == Invoice::TYPE_CREDIT_NOTE || $this->invoice->type == Invoice::TYPE_DEPOSIT) {
			...
			if (empty($error)) {
				if ($this->invoice->type != Invoice::TYPE_DEPOSIT) {
	...
	private function _fetchTemplateInvoice($id, $ref = '', $ref_ext = '', $contact_list = 1)
	{
		...
		if (!DolibarrApi::_checkAccessToResource('invoicerec', $this->template_invoice->id)) {
			throw new RestException(403, 'Access not allowed for login '.DolibarrApiAccess::$user->login);
		}
