...
/**
 *       \file       htdocs/compta/invoice/class/invoicestats.class.php
 *       \ingroup    invoices
 ...
 */
class InvoiceStats extends Stats
{
	/**
	 * @var int
	 */
	...
	public function __construct(DoliDB $db, $socid, $mode, $userId = 0, $typentid = 0, $categid = 0)
	{
		...
		if ($categid) {
			$this->where .= ' AND EXISTS (SELECT cats.fk_category FROM '.MAIN_DB_PREFIX.'category_societe as cats WHERE cats.fk_soc = f.fk_soc AND cats.fk_category = '.((int) $categid).')';
		}
	}
	...
	public function getAllByProduct($year, $limit = 10)
	{
		...
		$sql .= $this->join;
		$sql .= " WHERE ".$this->where;
		$sql .= " AND f.rowid = tl.fk_invoice AND tl.fk_product = product.rowid";
