...
/**
 *	\file       htdocs/compta/invoice/class/invoiceline.class.php
 *	\ingroup    invoice
 ...
 *  Saved into database table llx_invoicedet
 */
class InvoiceLine extends CommonInvoiceLine
{
	 ...
	 */
	public $element = 'invoicedet';

	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'invoicedet';
	...
	{
		$sql = 'SELECT fd.rowid, fd.fk_invoice, fd.fk_parent_line, fd.fk_product, fd.product_type, fd.label as custom_label, fd.description, fd.price, fd.qty, fd.vat_src_code, fd.tva_tx,';
	...
	public function insert($notrigger = 0, $noerrorifdiscountalreadylinked = 0)
	{
		...
		$sql = 'INSERT INTO '.MAIN_DB_PREFIX.'invoicedet';
		$sql .= ' (fk_invoice, fk_parent_line, label, description, qty,';
		...
		if ($resql) {
			$this->id = $this->db->last_insert_id(MAIN_DB_PREFIX.'invoicedet');
	...
	public function update($user = null, $notrigger = 0)
	{
		...
		// Update line in database
		$sql = "UPDATE ".MAIN_DB_PREFIX."invoicedet SET";
	...
	public function delete($tmpuser = null, $notrigger = 0)
	{
		...
		$sql .= ' SET fk_invoice_line = NULL';
		$sql .= ' WHERE fk_invoice_line = '.((int) $this->id);
		...
		$sql = "DELETE FROM ".MAIN_DB_PREFIX."invoicedet WHERE rowid = ".((int) $this->id);
	...
	public function get_prev_progress($invoiceid, $include_credit_note = true)
	{
		...
		if (is_null($this->fk_prev_id) || empty($this->fk_prev_id) || $this->fk_prev_id == "") {
			...
			if (!isset($invoicecache[$invoiceid])) {
				$invoicecache[$invoiceid] = new Invoice($this->db);
				$invoicecache[$invoiceid]->fetch($invoiceid);
			}
			if ($invoicecache[$invoiceid]->type != Invoice::TYPE_SITUATION) {
				return 0;
			}

			$sql = "SELECT situation_percent FROM ".MAIN_DB_PREFIX."invoicedet";
			...
			if ($resql && $this->db->num_rows($resql) > 0) {
				...
				if ($include_credit_note) {
					...
					$sql .= ' JOIN '.MAIN_DB_PREFIX.'invoice f ON (f.rowid = fd.fk_invoice) ';
					...
					$sql .= " AND f.type = ".Invoice::TYPE_CREDIT_NOTE;
	...
	public function getAllPrevProgress($invoiceid, $include_credit_note = true)
	{
		...
		if (is_null($this->fk_prev_id) || empty($this->fk_prev_id) || $this->fk_prev_id == "") {
			...
			while (!$all_found) {
				$sql = "SELECT situation_percent, fk_prev_id FROM ".MAIN_DB_PREFIX."invoicedet WHERE rowid = ".((int) $lastprevid);
				$resql = $this->db->query($sql);
				...
				if ($resql && $this->db->num_rows($resql) > 0) {
					...
					if ($include_credit_note) {
						$sql_credit_note = 'SELECT fd.situation_percent FROM '.MAIN_DB_PREFIX.'invoicedet fd';
						$sql_credit_note .= ' JOIN '.MAIN_DB_PREFIX.'invoice f ON (f.rowid = fd.fk_invoice) ';
						...
						$sql_credit_note .= " AND f.type = ".Invoice::TYPE_CREDIT_NOTE;
