...
/**
 *	\file       htdocs/compta/invoice/class/invoice.class.php
 *	\ingroup    invoice
 *	\brief      File of class to manage invoices
 */

require_once DOL_DOCUMENT_ROOT.'/core/class/commoninvoice.class.php';
require_once DOL_DOCUMENT_ROOT.'/compta/invoice/class/invoiceline.class.php';
...
class Invoice extends CommonInvoice
{
	 ...
	 */
	public $element = 'invoice';

	/**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'invoice';

	/**
	 * @var string    Name of subtable line
	 */
	public $table_element_line = 'invoicedet';

	/**
	 * @var string Fieldname with ID of parent key if this field has a parent
	 */
	public $fk_element = 'fk_invoice';
	...
	/**
	 * @var Invoice[] Table of previous situations
	 */
	public $tab_previous_situation_invoice = [];

	/**
	 * @var Invoice[] Table of next situations
	 */
	...
	public $fields = array(
		...
		'fk_invoice_source' => array('type' => 'integer', 'label' => 'SourceInvoice', 'enabled' => 1, 'visible' => -1, 'position' => 170),
	...
	public function create(User $user, $notrigger = 0, $forceduedate = 0)
	{
		...
		if ($this->fac_rec > 0) {
			...
			$_facrec = new InvoiceRec($this->db);
			...
			//$this->linked_objects['invoicerec'][0] = $this->fac_rec;
		...
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."invoice (";
		$sql .= " ref";
		$sql .= ", entity";
		$sql .= ", ref_ext";
		$sql .= ", type";
		...
		$sql .= ", ref_client";
		$sql .= ", fk_account";
		$sql .= ", module_source, pos_source, fk_fac_rec_source, fk_invoice_source, fk_user_author, fk_projet";
		...
		$sql .= ", ".($this->fk_invoice_source ? "'".$this->db->escape($this->fk_invoice_source)."'" : "null");
		...
		if ($resql) {
			$this->id = $this->db->last_insert_id(MAIN_DB_PREFIX.'invoice');

			// Update ref with new one
			$this->ref = '(PROV'.$this->id.')';
			$sql = 'UPDATE '.MAIN_DB_PREFIX."invoice SET ref='".$this->db->escape($this->ref)."' WHERE rowid=".((int) $this->id);
			...
			if (!$error && empty($this->fac_rec) && count($this->lines) && is_object($this->lines[0])) {	// If this->lines is array of InvoiceLines (preferred mode)
				...
				foreach ($this->lines as $i => $val) {
					...
					'@phan-var-force InvoiceLine $newinvoiceline';

					$newinvoiceline->context = $this->context;

					$newinvoiceline->fk_invoice = $this->id;
					...
					if ($result >= 0) {
						...
						if ($this->type == Invoice::TYPE_REPLACEMENT && $newinvoiceline->fk_remise_except) {
	...
	public function createFromCurrent(User $user, $invertdetail = 0)
	{
		...
		if (!empty($this->array_options)) {
			$invoice->array_options = $this->array_options;
		}
		...
		$invoice->fk_invoice_source = $this->fk_invoice_source;
		$invoice->type 			    = $this->type;
		$invoice->subtype 			= $this->subtype;
		$invoice->socid 		    = $this->socid;
		$invoice->date              = $this->date;
		$invoice->date_pointoftax   = $this->date_pointoftax;
		$invoice->note_public       = $this->note_public;
		$invoice->note_private      = $this->note_private;
		$invoice->ref_client        = $this->ref_client;
		$invoice->ref_customer      = $this->ref_customer;
		$invoice->model_pdf         = $this->model_pdf;
		$invoice->fk_project        = $this->fk_project;
		$invoice->cond_reglement_id = $this->cond_reglement_id;
		$invoice->mode_reglement_id = $this->mode_reglement_id;
		//$invoice->remise_absolue    = $this->remise_absolue;
		//$invoice->remise_percent    = $this->remise_percent;	// TODO deprecated

		$invoice->origin            = $this->origin;
		$invoice->origin_id         = $this->origin_id;
		$invoice->fk_account         = $this->fk_account;

		$invoice->lines = $this->lines; // Array of lines of invoice
		$invoice->situation_counter = $this->situation_counter;
		$invoice->situation_cycle_ref = $this->situation_cycle_ref;
		$invoice->situation_final = $this->situation_final;

		$invoice->retained_warranty = $this->retained_warranty;
		$invoice->retained_warranty_fk_cond_reglement = $this->retained_warranty_fk_cond_reglement;
		$invoice->retained_warranty_date_limit = $this->retained_warranty_date_limit;

		$invoice->fk_user_author = $user->id;
		$invoice->user_creation_id = $user->id;


		// Loop on each line of new invoice
		foreach ($invoice->lines as $i => $tmpline) {
			$invoice->lines[$i]->fk_prev_id = $this->lines[$i]->rowid;
			if ($invertdetail) {
				$invoice->lines[$i]->subprice  = -$invoice->lines[$i]->subprice;
				$invoice->lines[$i]->total_ht  = -$invoice->lines[$i]->total_ht;
				$invoice->lines[$i]->total_tva = -$invoice->lines[$i]->total_tva;
				$invoice->lines[$i]->total_localtax1 = -$invoice->lines[$i]->total_localtax1;
				$invoice->lines[$i]->total_localtax2 = -$invoice->lines[$i]->total_localtax2;
				$invoice->lines[$i]->total_ttc = -$invoice->lines[$i]->total_ttc;
				$invoice->lines[$i]->ref_ext = '';
			}
		}

		dol_syslog(get_class($this)."::createFromCurrent invertdetail=".$invertdetail." socid=".$this->socid." nboflines=".count($invoice->lines));
		...
		if ($facid <= 0) {
			...
			$this->fetchObjectLinked(null, '', $this->id, 'invoice');

			foreach ($this->linkedObjectsIds as $typeObject => $Tfk_object) {
				foreach ($Tfk_object as $fk_object) {
					$invoice->add_object_linked($typeObject, $fk_object);
				}
			}

			$invoice->add_object_linked('invoice', $this->fk_invoice_source);
		}
	...
	public function createFromOrder($object, User $user)
	{
		...
		for ($i = 0; $i < $num; $i++) {
			$line = new InvoiceLine($this->db);
			$src_line = $object->lines[$i];
			'@phan-var-force InvoiceLine $src_line';
	 ...
	 *
	 *  @param      Invoice			$object         	Object source
	 *  @param		User			$user				Object user
	 ...
	 * @return	?Invoice								The deposit created, or null if error (populates $origin->error in this case)
	 */
	public static function createDepositFromOrigin(CommonObject $origin, $date, $payment_terms_id, User $user, $notrigger = 0, $autoValidateDeposit = false, $overrideFields = [])
	{
		...
		// Invoice::create() also imports contact from origin
	...
	public function getNomUrl($withPicture = 0, $option = '', $max = 0, $short = 0, $moretitle = '', $notooltip = 0, $addlinktonotes = 0, $save_lastsearch_value = -1, $target = '')
	{
		...
		if ($option == 'withdraw') {
			$url = DOL_URL_ROOT.'/compta/invoice/prelevement.php?facid='.$this->id;
		} else {
			$url = DOL_URL_ROOT.'/compta/invoice/card.php?id='.$this->id;
		}
		...
		if ($addlinktonotes) {
			...
			if ($txttoshow) {
				...
				$result .= '<a href="'.DOL_URL_ROOT.'/compta/invoice/note.php?id='.$this->id.'" class="classfortooltip" title="'.dol_escape_htmltag($notetoshow, 1, 1).'">';
	...
	public function fetch($rowid, $ref = '', $ref_ext = '', $notused = 0, $fetch_situation = false)
	{
		...
		$sql .= ', f.fk_invoice_source, f.fk_fac_rec_source';
		...
		$sql .= ', c.code as cond_reglement_code, c.libelle as cond_reglement_libelle, c.libelle_invoice as cond_reglement_libelle_doc';
		...
		if ($resql) {
			if ($this->db->num_rows($resql)) {
				...
				$this->fk_invoice_source	= $obj->fk_invoice_source;
	...
	public function fetch_lines($only_product = 0, $loadalsotranslation = 0)
	{
		...
		$sql = 'SELECT l.rowid, l.fk_invoice, l.fk_product, l.fk_parent_line, l.label as custom_label, l.description, l.product_type, l.price, l.qty, l.vat_src_code, l.tva_tx,';
		...
		$sql .= ' WHERE l.fk_invoice = '.((int) $this->id);
		...
		if ($result) {
			...
			while ($i < $num) {
				...
				$line->fk_invoice       = $objp->fk_invoice;
	...
	public function fetchPreviousNextSituationInvoice()
	{
		...
		$sql = 'SELECT rowid, type, situation_cycle_ref, situation_counter FROM '.MAIN_DB_PREFIX.'invoice';
	...
	public function update(User $user, $notrigger = 0)
	{
		...
		// Update request
		$sql = "UPDATE ".MAIN_DB_PREFIX."invoice SET";
		...
		$sql .= " fk_invoice_source=".(isset($this->fk_invoice_source) ? (int) $this->fk_invoice_source : "null").",";
	...
	public function insert_discount($idremise)
	{
		...
		if ($result > 0) {
			...
			$facligne = new InvoiceLine($this->db);
			$facligne->fk_invoice = $this->id;
			...
			if ($remise->fk_invoice_source > 0) {
				$srcinvoice = new Invoice($this->db);
				$srcinvoice->fetch($remise->fk_invoice_source);
	...
	public function delete($user, $notrigger = 0, $idwarehouse = -1)
	{
		...
		if (!$error) {
			...
			$sql .= ' WHERE fk_invoice_source = '.((int) $rowid);
			$sql .= ' AND fk_invoice_line IS NULL';
			$resql = $this->db->query($sql);
			...
			if (count($list_rowid_det)) {
				...
				$sql .= ' SET fk_invoice = NULL, fk_invoice_line = NULL';
				$sql .= ' WHERE fk_invoice_line IN ('.$this->db->sanitize(implode(',', $list_rowid_det)).')';
			...
			$main = MAIN_DB_PREFIX.'invoicedet';
			$ef = $main."_extrafields";
			$sqlef = "DELETE FROM ".$ef." WHERE fk_object IN (SELECT rowid FROM ".$main." WHERE fk_invoice = ".((int) $rowid).")";
			// Delete invoice line
			$sql = 'DELETE FROM '.MAIN_DB_PREFIX.'invoicedet WHERE fk_invoice = '.((int) $rowid);

			if ($this->db->query($sqlef) && $this->db->query($sql) && $this->delete_linked_contact() >= 0) {
				$sql = 'DELETE FROM '.MAIN_DB_PREFIX.'invoice WHERE rowid = '.((int) $rowid);
				...
				if ($resql) {
					...
					if ($config->invoice->dir_output && !empty($this->ref)) {
						$dir = $config->invoice->dir_output."/".$ref;
						$file = $config->invoice->dir_output."/".$ref."/".$ref.".pdf";
	...
	public function validate($user, $force_number = '', $idwarehouse = 0, $notrigger = 0, $batch_rule = 0)
	{
		...
		if ((!getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && !$user->hasRight('invoice', 'creer'))
		|| (getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && !$user->hasRight('invoice', 'invoice_advance', 'validate'))) {
		...
		if ($this->type == self::TYPE_REPLACEMENT) {		// if this is a replacement invoice
			...
			$facreplaced = new Invoice($this->db);
			$result = $facreplaced->fetch($this->fk_invoice_source);
			if ($result <= 0) {
			...
			if ($idreplacement && $idreplacement != $this->id) {
				$facreplacement = new Invoice($this->db);
		...
		if ($num) {
			...
			*/
			 ...
			 */
			if (!$error && $this->type == self::TYPE_CREDIT_NOTE && $this->fk_invoice_source > 0) {
				$invoice_situation = new Invoice($this->db);
				$result = $invoice_situation->fetch($this->fk_invoice_source);
			...
			if (!$error) {
				...
				if (preg_match('/^[\(]?PROV/i', $this->ref)) {
					...
					$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'invoice/".$this->db->escape($this->newref)."'";
					$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'invoice/".$this->db->escape($this->ref)."' and entity = ".$config->entity;
					$resql = $this->db->query($sql);
					if (!$resql) {
						$error++;
						$this->error = $this->db->lasterror();
					}
					$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'invoice/".$this->db->escape($this->newref)."'";
					$sql .= " WHERE filepath = 'invoice/".$this->db->escape($this->ref)."' and entity = ".$config->entity;
					$resql = $this->db->query($sql);
					if (!$resql) {
						$error++;
						$this->error = $this->db->lasterror();
					...
					$dirsource = $config->invoice->dir_output.'/'.$oldref;
					$dirdest = $config->invoice->dir_output.'/'.$newref;
					if (!$error && file_exists($dirsource)) {
						...
						if (@rename($dirsource, $dirdest)) {
							...
							$listoffiles = dol_dir_list($config->invoice->dir_output.'/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function addline(
		...
		if ($this->status == self::STATUS_DRAFT) {
			...
			// Insert line
			$this->line = new InvoiceLine($this->db);

			$this->line->context = $this->context;

			$this->line->fk_invoice = $this->id;
			...
			if ($result > 0) {
				...
				// Mise a jour information denormalisees au niveau de la invoice meme
	...
	{
		$sql = 'SELECT fd.situation_percent FROM '.MAIN_DB_PREFIX.'invoicedet fd
				INNER JOIN '.MAIN_DB_PREFIX.'invoice f ON (fd.fk_invoice = f.rowid)
	...
	public function update_percent($line, $percent, $update_price = true)
	{
		...
		if (getDolGlobalInt('INVOICE_USE_SITUATION') == 2) {
			$previous_progress = $line->getAllPrevProgress($line->fk_invoice);
	...
	*/
	...
	public function getNextNumRef($soc, $mode = 'next')
	{
		...
		if ($this->module_source == 'takepos') {
			...
			if (!getDolGlobalString('FACTURE_ADDON')) {
				$config->global->FACTURE_ADDON = 'mod_invoice_terre';
			} elseif (getDolGlobalString('FACTURE_ADDON') == 'terre') {
				$config->global->FACTURE_ADDON = 'mod_invoice_terre';
			} elseif (getDolGlobalString('FACTURE_ADDON') == 'mercure') {
				$config->global->FACTURE_ADDON = 'mod_invoice_mercure';
			}
		...
		if (!empty($addon)) {
			...
			$obj = new $classname();
			'@phan-var-force ModeleNumRefInvoices $obj';
	...
	public function info($id)
	{
		...
		$sql .= ' FROM '.MAIN_DB_PREFIX.'invoice as c';
	...
	public function list_replacable_invoices($socid = 0)
	{
		...
		$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."invoice as ff ON f.rowid = ff.fk_invoice_source";
	...
	public function list_qualified_avoir_invoices($socid = 0)
	{
		...
		$sql .= " FROM ".MAIN_DB_PREFIX."invoice as f";
		$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."paiement_invoice as pf ON f.rowid = pf.fk_invoice";
		$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."invoice as ff ON (f.rowid = ff.fk_invoice_source AND ff.type=".self::TYPE_REPLACEMENT.")";
		$sql .= " WHERE f.entity IN (".getEntity('invoice').")";
		$sql .= " AND f.fk_statut in (".self::STATUS_VALIDATED.",".self::STATUS_CLOSED.")";
		//  $sql.= " WHERE f.fk_statut >= 1";
		//	$sql.= " AND (f.paye = 1";				// Classee payee completement
		//	$sql.= " OR f.close_code IS NOT NULL)";	// Classee payee partiellement
		$sql .= " AND ff.type IS NULL"; // Renvoi vrai si pas invoice de replacement
		...
		if (getDolGlobalString('INVOICE_USE_SITUATION_CREDIT_NOTE')) {
			...
			$sql .= " FROM ".MAIN_DB_PREFIX."invoice as fs";
	...
	public function load_board($user)
	{
		...
		if ($resql) {
			...
			$response->warning_delay = $config->invoice->client->warning_delay / 60 / 60 / 24;
			$response->label = $langs->trans("CustomerBillsUnpaid");
			$response->labelShort = $langs->trans("Unpaid");
			$response->url = DOL_URL_ROOT.'/compta/invoice/list.php?search_status=1&mainmenu=billing&leftmenu=customers_bills';
			$response->img = img_object('', "bill");

			$generic_invoice = new Invoice($this->db);

			while ($obj = $this->db->fetch_object($resql)) {
				$generic_invoice->date_lim_reglement = $this->db->jdate($obj->datefin);
				$generic_invoice->statut = $obj->status;
				$generic_invoice->status = $obj->status;

				$response->nbtodo++;
				$response->total += $obj->total_ht;

				if ($generic_invoice->hasDelay()) {
					$response->nbtodolate++;
					$response->url_late = DOL_URL_ROOT.'/compta/invoice/list.php?search_late=late&mainmenu=billing&leftmenu=customers_bills';
				}
			}
	...
	/* gestion des contacts d'une invoice */
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/invoice/doc/";
	 ...
	 *
	 * @return Invoice[]|int<-1,-1>	-1 if error, array of previous situations
	 */
	...
	public function setFinal(User $user, $notrigger = 0)
	{
		...
		$sql = 'UPDATE '.MAIN_DB_PREFIX.'invoice SET situation_final = '.((int) $this->situation_final).' WHERE rowid = '.((int) $this->id);
	...
	public function is_last_in_cycle()
	{
		...
		if (!empty($this->situation_cycle_ref)) {
			...
			$sql = 'SELECT max(situation_counter) FROM '.MAIN_DB_PREFIX.'invoice';
			$sql .= ' WHERE situation_cycle_ref = '.((int) $this->situation_cycle_ref);
			$sql .= ' AND entity = '.($this->entity > 0 ? $this->entity : $config->entity);
			$resql = $this->db->query($sql);
	...
	public function hasDelay()
	{
		...
		if ($this->status != Invoice::STATUS_VALIDATED) {
			return false;
		}

		$hasDelay = $this->date_lim_reglement < ($now - $config->invoice->client->warning_delay);
		if ($hasDelay && !empty($this->retained_warranty) && !empty($this->retained_warranty_date_limit)) {
			...
			if ($totalpaid >= 0 && $RetainedWarrantyAmount >= 0) {
				if (($totalpaid < $this->total_ttc - $RetainedWarrantyAmount) && $this->date_lim_reglement < ($now - $config->invoice->client->warning_delay)) {
					$hasDelay = 1;
				} elseif ($totalpaid < $this->total_ttc && $this->retained_warranty_date_limit < ($now - $config->invoice->client->warning_delay)) {
					$hasDelay = 1;
				} else {
					$hasDelay = 0;
				}
	...
	public function displayRetainedWarranty()
	{
		...
		if (!empty($this->retained_warranty)) {
			...
			if ($this->type == Invoice::TYPE_SITUATION && getDolGlobalString('INVOICE_RETAINED_WARRANTY_LIMITED_TO_FINAL_SITUATION')) {
				// Check if this situation invoice is 100% for real
				$displayWarranty = false;
				if (!empty($this->situation_final)) {
					$displayWarranty = true;
				} elseif (!empty($this->lines) && $this->status == Invoice::STATUS_DRAFT) {
	 ...
	 *  @param	int|string	$template			Name (or id) of email template (Must be a template of type 'invoice_send')
	...
	public function sendEmailsRemindersOnInvoiceDueDate($nbdays = 0, $paymentmode = 'all', $template = '', $datetouse = 'duedate', $forcerecipient = '')
	{
		...
		*/
		...
		$sql = "SELECT rowid as id FROM ".MAIN_DB_PREFIX."invoice as f";
		...
		$sql .= " AND f.entity IN (".getEntity('invoice', 0).")";	// One batch process only one company (no sharing)
		if (!empty($paymentmode) && $paymentmode != 'all') {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				if (!$error) {
					...
					if ($res > 0) {
						...
						$arraymessage = $formmail->getEMailTemplate($this->db, 'invoice_send', $user, $outputlangs, (is_numeric($template) ? $template : 0), 1, (is_numeric($template) ? '' : $template));
