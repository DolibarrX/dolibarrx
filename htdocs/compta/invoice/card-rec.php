...
/**
 *	\file       htdocs/compta/invoice/card-rec.php
 *	\ingroup    invoice
...
$permissionnote = $user->hasRight('invoice', 'creer'); // Used by the include of actions_setnotes.inc.php
$permissiondellink = $user->hasRight('invoice', 'creer'); // Used by the include of actions_dellink.inc.php
$permissiontoedit = $user->hasRight('invoice', 'creer'); // Used by the include of actions_lineupdonw.inc.php
...
$usercandelete = $user->hasRight('invoice', 'supprimer');
...
$usercansend = (!getDolGlobalString('MAIN_USE_ADVANCED_PERMS') || $user->rights->invoice->invoice_advance->send);
$usercanreopen = (!getDolGlobalString('MAIN_USE_ADVANCED_PERMS') || $user->rights->invoice->invoice_advance->reopen);
...
// Security check
$result = restrictedArea($user, 'invoice', $object->id, $objecttype);
...
if (empty($resHook)) {
	$backurlforlist = DOL_URL_ROOT.'/compta/invoice/invoicetemplate_list.php';
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = DOL_URL_ROOT.'/compta/invoice/invoicetemplate_list.php?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	// Delete
	if ($action == 'confirm_delete' && $confirm == 'yes' && $user->hasRight('invoice', 'supprimer')) {
		$object->delete($user);

		header("Location: ".DOL_URL_ROOT.'/compta/invoice/invoicetemplate_list.php');
		exit;
	}
	...
	if ($action == 'addline' && $usercancreate) {
		...
		$objectline = new InvoiceLineRec($db);
		...
		if (!empty($productid)) {
			...
			$typeinvoice = Invoice::TYPE_STANDARD;

			// Check price is not lower than minimum (check is done only for standard or replacement invoices)
			if (((getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && !$user->hasRight('produit', 'ignore_price_min_advance')) || !getDolGlobalString('MAIN_USE_ADVANCED_PERMS')) && (($typeinvoice == Invoice::TYPE_STANDARD || $typeinvoice == Invoice::TYPE_REPLACEMENT) && $price_min && ((float) price2num($pu_ht) * (1 - (float) $remise_percent / 100) < (float) price2num($price_min)))) {
...
$invoicerectmp = new InvoiceRec($db);
...
if ($action == 'create') {
	...
	$object = new Invoice($db); // Source invoice
	$invoiceRec = new InvoiceRec($db);
	...
	if ($object->fetch($id, $ref) > 0) {
		...
		print '<script>
				$(document).ready(function() {
                    if($("#selectmode_reglement_id option:selected").data("code") != "' . $invoiceRec::PAYMENTCODETOEDITSOCIETERIB . '") {
		...
		$list = ModelePDFInvoices::liste_modeles($db);
	...
	if ($object->id > 0) {
		...
		$linkback = '<a href="'.DOL_URL_ROOT.'/compta/invoice/invoicetemplate_list.php?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';

		$morehtmlref = '';
		if ($action != 'editref') {
			$morehtmlref .= $form->editfieldkey($object->ref, 'ref', $object->ref, $object, $user->hasRight('invoice', 'creer'), '', '', 0, 2);
		} else {
			$morehtmlref .= $form->editfieldval('', 'ref', $object->ref, $object, $user->hasRight('invoice', 'creer'), 'string');
		}

		$morehtmlref .= '<div class="refidno">';
		// Ref customer
		//$morehtmlref.=$form->editfieldkey("RefCustomer", 'ref_client', $object->ref_customer, $object, $user->hasRight('invoice', 'creer'), 'string', '', 0, 1);
		//$morehtmlref.=$form->editfieldval("RefCustomer", 'ref_client', $object->ref_customer, $object, $user->hasRight('invoice', 'creer'), 'string', '', null, null, '', 1);
		// Thirdparty
		...
		if (getDolGlobalInt('INVOICE_SUBTYPE_ENABLED')) {
			print "<tr><td>".$langs->trans("InvoiceSubtype")."</td><td>";
			if ($object->subtype > 0) {
				print $object->getSubtypeLabel('invoice_rec');
			}
			print "</td></tr>";
		}
		...
		print '</td>';
		if ($action != 'editconditions' && $user->hasRight('invoice', 'creer')) {
		...
		print '</td>';
		if ($action != 'editmode' && $user->hasRight('invoice', 'creer')) {
		...
		if ($object->mode_reglement_code == $object::PAYMENTCODETOEDITSOCIETERIB) {
			...
			if (($action != 'editbankaccountcustomer') && $user->hasRight('invoice', 'creer') && $object->statut == InvoiceRec::STATUS_DRAFT) {
		...
		print '<td>';
		if (($action != 'editbankaccount') && $user->hasRight('invoice', 'creer') && $object->statut == InvoiceRec::STATUS_DRAFT) {
		...
		print '<td>';
		if (($action != 'editmodelpdf') && $user->hasRight('invoice', 'creer') && $object->statut == InvoiceRec::STATUS_DRAFT) {
		...
		if ($action == 'editmodelpdf') {
			include_once DOL_DOCUMENT_ROOT.'/core/modules/invoice/modules_invoice.php';
			$list = [];
			$models = ModelePDFInvoices::liste_modeles($db);
			foreach ($models as $k => $model) {
				$list[] = str_replace(':', '|', $k).':'.$model;
			}
			$select = 'select;'.implode(',', $list);
			print $form->editfieldval($langs->trans("Model"), 'modelpdf', $object->model_pdf, $object, $user->hasRight('invoice', 'creer'), $select);
		} else {
		...
		print '</td>';
		if ($action != 'editfrequency' && $user->hasRight('invoice', 'creer')) {
		...
		print '</td><td>';
		if ($action == 'date_when' || $object->frequency > 0) {
			print $form->editfieldval($langs->trans("NextDateToExecution"), 'date_when', $object->date_when, $object, $user->hasRight('invoice', 'creer'), 'day', $object->date_when, null, '', '', 0, 'strikeIfMaxNbGenReached');
		}
		...
		if ($action == 'auto_validate' || $object->frequency > 0) {
			print $form->editfieldkey($langs->trans("StatusOfAutoGeneratedInvoices"), 'auto_validate', $object->auto_validate, $object, $user->hasRight('invoice', 'creer'));
		} else {
		...
		if ($action == 'auto_validate' || $object->frequency > 0) {
			print $form->editfieldval($langs->trans("StatusOfAutoGeneratedInvoices"), 'auto_validate', $object->auto_validate, $object, $user->hasRight('invoice', 'creer'), $select);
		}
		print '</td>';
		...
		if (getDolGlobalString('INVOICE_REC_CAN_DISABLE_DOCUMENT_FILE_GENERATION')) {
			...
			if ($action == 'generate_pdf' || $object->frequency > 0) {
				print $form->editfieldkey($langs->trans("StatusOfGeneratedDocuments"), 'generate_pdf', $object->generate_pdf, $object, $user->hasRight('invoice', 'creer'));
			} else {
			...
			if ($action == 'generate_pdf' || $object->frequency > 0) {
				print $form->editfieldval($langs->trans("StatusOfGeneratedDocuments"), 'generate_pdf', $object->generate_pdf, $object, $user->hasRight('invoice', 'creer'), $select);
			}
			print '</td>';
			print '</tr>';
		} else {
		...
		if ($object->status == $object::STATUS_DRAFT && $user->hasRight('invoice', 'creer') && $action != 'valid' && $action != 'editline') {
			if ($action != 'editline') {
		...
		if (empty($resHook)) {
				...
				if ($user->hasRight('invoice', 'creer')) {
					if (!empty($object->frequency) && $object->nb_gen_max > 0 && ($object->nb_gen_done >= $object->nb_gen_max)) {
						...
						if (empty($object->frequency) || $object->date_when <= $nowlasthour) {
							print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/compta/invoice/card.php?action=create&socid=' . $object->thirdparty->id . '&fac_rec=' . $object->id . '">' . $langs->trans("CreateBill") . '</a></div>';
						} else {
			...
			// Delete
			print dolGetButtonAction($langs->trans("Delete"), '', 'delete', $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&action=delete&token=' . newToken(), 'delete', $user->hasRight('invoice', 'supprimer'));
		}
		print '</div>';
