...
/**
 *	\file       htdocs/compta/invoice/list.php
 *	\ingroup    invoice
...
$diroutputmassaction = $config->invoice->dir_output.'/temp/massgeneration/'.$user->id;
...
$result = restrictedArea($user, 'invoice', $id, '', '', 'fk_soc', $fieldid);
...
if (empty($resHook)) {
	...
	$permissiontoread = $user->hasRight("invoice", "lire");
	$permissionToAdd = $user->hasRight("invoice", "creer");
	$permissiontodelete = $user->hasRight("invoice", "supprimer");
	$uploaddir = $config->invoice->dir_output;
	include DOL_DOCUMENT_ROOT.'/core/actions_massactions.inc.php';
}

if ($action == 'makepayment_confirm' && $user->hasRight('invoice', 'paiement')) {
	require_once DOL_DOCUMENT_ROOT.'/compta/paiement/class/paiement.class.php';
	...
	if (!empty($arrayofselected)) {
		...
		if (!$error) {
			foreach ($arrayofselected as $toselectid) {
				...
				$result = $invoice->fetch($toselectid);
				...
				if ($result < 0) {
					...
					if ($invoice->type != Invoice::TYPE_CREDIT_NOTE && $invoice->status == Invoice::STATUS_VALIDATED && $invoice->paye == 0) {
						$paiementAmount = $invoice->getSommePaiement();
						...
						$remaintopay = price2num($invoice->total_ttc - $totalallpayments);

						// hook to finalize the remaining amount, considering e.g. cash discount agreements
						$parameters = array('remaintopay' => $remaintopay);
						$resHook = $hookManager->executeHooks('finalizeAmountOfInvoice', $parameters, $invoice, $action); // Note that $action and $object may have been modified by some hooks
						if ($resHook > 0) {
							...
							$error++;
							setEventMessages($invoice->ref.' '.$langs->trans("ProcessingError"), $hookManager->errors, 'errors');
						}

						if ($remaintopay != 0) {
							$resultBank = $invoice->setBankAccount($bankid);
							if ($resultBank < 0) {
								setEventMessages($invoice->error, null, 'errors');
								$errorpayment++;
							} else {
								$paiement = new Paiement($db);
								$paiement->datepaye = $paiementdate;
								$paiement->amounts[$invoice->id] = $remaintopay; // Array with all payments dispatching with invoice id
								$paiement->multicurrency_amounts[$invoice->id] = $remaintopay;
								$paiement->paiementid = $paiementid;
								$paiement_id = $paiement->create($user, 1, $invoice->thirdparty);
								if ($paiement_id < 0) {
									$langs->load("errors");
									setEventMessages($invoice->ref.' '.$langs->trans($paiement->error), $paiement->errors, 'errors');
									$errorpayment++;
								} else {
							...
							setEventMessage($langs->trans('NoPaymentAvailable', $invoice->ref), 'warnings');
							$errorpayment++;
						}
					} else {
						setEventMessage($langs->trans('BulkPaymentNotPossibleForInvoice', $invoice->ref), 'warnings');
						$errorpayment++;
					}
				}

				if (empty($errorpayment)) {
					setEventMessage($langs->trans('PaymentRegisteredAndInvoiceSetToPaid', $invoice->ref));
					$db->commit();
				} else {
					$db->rollback();
				}
	...
	if (!$user->hasRight('prelevement', 'bons', 'creer')) {
		...
		foreach ($arrayofselected as $toselectid) {
			...
			if ($result > 0) {
				...
				if ($objecttmp->statut == Invoice::STATUS_DRAFT) {
					$error++;
				...
				$rsql .= " WHERE fk_invoice = ".((int) $objecttmp->id);
		...
		if (!empty($listofbills)) {
			...
			foreach ($listofbills as $aBill) {
				$db->begin();
				$result = $aBill->demande_prelevement($user, $aBill->resteapayer, 'direct-debit', 'invoice');
				if ($result > 0) {
					$db->commit();
					$nbwithdrawrequestok++;
				} else {
...
if ($sortfield == "f.datef") {
	$sql .= $db->hintindex('idx_invoice_datef');
}
...
if ($search_all) {
	$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'invoicedet as pd ON f.rowid = pd.fk_invoice';
}
if (!empty($search_fac_rec_source_title)) {
	$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'invoice_rec as facrec ON f.fk_fac_rec_source = facrec.rowid';
}
...
if ($search_option == 'late') {
	$sql .= " AND f.date_lim_reglement < '".$db->idate(dol_now() - $config->invoice->client->warning_delay)."'";
}
/*if ($search_sale > 0) {
...
if ($search_user > 0) {
	...
	$sql .= " ON ec.fk_c_type_contact = tc.rowid AND tc.element='invoice' AND tc.source='internal'";
...
if (!empty($searchCategoryProductList)) {
	...
	foreach ($searchCategoryProductList as $searchCategoryProduct) {
		if (intval($searchCategoryProduct) == -2) {
			$searchCategoryProductSqlList[] = "NOT EXISTS (SELECT ck.fk_product FROM ".MAIN_DB_PREFIX."category_product as ck, ".MAIN_DB_PREFIX."invoicedet as fd WHERE fd.fk_invoice = f.rowid AND fd.fk_product = ck.fk_product)";
		} elseif (intval($searchCategoryProduct) > 0) {
			if ($searchCategoryProductOperator == 0) {
				$searchCategoryProductSqlList[] = " EXISTS (SELECT ck.fk_product FROM ".MAIN_DB_PREFIX."category_product as ck, ".MAIN_DB_PREFIX."invoicedet as fd WHERE fd.fk_invoice = f.rowid AND fd.fk_product = ck.fk_product AND ck.fk_category = ".((int) $searchCategoryProduct).")";
			} else {
	...
	if ($listofcategoryid) {
		$searchCategoryProductSqlList[] = " EXISTS (SELECT ck.fk_product FROM ".MAIN_DB_PREFIX."category_product as ck, ".MAIN_DB_PREFIX."invoicedet as fd WHERE fd.fk_invoice = f.rowid AND fd.fk_product = ck.fk_product AND ck.fk_category IN (".$db->sanitize($listofcategoryid)."))";
	}
...
if ($num == 1 && getDolGlobalString('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all) {
	...
	header("Location: ".DOL_URL_ROOT.'/compta/invoice/card.php?id='.$id);
	exit;
}
...
if ($user->hasRight('invoice', 'paiement')) {
	$arrayofmassactions['makepayment'] = img_picture('', 'payment', 'class="picturefixedwidth"').$langs->trans("MakePaymentAndClassifyPayed");
}
if (isModEnabled('prelevement') && $user->hasRight('prelevement', 'bons', 'creer')) {
	$langs->load("withdrawals");
	$arrayofmassactions['withdrawrequest'] = img_picture('', 'payment', 'class="picturefixedwidth"').$langs->trans("MakeWithdrawRequest");
}
if ($user->hasRight('invoice', 'supprimer')) {
	if (getDolGlobalString('INVOICE_CAN_REMOVE_DRAFT_ONLY')) {
		$arrayofmassactions['predeletedraft'] = img_picture('', 'delete', 'class="picturefixedwidth"').$langs->trans("Deletedraft");
...
$newcardbutton = '';
if ($contextpage != 'poslist') {
	$url = DOL_URL_ROOT.'/compta/invoice/card.php?action=create';
	...
	$newcardbutton .= dolGetButtonTitle($langs->trans('NewBill'), '', 'fa fa-plus-circle', $url, '', $user->hasRight("invoice", "creer"));
}
...
if ($user->hasRight("user", "user", "lire")) {
	...
	$moreforfilter .= img_picture($tmptitle, 'user', 'class="picturefixedwidth"').$formother->select_salesrepresentatives($search_sale, 'search_sale', $user, 0, $tmptitle, 'maxwidth200');
	$moreforfilter .= '</div>';
}
// If the user can view prospects other than his'
if ($user->hasRight("user", "user", "lire")) {
	...
	$moreforfilter .= img_picture($tmptitle, 'user', 'class="picturefixedwidth"').$form->select_dolusers($search_user, 'search_user', $tmptitle, '', 0, '', '', 0, 0, 0, '', 0, '', 'maxwidth200');
	$moreforfilter .= '</div>';
}
...
if (isModEnabled('category') && $user->hasRight("category", "lire")) {
	...
	$moreforfilter .= img_picture($tmptitle, 'category', 'class="picturefixedwidth"').$formother->select_categories('customer', $search_categ_cus, 'search_categ_cus', 1, $tmptitle);
	$moreforfilter .= '</div>';
}
// alert on due date
$moreforfilter .= '<div class="divsearchfield">';
...
if (!empty($arrayfields['f.type']['checked'])) {
	...
	//$listtype[Invoice::TYPE_PROFORMA]=$langs->trans("InvoiceProForma");     // A proformat invoice is not an invoice but must be an order.
...
if ($num > 0) {
	...
	while ($i < $imaxinloop) {
		...
		$invoicestatic->id = $obj->id;
		$invoicestatic->ref = $obj->ref;
		$invoicestatic->ref_client = $obj->ref_client;		// deprecated
		$invoicestatic->ref_customer = $obj->ref_client;
		$invoicestatic->type = $obj->type;
		$invoicestatic->subtype = $obj->subtype;
		$invoicestatic->total_ht = $obj->total_ht;
		$invoicestatic->total_tva = $obj->total_tva;
		$invoicestatic->total_ttc = $obj->total_ttc;
		$invoicestatic->multicurrency_code = $obj->multicurrency_code;
		$invoicestatic->multicurrency_tx = $obj->multicurrency_tx;
		$invoicestatic->multicurrency_total_ht = $obj->multicurrency_total_ht;
		$invoicestatic->multicurrency_total_tva = $obj->multicurrency_total_vat;
		$invoicestatic->multicurrency_total_ttc = $obj->multicurrency_total_ttc;
		$invoicestatic->statut = $obj->fk_statut;	// deprecated
		$invoicestatic->status = $obj->fk_statut;
		$invoicestatic->close_code = $obj->close_code;
		$invoicestatic->total_ttc = $obj->total_ttc;
		$invoicestatic->paye = $obj->paye;
		$invoicestatic->socid = $obj->fk_soc;

		$invoicestatic->date = $db->jdate($obj->datef);
		$invoicestatic->date_validation = $db->jdate($obj->date_valid);
		$invoicestatic->date_lim_reglement = $db->jdate($obj->datelimite);

		$invoicestatic->note_public = $obj->note_public;
		$invoicestatic->note_private = $obj->note_private;

		if (getDolGlobalString('INVOICE_USE_SITUATION') && getDolGlobalString('INVOICE_USE_RETAINED_WARRANTY')) {
			$invoicestatic->retained_warranty = $obj->retained_warranty;
			$invoicestatic->retained_warranty_date_limit = $obj->retained_warranty_date_limit;
			$invoicestatic->situation_final = $obj->retained_warranty_date_limit;
			$invoicestatic->situation_final = $obj->retained_warranty_date_limit;
			$invoicestatic->situation_cycle_ref = $obj->situation_cycle_ref;
			$invoicestatic->situation_counter = $obj->situation_counter;
		}
		...
		$totalcreditnotes = $invoicestatic->getSumCreditNotesUsed();
		$totaldeposits = $invoicestatic->getSumDepositsUsed();
		$totalallpayments = $paiement + $totalcreditnotes + $totaldeposits;
		$remaintopay = $obj->total_ttc - $totalallpayments;

		$multicurrency_paiement = $invoicestatic->getSommePaiement(1);
		$multicurrency_totalcreditnotes = $invoicestatic->getSumCreditNotesUsed(1);
		$multicurrency_totaldeposits = $invoicestatic->getSumDepositsUsed(1);

		$totalallpayments = $paiement + $totalcreditnotes + $totaldeposits;
		$remaintopay = price2num($invoicestatic->total_ttc - $totalallpayments);

		$multicurrency_totalpay = $multicurrency_paiement + $multicurrency_totalcreditnotes + $multicurrency_totaldeposits;
		$multicurrency_remaintopay = price2num($invoicestatic->multicurrency_total_ttc - $multicurrency_totalpay);

		if ($invoicestatic->status == Invoice::STATUS_CLOSED && $invoicestatic->close_code == 'discount_vat') {		// If invoice closed with discount for anticipated payment
			$remaintopay = 0;
			$multicurrency_remaintopay = 0;
		}
		if ($invoicestatic->type == Invoice::TYPE_CREDIT_NOTE && $obj->paye == 1) {		// If credit note closed, we take into account the amount not yet consumed
			$remaincreditnote = $discount->getAvailableDiscounts($companystatic, null, 'rc.fk_invoice_source='.$invoicestatic->id);
			$remaintopay = -$remaincreditnote;
			$totalallpayments = price2num($invoicestatic->total_ttc - $remaintopay);
			$multicurrency_remaincreditnote = $discount->getAvailableDiscounts($companystatic, null, 'rc.fk_invoice_source='.$invoicestatic->id, 0, 0, 1);
			$multicurrency_remaintopay = -$multicurrency_remaincreditnote;
			$multicurrency_totalpay = price2num($invoicestatic->multicurrency_total_ttc - $multicurrency_remaintopay);
		}

		$invoicestatic->alreadypaid = $paiement;
		$invoicestatic->totalpaid = $paiement;
		$invoicestatic->totalcreditnotes = $totalcreditnotes;
		$invoicestatic->totaldeposits = $totaldeposits;

		$marginInfo = [];
		if ($with_margin_info) {
			$invoicestatic->fetch_lines();
			$marginInfo = $formmargin->getMarginInfosArray($invoicestatic);
			$total_ht += $obj->total_ht;
			$total_margin += $marginInfo['total_margin'];
		}

		$object = $invoicestatic;
		...
		if ($mode == 'kanban') {
			...
			print $invoicestatic->getKanbanView('', $arraydata);
			if ($i == ($imaxinloop - 1)) {
				print '</div>';
				print '</td></tr>';
			}
			...
			if (!empty($arrayfields['f.ref']['checked'])) {
				...
				if ($contextpage == 'poslist') {
					...
					print $invoicestatic->getNomUrl(1, '', 200, 0, '', 0, 1);
				}
			...
			// Type
			if (!empty($arrayfields['f.type']['checked'])) {
				print '<td class="nowraponall tdoverflowmax100" title="'.$invoicestatic->getLibType().'">';
				print $invoicestatic->getLibType(2);
				print "</td>";
				if (!$i) {
					$totalarray['nbfield']++;
				}
			...
			if (!empty($arrayfields['f.subtype']['checked'])) {
				$labeltoshow = '';
				if ($invoicestatic->subtype > 0) {
					$labeltoshow = $invoicestatic->getSubtypeLabel('invoice');
				}
			...
			if (!empty($arrayfields['f.fk_statut']['checked'])) {
				print '<td class="nowrap center">';
				print $invoicestatic->getLibStatut(5, $totalallpayments);
				print "</td>";
				if (!$i) {
					$totalarray['nbfield']++;
				}
