...
/**
 *  \file       htdocs/compta/invoice/stats/index.php
 *  \ingroup    invoice
 *  \brief      Page des stats invoices
 */
...
require_once DOL_DOCUMENT_ROOT.'/compta/invoice/class/invoicestats.class.php';
...
if ($mode == 'customer' && !$user->hasRight('invoice', 'lire')) {
	accessforbidden();
}
if ($mode == 'supplier' && !$user->hasRight('fournisseur', 'invoice', 'lire')) {
	accessforbidden();
}
...
$dir = $config->invoice->dir_temp;

if ($mode == 'supplier') {
	$picture = 'supplier_invoice';
	$title = $langs->trans("BillsStatisticsSuppliers");
	$dir = $config->fournisseur->invoice->dir_temp;
}
...
$stats = new InvoiceStats($db, $socid, $mode, ($userId > 0 ? $userId : 0), ($typent_id > 0 ? $typent_id : 0), ($categ_id > 0 ? $categ_id : 0));
if ($mode == 'customer') {
...
$h = 0;
$head = [];
$head[$h][0] = DOL_URL_ROOT.'/compta/invoice/stats/index.php?mode='.urlencode($mode);
