...
/**
 *	\file       htdocs/compta/invoice/invoicetemplate_list.php
 *	\ingroup    invoice
...
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."invoice_rec_extrafields as ef ON ef.fk_object = f.rowid";
...
while ($i < $imaxinloop) {
	...
	if (getDolGlobalString('MAIN_CHECKBOX_LEFT_COLUMN')) {
		...
		if ($user->hasRight('invoice', 'creer') && empty($invoicerectmp->suspended)) {
			if ($invoicerectmp->isMaxNbGenReached()) {
				...
				print '<a href="'.DOL_URL_ROOT.'/compta/invoice/card.php?action=create&amp;socid='.$objp->socid.'&amp;fac_rec='.$objp->facid.'">';
				print img_picture($langs->trans("CreateBill"), 'add', 'class="none"');
				print '</a>';
			} else {
