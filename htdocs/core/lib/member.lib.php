...
function member_prepare_head(Adherent $object)
{
	...
	if ((isModEnabled('ldap') && getDolGlobalString('LDAP_MEMBER_ACTIVE'))
		...
		$head[$h][0] = DOL_URL_ROOT . '/members/ldap.php?id=' . $object->id;
	...
	$head[$h][0] = DOL_URL_ROOT . '/members/note.php?id=' . $object->id;
	...
	$upload_dir = $config->adherent->multidir_output[$object->entity] . '/' . get_exdir(0, 0, 0, 1, $object, 'member');
	$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
	$nbLinks = Link::count($db, $object->element, $object->id);
	$head[$h][0] = DOL_URL_ROOT . '/members/document.php?id=' . $object->id;
	...
	// Show agenda tab
	$head[$h][0] = DOL_URL_ROOT . '/members/agenda.php?id=' . $object->id;
...
function member_type_prepare_head(AdherentType $object)
{
	...
	if (getDolGlobalInt('MAIN_MULTILANGS')) {
		$head[$h][0] = DOL_URL_ROOT . "/members/type_translation.php?rowid=" . $object->id;
	...
	if ((isModEnabled('ldap') && getDolGlobalString('LDAP_MEMBER_TYPE_ACTIVE'))
		...
		$head[$h][0] = DOL_URL_ROOT . '/members/type_ldap.php?rowid=' . $object->id;
		$head[$h][1] = $langs->trans("LDAPCard");
		$head[$h][2] = 'ldap';
		$h++;
	}
...
function member_admin_prepare_head()
{
	...
	$head[$h][0] = DOL_URL_ROOT . '/members/admin/member_emails.php';
	...
	$head[$h][0] = DOL_URL_ROOT . '/members/admin/website.php';
...
function member_stats_prepare_head($object)
{
	...
	$head[$h][0] = DOL_URL_ROOT . '/members/stats/geo.php?mode=memberbycountry';
	$head[$h][1] = $langs->trans("Country");
	$head[$h][2] = 'statscountry';
	$h++;

	$head[$h][0] = DOL_URL_ROOT . '/members/stats/geo.php?mode=memberbyregion';
	$head[$h][1] = $langs->trans("Region");
	$head[$h][2] = 'statsregion';
	$h++;

	$head[$h][0] = DOL_URL_ROOT . '/members/stats/geo.php?mode=memberbystate';
	$head[$h][1] = $langs->trans("State");
	$head[$h][2] = 'statsstate';
	$h++;

	$head[$h][0] = DOL_URL_ROOT . '/members/stats/geo.php?mode=memberbytown';
	$head[$h][1] = $langs->trans('Town');
	$head[$h][2] = 'statstown';
	$h++;

	$head[$h][0] = DOL_URL_ROOT . '/members/stats/byproperties.php';
...
function subscription_prepare_head(Subscription $object)
{
	...
	$head[$h][0] = DOL_URL_ROOT . '/members/subscription/info.php?rowid=' . $object->id;
