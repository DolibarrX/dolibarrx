...
class doc_generic_asset_odt extends ModelePDFAsset
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($config->asset->dir_output) {
			...
			$dir = $config->asset->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($config->asset->dir_temp);
				if (!is_writable($config->asset->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $config->asset->dir_temp);
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $config->asset->dir_temp,
