...
class modService extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		/* We can't enable this here because it must be enabled in both product and service module and this creates duplicate inserts
		 ...
		 'position'=>300,
		 'enabled'=>'isModEnabled("product") && preg_match(\'/^(admintools|all)/\',$leftmenu)',   // Define condition to show or hide menu entry. Use '$config->mymodule->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
		...
		if (getDolGlobalString('EXPORTTOOL_CATEGORIES')) {
			$this->export_sql_end[$r] .= ' LEFT JOIN '.MAIN_DB_PREFIX.'category_product as cp ON cp.fk_product = p.rowid LEFT JOIN '.MAIN_DB_PREFIX.'category as cat ON cp.fk_category = cat.rowid';
		}
		if (getDolGlobalInt('MAIN_MULTILANGS')) {
		...
		if (!isModEnabled("product")) {	// We enable next import templates only if module product not already enabled (to avoid duplicate entries)
			...
			if (getDolGlobalString('PRODUIT_SOUSPRODUITS')) {
				...
				$this->export_sql_end[$r]  = ' FROM '.MAIN_DB_PREFIX.'product as p';
				if (getDolGlobalString('MAIN_PRODUCT_PERENTITY_SHARED')) {
					$this->export_sql_end[$r] .= " LEFT JOIN " . MAIN_DB_PREFIX . "product_perentity as pac ON pac.fk_product = p.rowid AND pac.entity = " . ((int) $config->entity);
				}
				$this->export_sql_end[$r] .= ' LEFT JOIN '.MAIN_DB_PREFIX.'product_extrafields as extra ON p.rowid = extra.fk_object,';
