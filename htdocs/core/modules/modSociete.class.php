...
class modSociete extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		$this->requiredBy = array("modExpedition", "modInvoice", "modFournisseur", "modFicheinter", "modPropale", "modContract", "modOrder"); // List of module ids to disable if this one is disabled
		...
		// Add extra fields
		$sql = "SELECT name, label, fieldrequired FROM ".MAIN_DB_PREFIX."extrafields WHERE type <> 'separate' AND elementtype = 'societe' AND entity IN (0, ".$config->entity.")";
		$resql = $this->db->query($sql);
		...
		// Add extra fields
		$sql = "SELECT name, label, fieldrequired FROM ".MAIN_DB_PREFIX."extrafields WHERE type <> 'separate' AND elementtype = 'socpeople' AND entity IN (0, ".$config->entity.")";
		$resql = $this->db->query($sql);
		...
		$this->import_examplevalues_array[$r] = array(//field order as per structure of table llx_societe_rib
			...
			'sr.code_bank' => 'account sort code (GB)/Routing number (US) eg. "8456"',
