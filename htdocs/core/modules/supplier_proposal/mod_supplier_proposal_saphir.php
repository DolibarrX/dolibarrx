...
class mod_supplier_proposal_saphir extends ModeleNumRefSupplierProposal
{
	...
	public function info($langs)
	{
		...
		$texte .= '<td class="right">'.$form->textWithPicture('<input type="text" class="flat minwidth175" name="masksupplier_proposal" value="'.$mask.'">', $tooltip, 1, 'help', 'valignmiddle', 0, 3, $this->name).'</td>';
	...
	public function getNextValue($objsoc, $supplier_proposal)
	{
		...
		$mask = !getDolGlobalString('SUPPLIER_PROPOSAL_SAPHIR_MASK') ? '' : $config->global->SUPPLIER_PROPOSAL_SAPHIR_MASK;
