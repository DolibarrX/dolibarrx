	...
	public function info($langs)
	{
		...
		if (count($listofdir)) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=tickets/'.urlencode(basename($file['name'])).'">'.img_picture('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=TICKET_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picture('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($config->ticket->dir_output) {
			...
			if (file_exists($dir)) {
				...
				//print "file=".$file;
				//print "conf->ticket->dir_temp=".$config->ticket->dir_temp;

				dol_mkdir($config->ticket->dir_temp);
				if (!is_writable($config->ticket->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $config->ticket->dir_temp);
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $config->ticket->dir_temp,
