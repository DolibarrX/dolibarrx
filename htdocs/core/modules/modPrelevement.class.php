...
class modPrelevement extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		$this->depends = array("modFacture", "modBank"); // List of module class names as string that must be enabled if this module is enabled
	...
	public function init($options = '')
	{
		...
		$sql = array(
			"DELETE FROM ".MAIN_DB_PREFIX."document_model WHERE nom = '".$this->db->escape($this->const[0][2])."' AND type = 'bankaccount' AND entity = ".((int) $config->entity),
			"INSERT INTO ".MAIN_DB_PREFIX."document_model (nom, type, entity) VALUES('".$this->db->escape($this->const[0][2])."','bankaccount',".((int) $config->entity).")",
		);
