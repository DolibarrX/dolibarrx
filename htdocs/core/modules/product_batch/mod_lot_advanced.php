...
class mod_lot_advanced extends ModeleNumRefBatch
{
	...
	public function info($langs)
	{
		...
		$texte .= '<td class="right">'.$form->textWithPicture('<input type="text" class="flat minwidth175" name="maskLot" value="'.$mask.'">', $tooltip, 1, 'help', 'valignmiddle', 0, 3, $this->name).'</td>';
		...
		if (getDolGlobalString('PRODUCTBATCH_LOT_USE_PRODUCT_MASKS')) {
			$texte .= '<a class="reposition" href="'.$_SERVER["PHP_SELF"].'?action=setmaskslot&token='.newToken().'&value=0">'.img_picture($langs->trans("Enabled"), 'on').'</a>';
		} else {
			$texte .= '<a class="reposition" href="'.$_SERVER["PHP_SELF"].'?action=setmaskslot&token='.newToken().'&value=1">'.img_picture($langs->trans("Disabled"), 'off').'</a>';
		}
