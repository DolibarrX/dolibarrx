...
class doc_generic_reception_odt extends ModelePdfReception
{
	...
	public function info($langs)
	{
		...
		$listofdir = explode(',', preg_replace('/[\r\n]+/', ',', trim($config->global->RECEPTION_ADDON_PDF_ODT_PATH)));
		$listoffiles = [];
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=receptions/'.urlencode(basename($file['name'])).'">'.img_picture('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=RECEPTION_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picture('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($config->reception->dir_output) {
			...
			$dir = !empty($config->reception->multidir_output[$object->entity]) ? $config->reception->multidir_output[$object->entity] : $config->reception->dir_output;
			...
			if (file_exists($dir)) {
				...
				if (!is_writable($config->reception->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $config->reception->dir_temp);
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $config->reception->dir_temp,
