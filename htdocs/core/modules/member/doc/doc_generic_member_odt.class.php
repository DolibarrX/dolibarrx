	...
	public function info($langs)
	{
		...
		if (count($listofdir)) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=members/'.urlencode(basename($file['name'])).'">'.img_picture('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=MEMBER_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picture('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $mode = 'member', $nooutput = 0, $filename = 'tmp_cards')
	{
		...
		if ($config->member->dir_output) {
			...
			if (file_exists($dir)) {
				...
				//print "file=".$file;
				//print "conf->member->dir_temp=".$config->member->dir_temp;

				dol_mkdir($config->member->dir_temp);
				if (!is_writable($config->member->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $config->member->dir_temp);
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $config->member->dir_temp,
