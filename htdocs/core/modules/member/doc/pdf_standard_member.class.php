...
class pdf_standard_member extends CommonStickerGenerator
{
	...
	public function Add_PDF_card(&$pdf, $textleft, $header, $footer, $outputlangs, $textright = '', $idmember = 0, $photo = '')
	{
		...
		$backgroundimage = '';
		if (getDolGlobalString('ADHERENT_CARD_BACKGROUND') && file_exists($config->member->dir_output.'/' . getDolGlobalString('ADHERENT_CARD_BACKGROUND'))) {
			$backgroundimage = $config->member->dir_output.'/' . getDolGlobalString('ADHERENT_CARD_BACKGROUND');
		}
	...
	public function write_file($object, $outputlangs, $srctemplatepath, $mode = 'member', $nooutput = '', $filename = 'tmp_cards')
	{
		...
		if (is_object($object)) {
			$outputdir = $config->member->dir_output;
			$dir = $outputdir."/".get_exdir(0, 0, 0, 0, $object, 'member');
			$file = $dir.'/'.$filename;
		} else {
			$outputdir = $config->member->dir_temp;
			$dir = $outputdir;
			$file = $dir.'/'.$filename;
		}
