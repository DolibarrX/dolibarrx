...
class mod_barcode_thirdparty_standard extends ModeleNumRefBarCode
{
	...
	public function info($langs)
	{
		...
		$texte .= '<td class="right">'.$form->textWithPicture('<input type="text" class="flat minwidth175" name="value1" value="'.getDolGlobalString('BARCODE_STANDARD_THIRDPARTY_MASK').'"'.$disabled.'>', $tooltip, 1, 'help', 'valignmiddle', 0, 3, $this->name).'</td>';
	...
	public function verif_syntax($codefortest, $typefortest)
	{
		...
		// Get Mask value
		$mask = !getDolGlobalString('BARCODE_STANDARD_THIRDPARTY_MASK') ? '' : $config->global->BARCODE_STANDARD_THIRDPARTY_MASK;
		if (!$mask) {
			$this->error = 'NotConfigured';
			return -1;
		}
