...
class modTakePos extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		$this->needDolibarrVersion = array(4, 0); // Minimum version of Dolibarr required by module
		$this->warningsActivation = array('FR'=>'WarningNoteModulePOSForFrenchLaw'); // Warning to show when we activate module. array('always'='text') or array('FR'='text')
	...
	public function init($options = '')
	{
		...
		if (!getDolGlobalInt('CASHDESK_ID_THIRDPARTY1')) {	// If a customer has already ben set into the TakePos setup page
			...
			if ($searchcompanyid == 0) {
				...
				$societe->note_private = "Default customer automatically created by Point Of Sale module activation. Can be used as the default generic customer in the Point Of Sale setup. Can also be edited or removed if you do not need a generic customer.";
