...
/**
 *	\file       htdocs/core/modules/invoice/mod_invoice_mercure.php
 *	\ingroup    invoice
 ...
 */
class mod_invoice_mercure extends ModeleNumRefInvoices
{
	/**
	...
	public function info($langs)
	{
		...
		$texte .= '<td class="right">'.$form->textWithPicture('<input type="text" class="flat minwidth175" name="maskinvoice" value="'.getDolGlobalString("FACTURE_MERCURE_MASK_INVOICE").'">', $tooltip, 1, 'help', '', 0, 3, 'tooltipstandardmercure').'</td>';
		...
		$texte .= '<td class="right">'.$form->textWithPicture('<input type="text" class="flat minwidth175" name="maskcredit" value="'.getDolGlobalString("FACTURE_MERCURE_MASK_CREDIT").'">', $tooltip, 1, 'help', '', 0, 3, 'tooltipcreditnotemercure').'</td>';
		$texte .= '</tr>';
		...
		if (!getDolGlobalString('INVOICE_DISABLE_REPLACEMENT')) {
			...
			$texte .= '<td class="right">'.$form->textWithPicture('<input type="text" class="flat minwidth175" name="maskreplacement" value="'.getDolGlobalString("FACTURE_MERCURE_MASK_REPLACEMENT").'">', $tooltip, 1, 'help', '', 0, 3, 'tooltipreplacementmercure').'</td>';
			$texte .= '</tr>';
		}
		...
		if (!getDolGlobalString('INVOICE_DISABLE_DEPOSIT')) {
			...
			$texte .= '<td class="right">'.$form->textWithPicture('<input type="text" class="flat minwidth175" name="maskdeposit" value="'.getDolGlobalString("FACTURE_MERCURE_MASK_DEPOSIT").'">', $tooltip, 1, 'help', '', 0, 3, 'tooltipdownpaymentmercure').'</td>';
			$texte .= '</tr>';
		}
	...
	public function getNextValue($objsoc, $invoice, $mode = 'next')
	{
		...
		$where = '';
		//if ($invoice->type == 2) $where.= " AND type = 2";
		//else $where.=" AND type != 2";

		// Get entities
		$entity = getEntity('invoicenumber', 1, $invoice);
		$numFinal = get_next_value($db, $mask, 'invoice', 'ref', $where, $objsoc, (empty($invoice) ? dol_now() : $invoice->date), $mode, false, null, $entity);
