...
/**
 *	\file       htdocs/core/modules/invoice/doc/pdf_crabe.modules.php
 *	\ingroup    invoice
 ...
 */
class pdf_crabe extends ModelePDFInvoices
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($config->invoice->dir_output) {
			...
			if ($object->specimen) {
				$dir = empty($config->invoice->multidir_output[$object->entity]) ? $config->invoice->dir_output : $config->invoice->multidir_output[$object->entity];
				$file = $dir."/SPECIMEN.pdf";
			} else {
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = (empty($config->invoice->multidir_output[$object->entity]) ? $config->invoice->dir_output : $config->invoice->multidir_output[$object->entity])."/".$objectref;
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis)
	{
		...
		if (($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0) && !getDolGlobalString('INVOICE_NO_PAYMENT_DETAILS')) {
			...
			// Escompte
			if ($object->close_code == Invoice::CLOSECODE_DISCOUNTVAT) {
				$index++;
				$pdf->SetFillColor(255, 255, 255);
