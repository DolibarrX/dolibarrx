...
/**
 *  \file       htdocs/core/modules/invoice/doc/pdf_octopus.modules.php
 *  \ingroup    invoice
 ...
 */
class pdf_octopus extends ModelePDFInvoices
{
	/**
	 ...
	 * Situation invoices
	 *
	 * @var array{derniere_situation?:Invoices,date_derniere_situation?:int,cumul_anterieur:array<'HT'|'HTnet'|'retenue_garantie'|'total_a_payer'|'travaux_sup'|'TTC'|'TVA'|int,mixed>,current:array{HT:float,HTnet:float,TVA:float,TTC:float,retenue_garantie:float,travaux_sup:float,total_a_payer:float,derniere_situation?:Invoices,date_derniere_situation:int},nouveau_cumul:array<int|string,mixed|float|int>}
	 */
	...
	{
		global $config, $langs, $mysoc, $object;
		...
		if (getDolGlobalString('INVOICE_USE_SITUATION_RETAINED_WARRANTY') && !getDolGlobalString('INVOICE_USE_RETAINED_WARRANTY')) {
			...
			$config->global->INVOICE_USE_RETAINED_WARRANTY = getDolGlobalString('INVOICE_USE_SITUATION_RETAINED_WARRANTY');
			$config->global->USE_RETAINED_WARRANTY_ONLY_FOR_SITUATION_FINAL = 1;
		}
	 ...
	 *
	 *  @param		Invoice			$object				Object to generate
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if (empty($object) || ($object->type != Invoice::TYPE_SITUATION && ($object->type != Invoice::TYPE_CREDIT_NOTE &&  !empty($object->situation_cycle_ref)))) {
		...
		if ($config->invoice->multidir_output[$config->entity]) {
			...
			if ($object->specimen) {
				$dir = $config->invoice->multidir_output[$config->entity];
				$file = $dir."/SPECIMEN.pdf";
			} else {
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = $config->invoice->multidir_output[$object->entity]."/".$objectref;
	 ...
	 *  @param  Invoice		$object         Object invoice
	...
	public function drawPaymentsTable(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$sql .= " WHERE re.fk_invoice_source = f.rowid AND re.fk_invoice = ".((int) $object->id);
		...
		//$sql.= " WHERE pf.fk_paiement = p.rowid AND pf.fk_invoice = 1";
	 ...
	 *
	 *   @param		TCPDI|TCPDF	$pdf			Object PDF
	 *   @param		Invoice		$object			Object to show
	 *   @param		float		$posy			Y
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		if ($object->type == 0 && $objectidnext) {
			$objectreplacing = new Invoice($this->db);
		...
		if ($object->type == 1) {
			$objectreplaced = new Invoice($this->db);
			$objectreplaced->fetch($object->fk_invoice_source);
		...
		if ($object->type == 2 && !empty($object->fk_invoice_source)) {
			$objectreplaced = new Invoice($this->db);
			$objectreplaced->fetch($object->fk_invoice_source);
	 ...
	 *
	 *  @param	Invoice		   $object    		common object
	 ...
	 *
	 * @return array{derniere_situation?:Invoices,date_derniere_situation?:int,cumul_anterieur:array<'HT'|'HTnet'|'retenue_garantie'|'total_a_payer'|'travaux_sup'|'TTC'|'TVA'|int,mixed>,current:array{HT:float,HTnet:float,TVA:float,TTC:float,retenue_garantie:float,travaux_sup:float,total_a_payer:float,derniere_situation?:Invoices,date_derniere_situation:int},nouveau_cumul:array<int|string,mixed|float|int>}
	 *
	...
	public function getDataSituation(&$object)
	{
		...
		$TDataSituation['cumul_anterieur'] = array(
			...
			'travaux_sup' => 0,
			'HTnet' => 0, //montant HT
			'total_a_payer' => 0 //amount "to pay" on the invoice
		);

		//S'il y a des invoices de situations précédentes
			...
			foreach ($TPreviousInvoices as $i => $previousInvoice) {
				...
				foreach ($previousInvoice->lines as $k => $l) {
					...
					if (empty($l->fk_prev_id) && ! $isFirstSituation) {
						// TODO: à clarifier, mais pour moi, un invoice de situation précédente qui a des progressions à 0% c'est pas logique
	...
	public function displayRetainedWarranty($object)
	{
		if (is_callable(array($object, 'displayRetainedWarranty'))) {
			...
			if (!empty($object->retained_warranty)) {
				...
				if ($object->type == Invoice::TYPE_SITUATION && getDolGlobalString('USE_RETAINED_WARRANTY_ONLY_FOR_SITUATION_FINAL')) {
					// Check if this situation invoice is 100% for real
					$displayWarranty = false;
					if (!empty($object->situation_final)) {
						$displayWarranty = true;
					} elseif (!empty($object->lines) && $object->status == Invoice::STATUS_DRAFT) {
	 ...
	 * @param  InvoiceLine			$current_line	current line
	 ...
	 *
	 * @return array{HT:float,HTnet:float,TVA:float,TTC:float,retenue_garantie:float,travaux_sup:float,total_a_payer:float,derniere_situation?:Invoices,date_derniere_situation:int}
	 */
	...
	public function resumeLastPage(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis)
	{
		...
		while ($idinv < count($previousinvoices)) {
			...
			// Escompte
			if ($invoice->close_code == Invoice::CLOSECODE_DISCOUNTVAT) {
				$index++;
				$pdf->SetFillColor(255, 255, 255);
