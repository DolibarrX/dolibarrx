...
class modStripe extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		$this->configPageUrl = array("stripe.php@stripe");
		...
		/* $this->menu[$r]=array(
			...
			'position'=>500,
			'enabled'=>'$config->stripe->enabled && isModEnabled("bank") && $config->global->MAIN_FEATURES_LEVEL >= 2',  // Define condition to show or hide menu entry. Use '$config->mymodule->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			'perms'=>'$user->rights->bank->modifier',	// Use 'perms'=>'$user->hasRight("mymodule","level1","level2")' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2
