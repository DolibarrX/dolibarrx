...
/**
 *	\file       htdocs/core/modules/supplier_invoice/mod_invoice_fournisseur_tulip.php
...
/**
 * \class      mod_invoice_fournisseur_tulip
 * \brief      Tulip Class of numbering models of suppliers invoices references
*/
class mod_invoice_fournisseur_tulip extends ModeleNumRefSuppliersInvoices
{
	/**
	...
	public function info($langs)
	{
		...
		$texte .= ':</td>';
		$texte .= '<td class="right">'.$form->textWithPicture('<input type="text" class="flat minwidth175" name="maskinvoice" value="'.getDolGlobalString("SUPPLIER_INVOICE_TULIP_MASK").'">', $tooltip, 1, 'help', '', 0, 3, 'tooltipstandardtulip').'</td>';
		...
		$texte .= '<td class="right">'.$form->textWithPicture('<input type="text" class="flat minwidth175" name="maskcredit" value="'.getDolGlobalString("SUPPLIER_CREDIT_TULIP_MASK").'">', $tooltip, 1, 'help', '', 0, 3, 'tooltipcredittuplie').'</td>';
		$texte .= '</tr>';
		...
		if (!getDolGlobalString('INVOICE_DISABLE_REPLACEMENT') && getDolGlobalInt('MAIN_FEATURES_LEVEL') >= 2) {
			...
			$texte .= '<td class="right">'.$form->textWithPicture('<input type="text" class="flat minwidth175" name="maskreplacement" value="'.getDolGlobalString("SUPPLIER_REPLACEMENT_TULIP_MASK").'">', $tooltip, 1, 'help', '', 0, 3, 'tooltipreplacementtulip').'</td>';
			$texte .= '</tr>';
		}

		// Prefix setting of deposit
		$texte .= '<tr><td>'.$langs->trans("Mask").' ('.$langs->trans("InvoiceDeposit").'):</td>';
		$texte .= '<td class="right">'.$form->textWithPicture('<input type="text" class="flat minwidth175" name="maskdeposit" value="'.getDolGlobalString("SUPPLIER_DEPOSIT_TULIP_MASK").'">', $tooltip, 1, 'help', '', 0, 3, 'tooltipdownpaymenttulip').'</td>';
		$texte .= '</tr>';
	...
	public function getNextValue($objsoc, $object, $mode = 'next')
	{
		...
		$numFinal = get_next_value($db, $mask, 'invoice_fourn', 'ref', '', $objsoc, is_object($object) ?$object->date :'');
	 ...
	 *
	 *  @param  Societe     		$objsoc         Object third party
	 *  @param  InvoiceSupplier  $objforref      Object for number to search
	 *  @param  string      		$mode           'next' for next value or 'last' for last value
