...
/**
 *	\file       htdocs/core/boxes/box_invoices_imp.php
 *	\ingroup    invoices
 ...
 */
class box_invoices_imp extends ModeleBoxes
{
	...
	public function __construct($db, $param)
	{
		...
		$this->urltoaddentry = DOL_URL_ROOT.'/compta/invoice/card.php?action=create';
	...
	public function loadBox($max = 5)
	{
		...
		$this->info_box_head = array(
			'text' => $langs->trans("BoxTitleOldestUnpaidCustomerBills", $this->max).'<a class="paddingleft valignmiddle" href="'.DOL_URL_ROOT.'/compta/invoice/list.php?search_status=1&sortfield=f.date_lim_reglement,f.ref&sortorder=ASC,ASC"><span class="badge">...</span></a>',
		...
		if ($user->hasRight('invoice', 'lire')) {
			...
			$sql2 .= ", ".MAIN_DB_PREFIX."invoice as f";
			$sql2 .= " LEFT JOIN ".MAIN_DB_PREFIX."paiement_invoice as pf ON f.rowid = pf.fk_invoice";
			...
			if ($result) {
				...
				while ($line < min($num, $this->max)) {
					...
					$invoicestatic->date_lim_reglement = $datelimit;
