...
/**
 *	\file       htdocs/core/boxes/box_invoices.php
 *	\ingroup    invoices
 *	\brief      Module de generation de l'affichage de la box invoices
 */
 ...
 */
class box_invoices extends ModeleBoxes
{
	...
	public function loadBox($max = 5)
	{
		...
		$this->info_box_head = array(
			'text' => $text.'<a class="paddingleft" href="'.DOL_URL_ROOT.'/compta/invoice/list.php?sortfield=f.tms&sortorder=DESC"><span class="badge">...</span></a>',
		...
		if ($user->hasRight('invoice', 'lire')) {
			...
			if ($result) {
				...
				while ($line < $num) {
					...
					$invoicestatic->date = $this->db->jdate($objp->date);
					$invoicestatic->date_lim_reglement = $this->db->jdate($objp->datelimite);
					...
					$this->info_box_contents[$line][] = array(
						...
						'text' => $invoicestatic->LibStatut($objp->paye, $objp->status, 3, $objp->am),
					);
