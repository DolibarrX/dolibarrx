...
/**
 *      \file       htdocs/core/boxes/box_invoices_fourn.php
 *      \ingroup    supplier
 *      \brief      Fichier de gestion d'une box des invoices fournisseurs
 */
 ...
 */
class box_invoices_fourn extends ModeleBoxes
{
	...
	public function __construct($db, $param)
	{
		...
		$this->urltoaddentry = DOL_URL_ROOT.'/fourn/invoice/card.php?action=create';
	...
	public function loadBox($max = 5)
	{
		...
		$this->info_box_head = array(
			'text' => $text.'<a class="paddingleft" href="'.DOL_URL_ROOT.'/fourn/invoice/list.php?sortfield=f.tms&sortorder=DESC"><span class="badge">...</span></a>'
		);
		...
		if ($user->hasRight('fournisseur', 'invoice', 'lire')) {
			...
			if ($result) {
				...
				while ($line < $num) {
					...
					$invoicestatic->date = $date;
					$invoicestatic->date_echeance = $datelimite;
					$invoicestatic->statut = $objp->status;
					$invoicestatic->status = $objp->status;
					$invoicestatic->ref_supplier = $objp->ref_supplier;

					$alreadypaid = $invoicestatic->getSommePaiement();

					$invoicestatic->alreadypaid = $alreadypaid ? $alreadypaid : 0;
					...
					$this->info_box_contents[$line][] = array(
						...
						'text' => $invoicestatic->getNomUrl(1),
						'text2' => $late,
						'asis' => 1,
					);
					...
					$this->info_box_contents[$line][] = array(
						...
						'url' => DOL_URL_ROOT."/fourn/invoice/card.php?facid=".$objp->facid,
					);
					...
					$this->info_box_contents[$line][] = array(
						...
						'text' => $invoicestatic->LibStatut($objp->paye, $objp->status, 3, $alreadypaid, $objp->type),
					);
