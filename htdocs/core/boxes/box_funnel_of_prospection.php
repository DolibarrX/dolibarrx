		...
		if ($user->hasRight('projet', 'lire') || getDolGlobalString('PROJECT_USE_OPPORTUNITIES')) {
			...
			if ($resql) {
				...
				foreach ($listofstatus as $status) {
					...
					if ($status != 7) {
						...
						if (!$config->use_javascript_ajax) {
							...
							$stringtoprint .= '<td class="nowraponall right amount"><a href="list.php?statut='.$status.'">'.price((isset($valsamount[$status]) ? (float) $valsamount[$status] : 0), 0, '', 1, -1, -1, $config->currency).'</a></td>';
							$stringtoprint .= "</tr>\n";
						}
				...
				$this->info_box_contents[$line][] = array(
					...
					'maxlength' => 500,
					'text' => price($totalamount, 0, '', 1, -1, -1, $config->currency)
				);
				$line++;
				$this->info_box_contents[$line][] = array(
					'tr' => 'class="oddeven"',
					'td' => 'class="left "',
					'maxlength' => 500,
					'text' => $form->textWithPicture($langs->trans("OpportunityPonderatedAmount").' ('.$langs->trans("WonLostExcluded").')', $langs->trans("OpportunityPonderatedAmountDesc"), 1)

				);
				$this->info_box_contents[$line][] = array(
					'td' => 'class="nowraponall right amount"',
					'maxlength' => 500,
					'text' => price(price2num($ponderated_opp_amount, 'MT'), 0, '', 1, -1, -1, $config->currency)
				);
			} else {
