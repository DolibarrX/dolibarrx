...
/**
 *      \file       htdocs/core/boxes/box_invoices_fourn_imp.php
 *      \ingroup    fournisseur
 ...
 */
class box_invoices_fourn_imp extends ModeleBoxes
{
	...
	public function loadBox($max = 5)
	{
		...
		$this->info_box_head = array(
			'text' => $langs->trans("BoxTitleOldestUnpaidSupplierBills", $this->max).'<a class="paddingleft valignmiddle" href="'.DOL_URL_ROOT.'/fourn/invoice/list.php?search_status=1&sortfield=f.date_lim_reglement,f.ref&sortorder=ASC,ASC"><span class="badge">...</span></a>',
		...
		if ($user->hasRight('fournisseur', 'invoice', 'lire')) {
			...
			$sql2 .= ",".MAIN_DB_PREFIX."invoice_fourn as f";
			$sql2 .= " LEFT JOIN ".MAIN_DB_PREFIX."paiementfourn_invoicefourn as pf ON f.rowid = pf.fk_invoicefourn";
			...
			if ($result) {
				...
				while ($line < min($num, $this->max)) {
					...
					$this->info_box_contents[$line][] = array(
						...
						'text' => $invoicestatic->LibStatut($objp->paye, $objp->status, 3, $objp->am, $objp->type),
					);
