...
if (isModEnabled('multicurrency')) {
	...
	if ($object->multicurrency_code != $config->currency || $object->multicurrency_tx != 1) {
		...
		print '</td>';
		if ($usercancreate && $action != 'editmulticurrencyrate' && $currencyIsEditable && $object->multicurrency_code && $object->multicurrency_code != $config->currency) {
		...
		if ($action == 'editmulticurrencyrate' || $action == 'actualizemulticurrencyrate') {
			...
			if ($object->status == $object::STATUS_DRAFT && $object->multicurrency_code && $object->multicurrency_code != $config->currency) {
