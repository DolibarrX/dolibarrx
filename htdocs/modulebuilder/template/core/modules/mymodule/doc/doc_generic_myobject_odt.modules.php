...
class doc_generic_myobject_odt extends ModelePDFMyObject
{
	...
	public function info($langs)
	{
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=mymodule_myobject/'.urlencode(basename($file['name'])).'">'.img_picture('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=MYMODULE_MYOBJECT_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picture('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($config->mymodule->dir_output) {
			...
			$dir = $config->mymodule->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($config->mymodule->dir_temp);
				if (!is_writable($config->mymodule->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $config->mymodule->dir_temp);
				...
				if (!empty($usecontact)) {
					...
					if ($object->contact->socid != $object->thirdparty->id && (!isset($config->global->MAIN_USE_COMPANY_NAME_OF_CONTACT) || getDolGlobalInt('MAIN_USE_COMPANY_NAME_OF_CONTACT'))) {
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $config->mymodule->dir_temp,
